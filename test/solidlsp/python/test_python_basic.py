...
class TestPythonLanguageServerBasics:
    ...
    def test_request_references_user_class(
        ...
        assert (
            len(references) > 1
        ), "User class should be referenced in multiple files (using selectionRange if present)"

    @pytest.mark.parametrize("language_server", PYTHON_BACKEND_LANGUAGES, indirect=True)
    def test_request_references_item_class(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        assert (
            len(services_references) > 0
        ), "At least one reference should be in services.py (using selectionRange if present)"

    @pytest.mark.parametrize("language_server", PYTHON_BACKEND_LANGUAGES, indirect=True)
    def test_request_references_function_parameter(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        get_user_symbol = next(
            (s for s in symbols[0] if s.get("name") == "get_user"), None
        )
        ...
        assert (
            len(references) > 0
        ), "id parameter should be referenced within the method (using selectionRange if present)"

    @pytest.mark.parametrize("language_server", PYTHON_BACKEND_LANGUAGES, indirect=True)
    def test_request_references_create_user_method(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        create_user_symbol = next(
            (s for s in symbols[0] if s.get("name") == "create_user"), None
        )
        ...
        assert (
            len(references) > 1
        ), "Should get valid references for create_user (using selectionRange if present)"

    @pytest.mark.parametrize("language_server", PYTHON_LANGUAGE_BACKENDS, indirect=True)
    def test_request_text_document_diagnostics_with_filters(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        assert any(
            "missing_user" in message for message in diagnostic_messages
        ), diagnostic_messages
        assert any(
            "undefined_name" in message for message in diagnostic_messages
        ), diagnostic_messages

        factory_diagnostics = language_server.request_text_document_diagnostics(
            file_path, start_line=3, end_line=5, min_severity=1
        )
        ...
        assert all(
            "missing_user" in message for message in factory_messages
        ), factory_messages

        consumer_diagnostics = language_server.request_text_document_diagnostics(
            file_path, start_line=7, end_line=10, min_severity=1
        )
        consumer_messages = [
            diagnostic["message"] for diagnostic in consumer_diagnostics
        ]
        assert consumer_messages, "Expected diagnostics in broken_consumer range"
        assert all(
            "undefined_name" in message for message in consumer_messages
        ), consumer_messages
...
class TestProjectBasics:
    ...
    def test_retrieve_content_around_line(self, project: Project) -> None:
        ...
        with_context_around_user = project.retrieve_content_around_line(
            file_path, 31, 2, 2
        )
        ...
        assert (
            ...
            in with_context_around_user.matched_lines[0].line_content
        )
        assert with_context_around_user.num_matched_lines == 1
        assert (
            "    User model representing a system user."
            in with_context_around_user.lines[4].line_content
        )
        # Check line numbers
        ...
        assert (
            "return cls(id=id, name=name)" in with_context_above.lines[0].line_content
        )
        assert (
            "class User(BaseModel):" in with_context_above.matched_lines[0].line_content
        )
        ...
        assert (
            "class User(BaseModel):" in with_context_below.matched_lines[0].line_content
        )
        ...
        first_line_with_context_around = project.retrieve_content_around_line(
            file_path, 0, 2, 1
        )
        assert (
            len(first_line_with_context_around.lines) <= 4
        )  # Should have at most 4 lines (line 0 + 1 below + up to 2 above)
        assert (
            first_line_with_context_around.lines[0].line_number <= 2
        )  # First line should be at most line 2
        ...
        second_line_with_context_above = project.retrieve_content_around_line(
            file_path, 1, 3, 1
        )
        assert (
            len(second_line_with_context_above.lines) <= 5
        )  # Should have at most 5 lines (line 1 + 1 below + up to 3 above)
        assert (
            second_line_with_context_above.lines[0].line_number <= 1
        )  # First line should be at most line 1
        # Check match type for the target line
        ...
        last_line_with_context_around = project.retrieve_content_around_line(
            file_path, total_lines - 1, 1, 3
        )
        assert (
            len(last_line_with_context_around.lines) <= 5
        )  # Should have at most 5 lines (last line + 1 above + up to 3 below)
        assert (
            last_line_with_context_around.lines[-1].line_number >= total_lines - 4
        )  # Last line should be at least total_lines - 4
        # Check match type for the target line
    ...
    def test_search_files_for_pattern(self, project: Project) -> None:
        ...
        matches = project.search_source_files_for_pattern(
            user_class_pattern, paths_include_glob="**/models.py"
        )
        ...
        matches = project.search_source_files_for_pattern(
            method_pattern, paths_exclude_glob="**/models.py"
        )
        assert len(matches) > 0
        # Should find methods in services.py but not in models.py
        assert all(
            match.source_file_path is not None
            and "models.py" not in match.source_file_path
        ...
        assert any(
            match.source_file_path is not None and "models.py" in match.source_file_path
            for match in matches
        )
        assert any(
            match.source_file_path is not None
            and "services.py" in match.source_file_path
            for match in matches
        )
