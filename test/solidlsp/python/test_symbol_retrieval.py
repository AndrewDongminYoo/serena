...
class TestLanguageServerSymbols:
    ...
    def test_request_containing_symbol_function(
        ...
        containing_symbol = language_server.request_containing_symbol(
            file_path, 17, 20, include_body=True
        )
        ...
        if "body" in containing_symbol:
            assert (
                containing_symbol["body"]
                .get_text()
                .strip()
                .startswith("def create_user(self")
            )
    ...
    def test_references_to_variables(
        ...
        ref_symbols = [
            ref.symbol
            for ref in language_server.request_referencing_symbols(file_path, 74, 4)
        ]
    ...
    def test_request_referencing_symbols_function(
        ...
        assert (
            len(ref_symbols) > 0
        ), "No referencing symbols found for create_user (selectionRange)"
    ...
    def test_request_referencing_symbols_class(
        ...
        services_references = [
            ...
            and "services.py" in symbol["location"]["uri"]
        ]
        assert (
            len(services_references) > 0
        ), "No referencing symbols from services.py for User (selectionRange)"
    ...
    def test_request_referencing_symbols_parameter(
        ...
        assert (
            len(method_refs) > 0
        ), "No referencing symbols within method body for get_user (selectionRange)"
    ...
    def test_request_defining_symbol_nested_function(
        ...
        defining_symbol = language_server.request_defining_symbol(
            file_path, 9, 15
        )  # Position inside func_within_func
    ...
    def test_symbol_methods_integration(
        ...
        assert (
            defining_symbol["location"]["uri"] == containing_symbol["location"]["uri"]
        )
    ...
    def test_symbol_tree_structure(self, language_server: SolidLanguageServer) -> None:
        ...
        examples_package = next(
            child
            for child in repo_structure[0]["children"]
            if child["name"] == "examples"
        )
        # assert that children are __init__ and user_management
        assert {child["name"] for child in examples_package["children"]} == {
            "__init__",
            "user_management",
        }
        assert {child["kind"] for child in examples_package["children"]} == {
            SymbolKind.File
        }

        # assert that tree of user_management node is same as retrieved directly
        user_management_node = next(
            child
            for child in examples_package["children"]
            if child["name"] == "user_management"
        )
        if (
            "location" in user_management_node
            and "relativePath" in user_management_node["location"]
        ):
            user_management_rel_path = user_management_node["location"]["relativePath"]
            assert user_management_rel_path == os.path.join(
    ...
    def test_symbol_tree_structure_subdir(
        ...
        """Test that the symbol tree structure is correctly built."""
        # Get all symbols in the test file
        examples_package_roots = language_server.request_full_symbol_tree(
    ...
    def test_request_dir_overview(self, language_server: SolidLanguageServer) -> None:
        ...
        retrieved_symbols = {
            symbol["name"] for symbol in services_symbols if "name" in symbol
        }
    ...
    def test_request_document_overview(
        ...
        assert {"UserStats", "UserManager", "process_user_data", "main"}.issubset(
            symbol_names
        )
