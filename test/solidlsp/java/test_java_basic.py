...
pytestmark = [
    pytest.mark.java,
    pytest.mark.skipif(
        not language_tests_enabled(Language.JAVA), reason="Java tests disabled"
    ),
]
...
class TestJavaLanguageServer:
    ...
    def test_find_symbol(self, language_server: SolidLanguageServer) -> None:
        ...
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "Utils"
        ), "Utils class not found in symbol tree"
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "Model"
        ), "Model class not found in symbol tree"
    ...
    def test_find_referencing_symbols(
        ...
        ), "Main should reference Utils.printHello"
        ...
        for sym in symbols[0]:
            if sym.get("name") == "Model" and sym.get("kind") == 5:  # 5 = Class
                ...
                break
        assert (
            model_symbol is not None
        ), "Could not find 'Model' class symbol in Model.java"
    ...
    def test_overview_methods(self, language_server: SolidLanguageServer) -> None:
        ...
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "Main"
        ), "Main missing from overview"
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "Utils"
        ), "Utils missing from overview"
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "Model"
        ), "Model missing from overview"
    ...
    if language_has_verified_implementation_support(Language.JAVA):
        ...
        def test_find_implementations(
            ...
            pos = find_identifier_position(
                repo_path / "src/main/java/test_repo/Greeter.java", "formatGreeting"
            )
            ...
            implementations = language_server.request_implementation(
                "src/main/java/test_repo/Greeter.java", *pos
            )
            assert (
                implementations
            ), "Expected at least one implementation of Greeter.formatGreeting"
            assert any(
                "ConsoleGreeter.java" in implementation.get("relativePath", "")
    ...
    @pytest.mark.parametrize("language_server", [Language.JAVA], indirect=True)
    def test_lombok_generated_methods_visible_by_default(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        def _names_by_kind(doc, kind: int) -> set[str]:
            return {
                sym.get("name")
                for sym in doc.get_all_symbols_and_roots()[0]
        ...
        lombok_path = os.path.join(
            "src", "main", "java", "test_repo", "LombokModel.java"
        )
        ...
        for expected in (
            ...
            assert (
                expected in lombok_methods
            ), f"@Data did not surface {expected!r}; got: {sorted(lombok_methods)}"
        # @Builder(toBuilder=true): static factory + instance toBuilder + inner build()
        for expected in ("builder", "toBuilder", "build"):
            assert (
                expected in lombok_methods
            ), f"@Builder did not surface {expected!r}; got: {sorted(lombok_methods)}"
        # @With: copy-with methods
        for expected in ("withName", "withAge"):
            assert (
                expected in lombok_methods
            ), f"@With did not surface {expected!r}; got: {sorted(lombok_methods)}"
        # @Delegate: forwarder methods for every method of the delegate target
        for expected in ("greet", "farewell"):
            assert (
                expected in lombok_methods
            ), f"@Delegate did not surface forwarder {expected!r}; got: {sorted(lombok_methods)}"
        ...
        ), f"@Builder inner class missing; got: {sorted(lombok_classes)}"
        ...
        assert (
            "LombokModel" in lombok_ctors
        ), f"Lombok ctors missing; got ctors {sorted(lombok_ctors)}"

        # ---- FluentLombokModel: @Accessors(fluent=true) - prefix-stripped accessors ---------
        fluent_path = os.path.join(
            "src", "main", "java", "test_repo", "FluentLombokModel.java"
        )
        ...
        for expected in ("host", "tag"):
            assert (
                expected in fluent_methods
            ), f"@Accessors(fluent=true) did not surface {expected!r}; got: {sorted(fluent_methods)}"
