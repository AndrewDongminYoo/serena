...
def _make_fake_jdtls_install(
    ...
    *,
    ...
    with_native_fragments: bool = True,
    ...
    if with_native_fragments:
        (
            plugins
            / "org.eclipse.equinox.launcher.cocoa.macosx.aarch64_1.2.0.v20240329-1112.jar"
        ).touch()
        (
            plugins
            / "org.eclipse.equinox.launcher.gtk.linux.x86_64_1.2.0.v20240329-1112.jar"
        ).touch()
        (
            plugins
            / "org.eclipse.equinox.launcher.win32.win32.x86_64_1.2.0.v20240329-1112.jar"
        ).touch()
    if with_configs:
...
class TestResolveLauncherJar:
    def test_picks_main_launcher_excluding_native_fragments(
        self, tmp_path: Path
    ) -> None:
    ...
    def test_picks_highest_version_when_multiple_main_launchers(
        self, tmp_path: Path
    ) -> None:
        plugins = tmp_path / "plugins"
        plugins.mkdir()
        ...
        assert (
            result == newer
        ), "Expected the lexicographically highest launcher version to be selected"
    ...
    def test_raises_when_no_launcher_present(self, tmp_path: Path) -> None:
        ...
        with pytest.raises(
            SolidLSPException, match="No main Equinox launcher jar found"
        ):
...
class TestResolveConfigDir:
    ...
    def test_maps_platform_to_correct_config_dir(
        self, tmp_path: Path, platform_id: str, expected_dir: str
    ) -> None:
        _make_fake_jdtls_install(tmp_path)
        with patch(
            "solidlsp.language_servers.eclipse_jdtls.PlatformUtils.get_platform_id"
        ) as mock_get_pid:
    ...
    def test_raises_when_config_dir_missing(self, tmp_path: Path) -> None:
        ...
        with patch(
            ...
            with pytest.raises(
                SolidLSPException, match="Config directory .* not found"
            ):
...
class TestInspectJava:
    ...
    def test_parses_temurin_21_output(self) -> None:
        ...
        assert (
            home
            == "/Users/me/Library/Java/JavaVirtualMachines/temurin-21.0.10/Contents/Home"
        )
        assert major == 21
...
class TestResolveSystemJdk:
    ...
    def _patch_inspect_java(self, real_home: str, major: int):
        return patch.object(
            EclipseJDTLS.DependencyProvider,
            "_inspect_java",
            return_value=(real_home, major),
        )

    def test_uses_explicit_java_home_setting(
        ...
        with self._patch_inspect_java(str(explicit_jdk), 21):
            settings = SolidLSPSettings.CustomLSSettings(
                {"java_home": str(explicit_jdk)}
            )
            home, java_path = EclipseJDTLS.DependencyProvider._resolve_system_jdk(
                settings
            )
    ...
    def test_falls_back_to_java_home_env(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
    ...
    def test_falls_back_to_which_java(
        ...
        with patch(
            "solidlsp.language_servers.eclipse_jdtls.shutil.which",
            return_value=str(java_path),
        ):
            with self._patch_inspect_java(str(path_jdk), 21):
                home, _ = EclipseJDTLS.DependencyProvider._resolve_system_jdk(
                    custom_settings
                )
    ...
    def test_raises_when_no_java_anywhere(
        ...
        monkeypatch.delenv("JAVA_HOME", raising=False)
        with patch(
            "solidlsp.language_servers.eclipse_jdtls.shutil.which", return_value=None
        ):
            with pytest.raises(
                SolidLSPException, match="Could not locate a Java installation"
            ):
                EclipseJDTLS.DependencyProvider._resolve_system_jdk(custom_settings)

    def test_raises_for_invalid_explicit_java_home(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
    ...
    def test_raises_for_too_old_jdk(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        ...
        with self._patch_inspect_java(str(old_jdk), 17):
            settings = SolidLSPSettings.CustomLSSettings({})
            with pytest.raises(
                SolidLSPException, match=f"requires JDK {JDTLS_MIN_JDK_VERSION}"
            ):
                EclipseJDTLS.DependencyProvider._resolve_system_jdk(settings)
...
class TestSetupFromExistingInstall:
    ...
    def test_happy_path_returns_runtime_paths_with_no_gradle_and_no_intellicode(
        ...
        with patch(
            ...
            with patch.object(
                EclipseJDTLS.DependencyProvider,
                "_resolve_system_jdk",
                return_value=(str(jdk), str(jdk / "bin" / "java")),
            ):
                result = EclipseJDTLS.DependencyProvider._setup_from_existing_install(
                    str(jdtls_root), str(lombok_jar), custom_settings
                )
        ...
        assert Path(result.jdtls_launcher_jar_path).name.startswith(
    ...
    def test_raises_for_nonexistent_jdtls_path(
        ...
        with pytest.raises(SolidLSPException, match="not an existing directory"):
            EclipseJDTLS.DependencyProvider._setup_from_existing_install(
                str(tmp_path / "does-not-exist"), str(lombok_jar), custom_settings
            )
    ...
    def test_raises_when_plugins_dir_missing(
        ...
        with pytest.raises(SolidLSPException, match="'plugins/' directory not found"):
            EclipseJDTLS.DependencyProvider._setup_from_existing_install(
                str(empty_root), str(lombok_jar), custom_settings
            )
...
class TestSetupRuntimeDependenciesModeSwitch:
    ...
    def test_both_set_invokes_upstream_mode(self) -> None:
        ...
        with patch.object(
            ...
            result = EclipseJDTLS.DependencyProvider._setup_runtime_dependencies(
                "/ignored", settings
            )
    ...
    def test_only_jdtls_path_set_raises(self) -> None:
        ...
        with pytest.raises(SolidLSPException, match="must be set together"):
            EclipseJDTLS.DependencyProvider._setup_runtime_dependencies(
                "/ignored", settings
            )
...
class TestComputeWorkspaceHash:
    ...
    def test_default_mode_ignores_launcher_path(self) -> None:
        ...
        h1 = EclipseJDTLS.DependencyProvider._compute_workspace_hash(
            self.REPO, self.DEFAULT_LAUNCHER, empty_settings
        )
        h2 = EclipseJDTLS.DependencyProvider._compute_workspace_hash(
            self.REPO, self.UPSTREAM_LAUNCHER, empty_settings
        )
        assert h1 == h2
    ...
    def test_upstream_mode_includes_launcher_path(self) -> None:
        ...
        h1 = EclipseJDTLS.DependencyProvider._compute_workspace_hash(
            self.REPO, self.DEFAULT_LAUNCHER, settings
        )
        h2 = EclipseJDTLS.DependencyProvider._compute_workspace_hash(
            self.REPO, self.UPSTREAM_LAUNCHER, settings
        )
        assert h1 != h2
    ...
    def test_different_repo_paths_produce_different_hashes(self) -> None:
        empty_settings = SolidLSPSettings.CustomLSSettings({})
        h1 = EclipseJDTLS.DependencyProvider._compute_workspace_hash(
            "/a/repo", self.DEFAULT_LAUNCHER, empty_settings
        )
        h2 = EclipseJDTLS.DependencyProvider._compute_workspace_hash(
            "/b/repo", self.DEFAULT_LAUNCHER, empty_settings
        )
        assert h1 != h2
