...
class TestJsonLanguageServerBasics:
    ...
    def test_json_language_server_initialization(
    ...
    @pytest.mark.parametrize("language_server", [Language.JSON], indirect=True)
    @pytest.mark.parametrize("repo_path", [Language.JSON], indirect=True)
    def test_json_config_file_symbols(
        self, language_server: SolidLanguageServer, repo_path: Path
    ) -> None:
        ...
        assert (
            len(all_symbols) > 0
        ), f"Should find symbols in config.json, found {len(all_symbols)}"
    ...
    @pytest.mark.parametrize("language_server", [Language.JSON], indirect=True)
    @pytest.mark.parametrize("repo_path", [Language.JSON], indirect=True)
    def test_json_data_file_symbols(
        self, language_server: SolidLanguageServer, repo_path: Path
    ) -> None:
        ...
        assert (
            len(all_symbols) > 0
        ), f"Should find symbols in data.json, found {len(all_symbols)}"
