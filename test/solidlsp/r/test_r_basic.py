...
@pytest.mark.skipif(
    shutil.which("R") is None and not is_ci, reason="R is not available"
)
@pytest.mark.r
class TestRLanguageServer:
    ...
    def test_symbol_retrieval(self, language_server: SolidLanguageServer):
        ...
        assert expected_functions.issubset(
            function_names
        ), f"Expected functions {expected_functions} but found {function_names}"
    ...
    def test_find_references_across_files(self, language_server: SolidLanguageServer):
        ...
        references = language_server.request_references(
            analysis_file, 12, 15
        )  # cursor on 'calculate_mean'

        assert (
            references
        ), f"Expected non-empty references for calculate_mean but got {references=}"

        # Must find the definition in utils.R (cross-file reference)
        reference_files = [ref["uri"] for ref in references]
        assert any(
            uri.endswith("utils.R") for uri in reference_files
        ), "Cross-file reference to definition in utils.R not found"
