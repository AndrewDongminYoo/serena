...
class TestSolidityLanguageServerBasics:
    ...
    def test_solidity_language_server_initialization(
    ...
    @pytest.mark.parametrize("language_server", [Language.SOLIDITY], indirect=True)
    @pytest.mark.parametrize("repo_path", [Language.SOLIDITY], indirect=True)
    def test_token_contract_symbols(
        self, language_server: SolidLanguageServer, repo_path: Path
    ) -> None:
        ...
        assert (
            len(all_symbols) > 0
        ), f"Should find symbols in Token.sol, found {len(all_symbols)}"
        ...
        # Custom errors
        assert (
            "ZeroAddress" in symbol_names
        ), "Should detect the 'ZeroAddress' custom error"
        assert (
            "InsufficientBalance" in symbol_names
        ), "Should detect the 'InsufficientBalance' custom error"
        ...
        assert (
            "transferFrom" in symbol_names
        ), "Should detect the 'transferFrom' function"
    ...
    def test_interface_symbols(
        ...
        assert (
            len(all_symbols) > 0
        ), f"Should find symbols in IERC20.sol, found {len(all_symbols)}"
    ...
    def test_library_symbols(
        ...
        assert (
            len(all_symbols) > 0
        ), f"Should find symbols in SafeMath.sol, found {len(all_symbols)}"
    ...
    @pytest.mark.parametrize("language_server", [Language.SOLIDITY], indirect=True)
    @pytest.mark.parametrize("repo_path", [Language.SOLIDITY], indirect=True)
    def test_within_file_references(
        self, language_server: SolidLanguageServer, repo_path: Path
    ) -> None:
        ...
        references = language_server.request_references(
            "contracts/Token.sol", definition_line, definition_char
        )
        ...
        assert any(
            "Token.sol" in uri for uri in ref_files
        ), "References should include Token.sol"
    ...
    def test_cross_file_references(
        ...
        pos = _find_identifier_position(
            repo_path / "contracts/interfaces/IERC20.sol", "transfer"
        )
        assert pos is not None, "Should find 'transfer' identifier in IERC20.sol"
        definition_line, definition_char = pos

        references = language_server.request_references(
            "contracts/interfaces/IERC20.sol", definition_line, definition_char
        )

        assert (
            references is not None
        ), "Should return cross-file references for IERC20.transfer"
        assert (
            len(references) >= 1
        ), f"IERC20.transfer should be referenced at least once (in Token.sol), found {len(references)}"

        ref_files = {ref.get("uri", "") for ref in references}
        assert any(
            "Token.sol" in uri for uri in ref_files
        ), "IERC20.transfer references should include Token.sol"
    ...
    if language_has_verified_implementation_support(Language.SOLIDITY):
        ...
        def test_find_implementations(
            ...
            implementations = language_server.request_implementation(
                "contracts/interfaces/IERC20.sol", *pos
            )
            assert (
                implementations
            ), "Expected Token.transfer to be returned as an implementation"
            assert any(
                "Token.sol" in implementation.get("relativePath", "")
        ...
        def test_request_implementing_symbols(
            ...
            ), "Expected implementing symbols for IERC20.transfer"
            assert any(
                symbol.get("name") == "transfer"
                and "Token.sol" in symbol["location"].get("relativePath", "")
