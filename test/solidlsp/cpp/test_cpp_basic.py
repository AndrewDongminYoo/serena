...
@pytest.mark.cpp
@pytest.mark.skipif(
    not _cpp_servers, reason="No C++ language server (clangd or ccls) available"
)
class TestCppLanguageServer:
    ...
    def test_find_symbol(self, language_server: SolidLanguageServer) -> None:
        ...
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "add"
        ), "Function 'add' not found in symbol tree"
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "main"
        ), "Function 'main' not found in symbol tree"
    ...
    def test_find_referencing_symbols_across_files(
        ...
        assert any(
            "a.cpp" in ref_file for ref_file in ref_files
        ), f"Should find reference in a.cpp, {refs=}"
        ...
        refs2 = language_server.request_references(
            file_path, sel_start["line"], sel_start["character"]
        )
        assert sorted(map(_ref_key, refs2)) == sorted(
            map(_ref_key, refs)
        ), "Reference results should be stable across calls"

    @pytest.mark.parametrize("language_server", _cpp_servers, indirect=True)
    @pytest.mark.xfail(
        strict=True,
        reason=(
            "Both clangd and ccls do not support cross-file references for newly created files that were never opened by the LS."
        ),
    )
    def test_find_references_in_newly_written_file(
        self, language_server: SolidLanguageServer
    ) -> None:
        # Create a new file that references the 'add' function from b.cpp
        new_file_path = os.path.join("temp_new_file.cpp")
        new_file_abs_path = os.path.join(
            language_server.repository_root_path, new_file_path
        )
...
""")
            ...
            with language_server.open_file(new_file_path):
                ...
                new_file_symbols = language_server.request_document_symbols(
                    new_file_path
                ).get_all_symbols_and_roots()
            ...
            assert (
                uri in language_server.open_file_buffers
            ), "File should remain in open_file_buffers"
            ...
            ), "Could not find 'add' function symbol in b.cpp"

            # Request references for 'add'
            sel_start = add_symbol["selectionRange"]["start"]
            refs = language_server.request_references(
                b_file_path, sel_start["line"], sel_start["character"]
            )
            ref_files = [ref.get("relativePath", "") for ref in refs]
...
class TestCppDocumentSymbolCache:
    ...
    def test_cache_invalidates_when_clangd_context_changes(
        self, tmp_path: Path
    ) -> None:
        ...
        with start_ls_context(
            Language.CPP, repo_path=str(repo_path), solidlsp_dir=tmp_path
        ) as ls_default:
