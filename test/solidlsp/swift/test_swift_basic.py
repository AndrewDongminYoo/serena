...
WINDOWS_SKIP_REASON = (
    "GitHub Actions configuration for Swift on Windows is complex, skipping for now."
)

pytestmark = [
    pytest.mark.swift,
    pytest.mark.skipif(WINDOWS_SKIP, reason=WINDOWS_SKIP_REASON),
]
...
class TestSwiftLanguageServerBasics:
    ...
    def test_goto_definition_calculator_class(
        ...
        assert calculator_def.get("uri", "").endswith(
            "main.swift"
        ), "Definition should be in main.swift"

        # The Calculator class is defined starting at line 16
        start_line = calculator_def.get("range", {}).get("start", {}).get("line")
        assert (
            start_line == 15
        ), f"Calculator class definition should be at line 16, got {start_line + 1}"

    @pytest.mark.parametrize("language_server", [Language.SWIFT], indirect=True)
    def test_goto_definition_user_struct(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        assert user_def.get("uri", "").endswith(
            "main.swift"
        ), "Definition should be in main.swift"
        ...
        assert (
            start_line == 25
        ), f"User struct definition should be at line 26, got {start_line + 1}"

    @pytest.mark.parametrize("language_server", [Language.SWIFT], indirect=True)
    def test_goto_definition_calculator_method(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        assert (
            start_line == 16
        ), f"add method definition should be at line 17, got {start_line + 1}"
    ...
    def test_goto_definition_cross_file(
        ...
        definitions = language_server.request_definition(
            utils_file, sel_start["line"], sel_start["character"]
        )
        assert isinstance(definitions, list), "Definitions should be a list"

        # Should find the Utils struct definition itself
        utils_def = definitions[0]
        assert utils_def.get("uri", "").endswith(
            "utils.swift"
        ), "Definition should be in utils.swift"

    @pytest.mark.xfail(is_ci, reason="Test is flaky in CI")  # See #1040
    @pytest.mark.parametrize("language_server", [Language.SWIFT], indirect=True)
    def test_request_references_calculator_class(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        calculator_refs = [
            ref for ref in references if ref.get("uri", "").endswith("main.swift")
        ]
        assert (
            len(calculator_refs) > 0
        ), "Calculator class should be referenced in main.swift"

        # Check that one reference is at line 5 (let calculator = Calculator())
        line_5_refs = [
            ref
            for ref in calculator_refs
            if ref.get("range", {}).get("start", {}).get("line") == 4
        ]
        assert len(line_5_refs) > 0, "Calculator should be referenced at line 5"

    @pytest.mark.xfail(is_ci, reason="Test is flaky in CI")  # See #1040
    @pytest.mark.parametrize("language_server", [Language.SWIFT], indirect=True)
    def test_request_references_user_struct(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        line_9_refs = [
            ...
            if ref.get("range", {}).get("start", {}).get("line") == 8
        ]
        assert len(line_9_refs) > 0, "User should be referenced at line 9"

    @pytest.mark.xfail(is_ci, reason="Test is flaky in CI")  # See #1040
    @pytest.mark.parametrize("language_server", [Language.SWIFT], indirect=True)
    def test_request_references_utils_struct(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        line_12_refs = [
            ref
            for ref in utils_refs
            if ref.get("range", {}).get("start", {}).get("line") == 11
        ]
    ...
    def test_retrieve_content_around_line(self, project: Project) -> None:
        ...
        for line_num in range(1, 50):  # Search first 50 lines
            try:
                ...
                if (
                    line_content.lines
                    and "class Calculator" in line_content.lines[0].line_content
                ):
        ...
        with_context_around_calculator = project.retrieve_content_around_line(
            file_path, calculator_line, 2, 2
        )
        assert len(with_context_around_calculator.lines) == 5
        assert (
            "class Calculator"
            in with_context_around_calculator.matched_lines[0].line_content
        )
