...
@pytest.mark.skipif(
    ...
    reason="ansible-language-server does not support native Windows operation",
)
@pytest.mark.ansible
class TestAnsibleLanguageServerBasics:
    ...
    @pytest.mark.parametrize("language_server", [Language.ANSIBLE], indirect=True)
    def test_hover_on_module_contains_documentation(
        ...
        assert (
            "package" in hover_text.lower()
        ), f"Hover should mention 'package', got: {hover_text[:300]}"

    @pytest.mark.parametrize("language_server", [Language.ANSIBLE], indirect=True)
    def test_completions_contain_module_names(
        self, language_server: SolidLanguageServer
    ) -> None:
    ...
    @pytest.mark.xfail(
        reason="Seems like ansible LS lacks basic functionality at the moment, textDocument/documentSymbol doesn't work"
    )
    @pytest.mark.parametrize("language_server", [Language.ANSIBLE], indirect=True)
