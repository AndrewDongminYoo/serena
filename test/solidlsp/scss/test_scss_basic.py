...
class TestScssLanguageServerBasics:
    ...
    @pytest.mark.parametrize("language_server", [Language.SCSS], indirect=True)
    def test_variables_document_symbols(
        ...
        for var in (
            ...
            assert (
                var in names
            ), f"Expected variable {var} to appear in SCSS symbols: {names}"

    @pytest.mark.parametrize("language_server", [Language.SCSS], indirect=True)
    def test_mixins_document_symbols(
        self, language_server: SolidLanguageServer
    ) -> None:
        all_symbols, _ = language_server.request_document_symbols(
            "_mixins.scss"
        ...
        for expected in ("card-surface", "focus-ring", "rem"):
            assert (
                expected in joined
            ), f"Expected '{expected}' to appear in SCSS mixin symbols: {names}"

    @pytest.mark.parametrize("language_server", [Language.SCSS], indirect=True)
    def test_buttons_document_symbols(
        self, language_server: SolidLanguageServer
    ) -> None:
        all_symbols, _ = language_server.request_document_symbols(
            "buttons.scss"
        ...
        for selector in (".button", ".button-primary", ".button-secondary"):
            assert (
                selector in joined
            ), f"Expected selector '{selector}' to appear in SCSS symbols: {names}"

    @pytest.mark.parametrize("language_server", [Language.SCSS], indirect=True)
    def test_cross_file_definition_variable(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        coords = find_text_coordinates(
            read_repo_file(language_server, path), f"({re.escape(needle)})"
        )
        ...
        assert (
            definitions
        ), f"Expected non-empty cross-file definition list for vars.$color-text, got {definitions}"
        target_uris = [d["uri"] for d in definitions]
        assert any(
            uri.endswith("_variables.scss") for uri in target_uris
        ), f"Expected definition to resolve into _variables.scss, got URIs: {target_uris}"

    @pytest.mark.parametrize("language_server", [Language.SCSS], indirect=True)
    def test_cross_file_definition_mixin(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        assert (
            definitions
        ), f"Expected non-empty cross-file definition list for mix.card-surface, got {definitions}"
        target_uris = [d["uri"] for d in definitions]
        assert any(
            uri.endswith("_mixins.scss") for uri in target_uris
        ), f"Expected definition to resolve into _mixins.scss, got URIs: {target_uris}"

    @pytest.mark.parametrize("language_server", [Language.SCSS], indirect=True)
    def test_cross_file_definition_function(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        assert (
            definitions
        ), f"Expected non-empty cross-file definition list for mix.rem, got {definitions}"
        target_uris = [d["uri"] for d in definitions]
        assert any(
            uri.endswith("_mixins.scss") for uri in target_uris
        ), f"Expected definition to resolve into _mixins.scss, got URIs: {target_uris}"
    ...
    @pytest.mark.parametrize("language_server", [Language.SCSS], indirect=True)
    def test_full_symbol_tree_includes_all_files(
        self, language_server: SolidLanguageServer
    ) -> None:
        all_symbols = request_all_symbols(language_server)
        relative_paths = {
            s.get("location", {}).get("relativePath") for s in all_symbols
        }
...
class TestScssReferences:
    ...
    @pytest.mark.parametrize("language_server", [Language.SCSS], indirect=True)
    def test_mixin_references_span_files(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        assert any(
            p.endswith("buttons.scss") for p in ref_paths
        ), f"Expected card-surface references to include buttons.scss, got: {ref_paths}"
        assert any(
            p.endswith("main.scss") for p in ref_paths
        ), f"Expected card-surface references to include main.scss, got: {ref_paths}"

    @pytest.mark.parametrize("language_server", [Language.SCSS], indirect=True)
    def test_variable_references_span_files(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        assert any(
            p.endswith("_mixins.scss") for p in ref_paths
...
class TestScssForward:
    ...
    @pytest.mark.parametrize("language_server", [Language.SCSS], indirect=True)
    def test_forwarded_buttons_appear_in_workspace(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        button_symbols = [
            ...
            if s.get("location", {}).get("relativePath") == "buttons.scss"
        ]
        names = [s["name"] for s in button_symbols]
        joined = " | ".join(names)
        assert (
            ".button" in joined
        ), f"Expected .button selector to remain reachable via @forward, got: {names}"
...
class TestScssHover:
    ...
    def test_hover_on_variable_use(self, language_server: SolidLanguageServer) -> None:
        ...
        assert (
            hover is not None
        ), f"Expected hover info for $color-text in {path}, got None"
        ...
        assert (
            "color-text" in text
        ), f"Expected '$color-text' or its value in hover text, got: {text}"
    ...
    def test_hover_on_mixin_call(self, language_server: SolidLanguageServer) -> None:
        ...
        assert (
            hover is not None
        ), f"Expected hover info for card-surface in {path}, got None"
        contents = hover.get("contents")
        assert contents, f"Expected non-empty hover contents, got: {hover}"
        text = contents["value"] if isinstance(contents, dict) else str(contents)
        assert (
            "card-surface" in text
        ), f"Expected 'card-surface' in hover text, got: {text}"
...
class TestScssCompletions:
    ...
    @pytest.mark.parametrize("language_server", [Language.SCSS], indirect=True)
    def test_completion_after_namespace_dot(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        assert any(
            "color-primary" in label for label in labels
        ), f"Expected $color-primary completion after `vars.` prefix, got: {joined}"
...
class TestScssSymbolKinds:
    ...
    def test_variable_symbol_kind(self, language_server: SolidLanguageServer) -> None:
        ...
        assert (
            "$color-primary" in by_name
        ), f"Variable not in symbol list: {list(by_name)}"
    ...
    @pytest.mark.parametrize("language_server", [Language.SCSS], indirect=True)
    def test_mixin_and_function_symbol_kinds(
        self, language_server: SolidLanguageServer
    ) -> None:
        all_symbols, _ = language_server.request_document_symbols(
            "_mixins.scss"
        ...
        def find_one(needle: str) -> dict:
            ...
            assert (
                matches
            ), f"No symbol matched '{needle}' in {[s['name'] for s in all_symbols]}"
            return matches[0]
        ...
        assert (
            SymbolKind(mixin["kind"]) in callable_kinds
        ), f"Expected card-surface kind in {{Method, Function}}, got {SymbolKind(mixin['kind']).name}"
        assert (
            SymbolKind(func["kind"]) in callable_kinds
        ), f"Expected rem kind in {{Method, Function}}, got {SymbolKind(func['kind']).name}"

    @pytest.mark.parametrize("language_server", [Language.SCSS], indirect=True)
    def test_completion_kind_is_meaningful(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        completions = language_server.request_completions(path, line, col + 5)
        var_items = [
            c for c in completions if "color-primary" in c.get("completionText", "")
        ]
        ...
        assert any(
            LSPTypes.CompletionItemKind(c["kind"]) in accepted for c in var_items
...
class TestSomeSassWithPlainCss:
    ...
    @pytest.mark.parametrize("language_server", [Language.SCSS], indirect=True)
    def test_main_css_document_symbols(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        for selector in (
            ...
            assert (
                selector in joined
            ), f"Expected selector '{selector}' to appear in CSS symbols: {names}"

    @pytest.mark.parametrize("language_server", [Language.SCSS], indirect=True)
    def test_theme_css_document_symbols(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        assert (
            ":root" in joined
        ), f"Expected ':root' selector to appear in CSS symbols: {names}"

    @pytest.mark.parametrize("language_server", [Language.SCSS], indirect=True)
    def test_full_symbol_tree_includes_css_files(
        self, language_server: SolidLanguageServer
    ) -> None:
    ...
    def test_hover_on_css_property(self, language_server: SolidLanguageServer) -> None:
        ...
        assert (
            hover is not None
        ), f"Expected hover info for background-color in {path}, got None"
        contents = hover.get("contents")
        assert contents, f"Expected non-empty hover contents, got: {hover}"
        text = contents["value"] if isinstance(contents, dict) else str(contents)
        assert (
            "background" in text.lower()
        ), f"Expected hover text to mention 'background', got: {text}"

    @pytest.mark.parametrize("language_server", [Language.SCSS], indirect=True)
    def test_property_completion_in_css_rule(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        labels = {c.get("completionText", "") for c in completions}
        assert any(
            label in labels for label in ("padding", "margin", "color", "border")
        ), f"Expected at least one common CSS property name in completions, got sample: {sorted(labels)[:20]}"

    @pytest.mark.parametrize("language_server", [Language.SCSS], indirect=True)
    def test_cross_file_completion_for_css_custom_property(
        self, language_server: SolidLanguageServer
    ) -> None:
