...
class TestLean4LanguageServer:
    ...
    def test_within_file_references(self, language_server: SolidLanguageServer) -> None:
        ...
        references = language_server.request_references(
            "Helper.lean", line=11, column=4
        )

        assert (
            len(references) >= 1
        ), f"Expected at least 1 reference to isPositive (used in absolute), got {len(references)}"

        # Check that isPositive is referenced within Helper.lean at line 15 (absolute calls isPositive)
        ref_locations = [
            (ref["relativePath"], ref["range"]["start"]["line"]) for ref in references
        ]
        helper_refs = [
            (path, line) for path, line in ref_locations if "Helper.lean" in path
        ]
        assert any(
    ...
    @pytest.mark.parametrize("language_server", [Language.LEAN4], indirect=True)
    def test_cross_file_references_add(
        ...
        assert (
            len(references) >= 1
        ), f"Expected at least 1 reference to add in Main.lean, got {len(references)}"
        ...
        main_refs = [
            (path, line) for path, line in ref_locations if "Main.lean" in path
        ]
        assert (
            len(main_refs) >= 1
        ), f"Expected at least 1 reference to add in Main.lean, got: {ref_locations}"
    ...
    @pytest.mark.parametrize("language_server", [Language.LEAN4], indirect=True)
    def test_cross_file_references_calculator(
        self, language_server: SolidLanguageServer
    ) -> None:
        """
        ...
        references = language_server.request_references(
            "Helper.lean", line=0, column=10
        )

        assert (
            len(references) >= 1
        ), f"Expected at least 1 reference to Calculator in Main.lean, got {len(references)}"
        ...
        assert (
            len(main_refs) >= 1
        ), f"Expected at least 1 reference to Calculator in Main.lean, got: {ref_locations}"
        main_ref_lines = {line for _, line in main_refs}
    ...
    def test_go_to_definition_within_file(
        ...
        assert (
            len(definitions) >= 1
        ), f"Expected at least 1 definition for calculate, got {len(definitions)}"

        def_location = definitions[0]
        assert def_location["uri"].endswith(
            "Main.lean"
        ), f"Expected definition in Main.lean, got: {def_location['uri']}"
        assert (
            def_location["range"]["start"]["line"] == 5
        ), f"Expected definition at line 5, got: {def_location['range']['start']['line']}"

    @pytest.mark.parametrize("language_server", [Language.LEAN4], indirect=True)
    def test_go_to_definition_across_files(
        self, language_server: SolidLanguageServer
    ) -> None:
        """
        ...
        definitions = language_server.request_definition(
            "Main.lean", line=15, column=19
        )

        assert (
            len(definitions) >= 1
        ), f"Expected at least 1 definition for add, got {len(definitions)}"

        def_location = definitions[0]
        assert def_location["uri"].endswith(
            "Helper.lean"
        ), f"Expected definition in Helper.lean, got: {def_location['uri']}"
        assert (
            def_location["range"]["start"]["line"] == 5
        ), f"Expected definition at line 5, got: {def_location['range']['start']['line']}"
