...
@pytest.mark.skipif(
    shutil.which("regal") is None and not is_ci, reason="Regal is not available"
)
@pytest.mark.rego
class TestRegoLanguageServer:
    ...
    def test_request_document_symbols_authz(
    ...
    @pytest.mark.parametrize("language_server", [Language.REGO], indirect=True)
    def test_request_document_symbols_helpers(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        assert (
            "is_valid_username" in symbol_names
        ), "is_valid_username function not found"
    ...
    def test_find_symbol_full_tree(self, language_server: SolidLanguageServer) -> None:
        ...
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "allow"
        ), "allow rule not found in symbol tree"
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "is_valid_user"
        ), "is_valid_user function not found in symbol tree"
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "is_admin"
        ), "is_admin function not found in symbol tree"

    @pytest.mark.parametrize("language_server", [Language.REGO], indirect=True)
    def test_request_definition_within_file(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        is_admin_symbol = next(
            (s for s in symbol_list if s.get("name") == "is_admin"), None
        )
        assert (
            is_admin_symbol is not None
        ), "is_admin symbol should always be found in authz.rego"
        ...
        ), "Should find definition for admin_roles"
    ...
    @pytest.mark.parametrize("language_server", [Language.REGO], indirect=True)
    def test_request_definition_across_files(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        assert (
            allow_symbol is not None
        ), "allow symbol should always be found in authz.rego"
        ...
        assert (
            definitions is not None and len(definitions) > 0
        ), "Should find cross-file definition for is_valid_user"
    ...
    @pytest.mark.parametrize("language_server", [Language.REGO], indirect=True)
    def test_find_symbols_validation(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        assert (
            "validate_user_input" in symbol_names
        ), "validate_user_input rule not found"
        assert (
            "has_valid_credentials" in symbol_names
        ), "has_valid_credentials function not found"
