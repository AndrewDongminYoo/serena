    ...
    if language_has_verified_implementation_support(Language.RUST):
        ...
        def test_find_implementations(
            ...
            pos = find_identifier_position(
                repo_path / os.path.join("src", "lib.rs"), "format_greeting"
            )
            ...
            implementations = language_server.request_implementation(
                os.path.join("src", "lib.rs"), *pos
            )
            assert (
                implementations
            ), "Expected at least one implementation of Greeter.format_greeting"
            assert any(
                "src/lib.rs"
                in implementation.get("relativePath", "").replace("\\", "/")
        ...
        def test_request_implementing_symbols(
            ...
            ), "Expected implementing symbols for Greeter.format_greeting"
            assert any(
                symbol.get("name") == "format_greeting"
                and "src/lib.rs"
                in symbol["location"].get("relativePath", "").replace("\\", "/")
