...
class TestRust2024EditionLanguageServer:
    ...
    def test_find_references_raw(self, rust_language_server) -> None:
        ...
        symbols = rust_language_server.request_document_symbols(
            file_path
        ...
        refs = rust_language_server.request_references(
        ...
        ), "main.rs should reference add function"
    ...
    def test_find_symbol(self, rust_language_server) -> None:
        ...
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "multiply"
        ), "multiply function not found in symbol tree"
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "Calculator"
        ), "Calculator struct not found in symbol tree"
    ...
    def test_find_referencing_symbols_multiply(self, rust_language_server) -> None:
        ...
        for sym in symbols[0]:
            if sym.get("name") == "multiply":
                ...
                break
        assert (
            multiply_symbol is not None
        ), "Could not find 'multiply' function symbol in lib.rs"
        ...
        assert isinstance(
            refs, list
        ), "Should return a list of references (even if empty)"
    ...
    def test_find_calculator_struct_and_impl(self, rust_language_server) -> None:
        ...
        for sym in symbols[0]:
            if sym.get("name") == "Calculator" and sym.get("kind") == 23:  # Struct kind
                ...
                sym.get("name") == "Calculator" and sym.get("kind") == 11
            ):  # Interface/Impl kind
                calculator_impl = sym

        assert (
            calculator_struct is not None
        ), "Could not find 'Calculator' struct symbol in lib.rs"
        ...
        if calculator_impl is not None:
            ...
            assert (
                "get_result" in method_names
            ), "Calculator impl should have 'get_result' method"
    ...
    def test_overview_methods(self, rust_language_server) -> None:
        ...
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "add"
        ), "add missing from overview"
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "multiply"
        ), "multiply missing from overview"
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "Calculator"
        ), "Calculator missing from overview"
