...
class TestVueLanguageServer:
    ...
    def test_vue_files_in_symbol_tree(
        self, language_server: SolidLanguageServer
    ) -> None:
        symbols = language_server.request_full_symbol_tree()
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "App"
        ), "App not found in symbol tree"
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "CalculatorButton"
        ), "CalculatorButton not found in symbol tree"
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "CalculatorInput"
        ), "CalculatorInput not found in symbol tree"
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "CalculatorDisplay"
        ), "CalculatorDisplay not found in symbol tree"
    ...
    def test_find_referencing_symbols(
        ...
        assert (
            len(refs) >= 4
        ), f"useCalculatorStore should have at least 4 references (definition + 3 usages), got {len(refs)}"

        # Verify we have references from .vue files
        vue_refs = [ref for ref in refs if ".vue" in ref.get("relativePath", "")]
        assert (
            len(vue_refs) >= 3
        ), f"Should have at least 3 Vue component references, got {len(vue_refs)}"


@pytest.mark.vue
class TestVueDualLspArchitecture:
    @pytest.mark.parametrize("language_server", [Language.VUE], indirect=True)
    def test_typescript_server_coordination(
        self, language_server: SolidLanguageServer
    ) -> None:
        ts_file = os.path.join("src", "stores", "calculator.ts")
        ts_symbols = language_server.request_document_symbols(
            ts_file
        ).get_all_symbols_and_roots()
        ...
        assert (
            len(ts_symbols[0]) >= 5
        ), f"TypeScript server should return multiple symbols for calculator.ts, got {len(ts_symbols[0])}"
        assert (
            "useCalculatorStore" in ts_symbol_names
        ), "TypeScript server should extract store function"

        # Verify Vue server can parse .vue files
        vue_file = os.path.join("src", "App.vue")
        vue_symbols = language_server.request_document_symbols(
            vue_file
        ).get_all_symbols_and_roots()
        vue_symbol_names = [s.get("name") for s in vue_symbols[0]]

        assert (
            len(vue_symbols[0]) >= 15
        ), f"Vue server should return at least 15 symbols for App.vue, got {len(vue_symbols[0])}"
        assert (
            "appTitle" in vue_symbol_names
        ), "Vue server should extract ref declarations from script setup"

    @pytest.mark.parametrize("language_server", [Language.VUE], indirect=True)
    def test_cross_file_references_vue_to_typescript(
        self, language_server: SolidLanguageServer
    ) -> None:
        store_file = os.path.join("src", "stores", "calculator.ts")
        store_symbols = language_server.request_document_symbols(
            store_file
        ).get_all_symbols_and_roots()
        ...
        assert (
            len(refs) >= 4
        ), f"useCalculatorStore should have at least 4 references (definition + 3 usages), found {len(refs)} references"
        ...
        for expected_file in expected_vue_files:
            matching_refs = [
                ref for ref in vue_refs if expected_file in ref.get("uri", "")
            ]
            if matching_refs:
    ...
    @pytest.mark.parametrize("language_server", [Language.VUE], indirect=True)
    def test_cross_file_references_typescript_to_vue(
        self, language_server: SolidLanguageServer
    ) -> None:
        types_file = os.path.join("src", "types", "index.ts")
        types_symbols = language_server.request_document_symbols(
            types_file
        ).get_all_symbols_and_roots()
        ...
        assert (
            "Operation" in types_symbol_names
        ), "Operation type should exist in types file"
        ...
        refs = language_server.request_references(
            types_file, sel_start["line"], sel_start["character"]
        )

        # Verify we found references: definition + usage in calculator.ts and Vue files
        assert (
            len(refs) >= 2
        ), f"Operation type should have at least 2 references (definition + usages), found {len(refs)} references"
...
class TestVueEdgeCases:
    ...
    def test_symbol_tree_structure(self, language_server: SolidLanguageServer) -> None:
        ...
        components_files = list(
            {p for p in all_paths if "components" in p and ".vue" in p}
        )
        stores_files = list({p for p in all_paths if "stores" in p and ".ts" in p})
        composables_files = list(
            {p for p in all_paths if "composables" in p and ".ts" in p}
        )
        ...
        for expected_file in expected_files:
            ...
            ), f"Expected file '{expected_file}' should be in symbol tree. All paths: {all_paths}"
    ...
    def test_document_overview(self, language_server: SolidLanguageServer) -> None:
        ...
        assert isinstance(
            overview, list
        ), f"Overview should be a list, got: {type(overview)}"
        assert (
            len(overview) >= 1
        ), f"App.vue should have at least 1 top-level symbol in overview, got {len(overview)}"
        ...
        assert isinstance(
            store_overview, list
        ), f"Store overview should be a list, got: {type(store_overview)}"
        assert (
            len(store_overview) >= 1
        ), f"calculator.ts should have at least 1 top-level symbol in overview, got {len(store_overview)}"

        store_symbol_names = [
            s.get("name") for s in store_overview if isinstance(s, dict)
        ]
        assert (
        ...
        assert isinstance(
            button_overview, list
        ), f"Button overview should be a list, got: {type(button_overview)}"
        assert (
        ...
        button_symbol_names = [
            s.get("name") for s in button_overview if isinstance(s, dict)
        ]
    ...
    def test_directory_overview(self, language_server: SolidLanguageServer) -> None:
        ...
        assert isinstance(
            dir_overview, dict
        ), f"Directory overview should be a dict, got: {type(dir_overview)}"
        assert (
            len(dir_overview) == 3
        ), f"src/components directory should have exactly 3 files in overview, got {len(dir_overview)}"
        ...
        for expected_component in expected_components:
            ...
            matching_files = [
                path for path in dir_overview.keys() if expected_component in path
            ]
            ...
            assert isinstance(
                symbols, list
            ), f"Symbols for {file_path} should be a list, got {type(symbols)}"
            assert (
                len(symbols) >= 1
            ), f"Component {expected_component} should have at least 1 symbol in overview, got {len(symbols)}"
        ...
        assert isinstance(
            stores_overview, dict
        ), f"Stores overview should be a dict, got: {type(stores_overview)}"
        assert (
        ...
        calculator_files = [
            path for path in stores_overview.keys() if "calculator.ts" in path
        ]
        ...
        store_symbol_names = [
            s.get("name") for s in store_symbols if isinstance(s, dict)
        ]
        assert (
            "useCalculatorStore" in store_symbol_names
        ...
        assert isinstance(
            composables_overview, dict
        ), f"Composables overview should be a dict, got: {type(composables_overview)}"
        assert (
        ...
        for expected_composable in expected_composables:
            matching_files = [
                path
                for path in composables_overview.keys()
                if expected_composable in path
            ]
            assert len(matching_files) == 1, (
