...
@pytest.mark.skipif(
    shutil.which("elm") is None and not is_ci, reason="Elm compiler is not available"
)
@pytest.mark.elm
class TestElmLanguageServer:
    ...
    def test_find_symbol(self, language_server: SolidLanguageServer) -> None:
        ...
        ), "calculateSum function not found in symbol tree"
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "formatMessage"
        ), "formatMessage function not found in symbol tree"
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "addNumbers"
        ), "addNumbers function not found in symbol tree"
    ...
    def test_find_references_across_files(
        ...
        for sym in symbols[0]:
            if sym.get("name") == "formatMessage":
                ...
                break
        assert (
            formatMessage_symbol is not None
        ), "Could not find 'formatMessage' symbol in Utils.elm"

        # Get references from the definition in Utils.elm
        sel_start = formatMessage_symbol["selectionRange"]["start"]
        refs = language_server.request_references(
            utils_path, sel_start["line"], sel_start["character"]
        )
        ...
        assert any(
            "Main.elm" in ref.get("relativePath", "") for ref in refs
        ), "Expected to find usage of formatMessage in Main.elm"
