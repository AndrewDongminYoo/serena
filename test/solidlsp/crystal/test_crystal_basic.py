...
pytestmark = [
    pytest.mark.crystal,
    pytest.mark.skipif(
        shutil.which("crystalline") is None, reason="Crystalline is not installed"
    ),
]
...
class TestCrystalDocumentSymbols:
    ...
    def test_document_symbols_main(self, language_server: SolidLanguageServer) -> None:
        ...
        assert (
            "Calculator" in symbol_names
        ), f"Calculator not found in symbols. Found: {symbol_names}"
        assert (
            "User" in symbol_names
        ), f"User not found in symbols. Found: {symbol_names}"
    ...
    def test_document_symbols_utils(self, language_server: SolidLanguageServer) -> None:
        ...
        symbol_names = [s.get("name") for s in all_symbols if s.get("name")]
        assert (
            "Utils" in symbol_names
        ), f"Utils not found in symbols. Found: {symbol_names}"
    ...
    def test_find_symbol(self, language_server: SolidLanguageServer) -> None:
        ...
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "Calculator"
        ), "Calculator not found in symbol tree"
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "User"
        ), "User not found in symbol tree"
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "Utils"
        ), "Utils not found in symbol tree"
...
class TestCrystalDefinition:
    ...
    @pytest.mark.parametrize("language_server", [Language.CRYSTAL], indirect=True)
    def test_goto_definition_within_file(
        ...
        assert calculator_def.get("uri", "").endswith(
            "main.cr"
        ), "Definition should be in main.cr"
        assert (
            calculator_def["range"]["start"]["line"] == 2
        ), "Calculator class should be defined at line 3 (0-indexed: 2)"
