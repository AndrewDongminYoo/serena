...
class TestBashLanguageServerBasics:
    ...
    def test_bash_language_server_initialization(
    ...
    @pytest.mark.parametrize("language_server", [Language.BASH], indirect=True)
    def test_bash_request_document_symbols_with_body(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        greet_user_symbol = next(
            (sym for sym in function_symbols if sym["name"] == "greet_user"), None
        )
        ...
        if "body" in greet_user_symbol:
            ...
            assert (
                "function greet_user()" in body
            ), "Function body should contain function definition"
    ...
    def test_bash_utils_functions(self, language_server: SolidLanguageServer) -> None:
        ...
        for func_name in expected_utils_functions:
            assert (
                func_name in utils_function_names
            ), f"Should find {func_name} function in utils.sh"

        assert (
            len(utils_function_symbols) >= 8
        ), f"Should find at least 8 functions in utils.sh, found {len(utils_function_symbols)}"

    @pytest.mark.parametrize("language_server", [Language.BASH], indirect=True)
    def test_bash_function_syntax_patterns(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        assert (
            "main" in main_function_names
        ), "LSP should detect traditional function syntax"

        # Functions with 'function' keyword: function name() {
        assert (
            "greet_user" in main_function_names
        ), "LSP should detect function keyword syntax"
        assert (
            "process_items" in main_function_names
        ), "LSP should detect function keyword syntax"
        ...
        for expected_func in expected_utils:
            assert (
                expected_func in utils_function_names
            ), f"LSP should detect {expected_func} function"

        # Verify total counts match expectations
        assert (
            len(main_functions) >= 3
        ), f"Should find at least 3 functions in main.sh, found {len(main_functions)}"
        assert (
            len(utils_functions) >= 8
        ), f"Should find at least 8 functions in utils.sh, found {len(utils_functions)}"
