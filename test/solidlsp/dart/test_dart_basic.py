...
class TestDartLanguageServer:
    ...
    def test_find_definition_across_files(
        ...
        if not definition_location_list:
            pytest.skip(
                "Language server doesn't support cross-file definition lookup for this case"
            )
    ...
    @pytest.mark.parametrize("language_server", [Language.DART], indirect=True)
    @pytest.mark.parametrize("repo_path", [Language.DART], indirect=True)
    def test_find_definition_class_method(
        self, language_server: SolidLanguageServer, repo_path: Path
    ) -> None:
    ...
    def test_find_references_within_file(
        ...
        main_dart_references = [
            ref for ref in references if ref["uri"].endswith("main.dart")
        ]
    ...
    def test_find_references_across_files(
        ...
        references = language_server.request_references(
            helper_dart_path, 4, 4
        )  # cursor on 'subtract' in definition

        assert (
            references
        ), f"Expected non-empty references for subtract function but got {references=}"
    ...
    @pytest.mark.parametrize("language_server", [Language.DART], indirect=True)
    @pytest.mark.parametrize("repo_path", [Language.DART], indirect=True)
    def test_find_definition_constructor(
        self, language_server: SolidLanguageServer, repo_path: Path
    ) -> None:
    ...
    @pytest.mark.parametrize("language_server", [Language.DART], indirect=True)
    @pytest.mark.parametrize("repo_path", [Language.DART], indirect=True)
    def test_find_definition_import(
        self, language_server: SolidLanguageServer, repo_path: Path
    ) -> None:
        ...
        definition_location_list = language_server.request_definition(
            models_dart_path, 26, 2
        )  # cursor on 'User' in constructor

        # Skip if language server doesn't find definition in this case
        if not definition_location_list:
            pytest.skip(
                "Language server doesn't support definition lookup for this case"
            )
    ...
    def test_find_symbol(self, language_server: SolidLanguageServer) -> None:
        ...
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "Calculator"
        ), "Calculator class not found in symbol tree"
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "add"
        ), "add method not found in symbol tree"
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "subtract"
        ), "subtract function not found in symbol tree"
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "MathHelper"
        ), "MathHelper class not found in symbol tree"
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "User"
        ), "User class not found in symbol tree"
    ...
    def test_find_referencing_symbols(
        ...
        symbol_list = (
            symbols[0] if symbols and isinstance(symbols[0], list) else symbols
        )
        ...
        assert any(
            "main.dart" in ref.get("relativePath", "")
            or "main.dart" in ref.get("uri", "")
            for ref in refs
    ...
    def test_request_containing_symbol_method(
        ...
        containing_symbol = language_server.request_containing_symbol(
            file_path, 13, 10, include_body=True
        )
    ...
    def test_request_defining_symbol_imported_class(
        ...
        if defining_symbol is not None:
            ...
            assert defining_symbol.get("kind") in [
                SymbolKind.Function.value,
                SymbolKind.Method.value,
            ]

    @pytest.mark.parametrize("language_server", [Language.DART], indirect=True)
    def test_request_defining_symbol_class_method(
        self, language_server: SolidLanguageServer
    ) -> None:
    ...
    def test_request_document_symbols(
        ...
        calculator_symbol = next(
            (s for s in symbol_list if s.get("name") == "Calculator"), None
        )
        if (
            calculator_symbol
            and "children" in calculator_symbol
            and calculator_symbol["children"]
        ):
            method_names = [
                child.get("name") for child in calculator_symbol["children"]
            ]
    ...
    @pytest.mark.parametrize("language_server", [Language.DART], indirect=True)
    def test_request_referencing_symbols_comprehensive(
        self, language_server: SolidLanguageServer
    ) -> None:
    ...
    @pytest.mark.parametrize("language_server", [Language.DART], indirect=True)
    def test_cross_file_symbol_resolution(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        helper_symbols = language_server.request_document_symbols(
            helper_file_path
        ).get_all_symbols_and_roots()
        symbol_list = (
            helper_symbols[0]
            if helper_symbols and isinstance(helper_symbols[0], list)
            else helper_symbols
        )

        subtract_symbol = next(
            (s for s in symbol_list if s.get("name") == "subtract"), None
        )

        if subtract_symbol and "selectionRange" in subtract_symbol:
            sel_start = subtract_symbol["selectionRange"]["start"]
            refs = language_server.request_references(
                helper_file_path, sel_start["line"], sel_start["character"]
            )
            ...
            main_dart_refs = [
                ref
                for ref in refs
                if "main.dart" in ref.get("uri", "")
                or "main.dart" in ref.get("relativePath", "")
            ]
    ...
    @pytest.mark.parametrize("language_server", [Language.DART], indirect=True)
    def test_symbol_body_contains_full_method(
        self, language_server: SolidLanguageServer
    ) -> None:
