...
class TestMarkdownLanguageServerBasics:
    ...
    def test_markdown_language_server_initialization(
    ...
    @pytest.mark.parametrize("language_server", [Language.MARKDOWN], indirect=True)
    def test_markdown_request_document_symbols(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        assert (
            "Test Repository" in heading_names or len(all_symbols) > 0
    ...
    @pytest.mark.parametrize("language_server", [Language.MARKDOWN], indirect=True)
    def test_markdown_request_symbols_from_guide(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        assert (
            len(all_symbols) > 0
        ), f"Should find headings in guide.md, found {len(all_symbols)}"

    @pytest.mark.parametrize("language_server", [Language.MARKDOWN], indirect=True)
    def test_markdown_request_symbols_from_api(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        assert (
            len(all_symbols) > 0
        ), f"Should find headings in api.md, found {len(all_symbols)}"

    @pytest.mark.parametrize("language_server", [Language.MARKDOWN], indirect=True)
    def test_markdown_request_document_symbols_with_body(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        assert (
            all_symbols is not None
        ), "Should return symbols even if body extraction is limited"

    @pytest.mark.parametrize("language_server", [Language.MARKDOWN], indirect=True)
    def test_markdown_headings_not_low_level(
        self, language_server: SolidLanguageServer
    ) -> None:
    ...
    @pytest.mark.parametrize("language_server", [Language.MARKDOWN], indirect=True)
    def test_markdown_nested_headings_remapped(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        for symbol in all_symbols:
            assert (
                symbol["kind"] == SymbolKind.Namespace
            ), f"Nested heading '{symbol['name']}' should be remapped to Namespace"
