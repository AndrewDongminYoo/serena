...
class TestHlslSymbols:
    ...
    def test_find_function(self, language_server: SolidLanguageServer) -> None:
        ...
        assert (
            symbol is not None
        ), "Expected 'SafeNormalize' function in document symbols"
    ...
    def test_find_cbuffer_members(self, language_server: SolidLanguageServer) -> None:
        ...
        """
        symbol = _find_symbol_by_name(
            language_server, "compute_test.hlsl", "TextureSize"
        )
        assert (
            symbol is not None
        ), "Expected 'TextureSize' cbuffer member in document symbols"
    ...
    def test_find_compute_kernel(self, language_server: SolidLanguageServer) -> None:
        ...
        assert (
            symbol is not None
        ), "Expected 'CSMain' compute kernel in document symbols"
    ...
    def test_full_symbol_tree(self, language_server: SolidLanguageServer) -> None:
        ...
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "VertexInput"
        ), "VertexInput not in symbol tree"
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "CalculateDiffuse"
        ), "CalculateDiffuse not in symbol tree"
...
class TestHlslDefinition:
    ...
    def test_goto_definition_cross_file(
        ...
        assert any(
            "common.hlsl" in p for p in def_paths
        ), f"Expected definition in common.hlsl, got: {def_paths}"

    @pytest.mark.parametrize("language_server", [Language.HLSL], indirect=True)
    def test_goto_definition_cross_file_remap(
        self, language_server: SolidLanguageServer
    ) -> None:
...
class TestHlslReferences:
    ...
    @pytest.mark.parametrize("language_server", [Language.HLSL], indirect=True)
    def test_references_not_supported(
        self, language_server: SolidLanguageServer
    ) -> None:
...
class TestHlslHover:
    ...
    def test_hover_on_function(self, language_server: SolidLanguageServer) -> None:
        ...
        ), "Hover should return information for SafeNormalize"
