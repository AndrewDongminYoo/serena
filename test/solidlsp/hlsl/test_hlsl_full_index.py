...
class TestHlslFullIndex:
    ...
    def test_all_files_indexed_in_symbol_tree(
        ...
        assert (
            not missing
        ), f"Files missing from full symbol tree: {missing}. Found: {file_names}"

    @pytest.mark.parametrize("language_server", [Language.HLSL], indirect=True)
    def test_subdirectory_file_symbols_present(
        self, language_server: SolidLanguageServer
    ) -> None:
    ...
    @pytest.mark.parametrize("language_server", [Language.HLSL], indirect=True)
    def test_include_file_document_symbols_directly(
        self, language_server: SolidLanguageServer
    ) -> None:
        """request_document_symbols on terrain/terrain_sdf.hlsl should return its symbols."""
        doc_symbols = language_server.request_document_symbols(
            "terrain/terrain_sdf.hlsl"
        )
        ...
        for name in TERRAIN_SDF_UNIQUE_SYMBOLS:
            ...
            ), f"Expected '{name}' in document symbols for terrain/terrain_sdf.hlsl, got: {symbol_names}"
