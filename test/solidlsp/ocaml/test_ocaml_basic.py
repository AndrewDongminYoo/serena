...
@pytest.mark.skipif(
    shutil.which("opam") is None and not is_ci, reason="OPAM is not available"
)
@pytest.mark.ocaml
class TestOCamlLanguageServer:
    ...
    def test_find_symbol(self, language_server: SolidLanguageServer) -> None:
        ...
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "DemoModule"
        ), "DemoModule not found in symbol tree"
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "fib"
        ), "fib not found in symbol tree"
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "someFunction"
        ), "someFunction function not found in symbol tree"
    ...
    def test_find_referencing_symbols(
        ...
        assert (
            len(refs) >= 3
        ), f"Expected at least 3 references to fib (definition + 2 recursive), found {len(refs)}"
        ...
        assert (
            len(lib_refs) >= 3
        ), f"Expected at least 3 references in lib/test_repo.ml, found {len(lib_refs)}"
    ...
    def test_mixed_ocaml_modules(self, language_server: SolidLanguageServer) -> None:
        ...
        assert SymbolUtils.symbol_tree_contains_name(
            all_symbols, "fib"
        ), "Should find fib from .ml file"
        assert SymbolUtils.symbol_tree_contains_name(
            all_symbols, "DemoModule"
        ), "Should find DemoModule from .ml file"
        assert SymbolUtils.symbol_tree_contains_name(
            all_symbols, "someFunction"
        ), "Should find someFunction from DemoModule"
        assert SymbolUtils.symbol_tree_contains_name(
            all_symbols, "num_domains"
        ), "Should find num_domains constant"
    ...
    def test_reason_file_patterns(self) -> None:
        ...
        assert not file_matcher.is_relevant_filename(
            "test.py"
    ...
    @pytest.mark.parametrize("language_server", [Language.OCAML], indirect=True)
    def test_module_hierarchy_navigation(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        assert (
            len(refs) >= 1
        ), f"Expected at least 1 reference to DemoModule, found {len(refs)}"

        # Check that references are found - use forward slashes for URI matching
        lib_refs = [ref for ref in refs if "lib/test_repo.ml" in ref.get("uri", "")]
        assert (
            len(lib_refs) >= 1
        ), f"Expected at least 1 reference in lib/test_repo.ml, found {len(lib_refs)}"
    ...
    def test_let_binding_references(self, language_server: SolidLanguageServer) -> None:
        ...
        refs = language_server.request_references(
            file_path, num_domains_line, num_domains_char
        )

        # Should find at least 1 reference (the definition)
        assert (
            len(refs) >= 1
        ), f"Expected at least 1 reference to num_domains, found {len(refs)}"
        ...
        assert (
            len(ml_refs) >= 1
        ), f"Expected at least 1 reference in lib/test_repo.ml, found {len(ml_refs)}"

    @pytest.mark.parametrize("language_server", [Language.OCAML], indirect=True)
    def test_recursive_function_analysis(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        same_file_refs = [
            ref for ref in refs if "lib/test_repo.ml" in ref.get("uri", "")
        ]
        ...
        ref_lines = [
            ref.get("range", {}).get("start", {}).get("line", -1)
            for ref in same_file_refs
        ]
        unique_lines = len(set(ref_lines))
        assert (
            unique_lines >= 2
        ), f"Recursive calls should appear on multiple lines, found {unique_lines} unique lines"

    @pytest.mark.parametrize("language_server", [Language.OCAML], indirect=True)
    def test_open_statement_resolution(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        demo_module_accessible = SymbolUtils.symbol_tree_contains_name(
            all_symbols, "DemoModule"
        )
        ...
        assert (
            len(symbols) > 0
        ), "Should find symbols in main.ml that use opened modules"
