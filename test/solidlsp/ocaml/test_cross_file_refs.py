...
class TestCrossFileReferences:
    ...
    def test_fib_has_cross_file_references(
        ...
        test_refs = [
            ref for ref in refs if "test/test_test_repo.ml" in ref.get("uri", "")
        ]
        ...
        assert (
            len(lib_refs) >= 3
        ), f"Expected at least 3 references in lib/test_repo.ml (definition + 2 recursive), but got {len(lib_refs)}"

        # Cross-file references require OCaml >= 5.2 and ocaml-lsp-server >= 1.23.0
        if (
            isinstance(language_server, OcamlLanguageServer)
            and language_server.supports_cross_file_references
        ):
            assert len(refs) >= 9, (
