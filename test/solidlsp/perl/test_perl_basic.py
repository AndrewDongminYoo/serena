...
@pytest.mark.skipif(
    ...
    reason="Perl::LanguageServer does not support native Windows operation",
)
class TestPerlLanguageServer:
    ...
    def test_document_symbols(self, language_server: SolidLanguageServer) -> None:
        ...
        function_symbols = [
            s for s in all_symbols if s.get("kind") == 12
        ]  # 12 = Function/Method
        assert (
            len(function_symbols) >= 2
        ), f"Expected at least 2 functions (greet, use_helper_function), found {len(function_symbols)}"

        function_names = [s.get("name") for s in function_symbols]
        assert (
            "greet" in function_names
        ), f"Expected 'greet' function in symbols, found: {function_names}"
        assert (
            "use_helper_function" in function_names
        ), f"Expected 'use_helper_function' in symbols, found: {function_names}"
    ...
    def test_find_definition_across_files(
        ...
        )  # add method on line 2 (0-indexed 1)
    ...
    def test_find_references_across_files(
        ...
        assert (
            len(reference_locations) >= 2
        ), f"Expected at least 2 references to helper_function, found {len(reference_locations)}"

        main_pl_refs = [
            ref for ref in reference_locations if ref["uri"].endswith("main.pl")
        ]
        assert (
            len(main_pl_refs) >= 2
        ), f"Expected at least 2 references in main.pl, found {len(main_pl_refs)}"

        main_pl_lines = sorted([ref["range"]["start"]["line"] for ref in main_pl_refs])
        assert (
            17 in main_pl_lines
        ), f"Expected reference at line 18 (0-indexed 17), found: {main_pl_lines}"
        assert (
            20 in main_pl_lines
        ), f"Expected reference at line 21 (0-indexed 20), found: {main_pl_lines}"
