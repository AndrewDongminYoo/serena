...
class TestHaxeLanguageServer:
    ...
    def test_find_symbol(self, language_server: SolidLanguageServer) -> None:
        ...
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "Main"
        ), "Main class not found in symbol tree"
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "greet"
        ), "greet method not found in symbol tree"
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "calculateResult"
        ), "calculateResult method not found in symbol tree"
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "Helper"
        ), "Helper class not found in symbol tree"
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "addNumbers"
        ), "addNumbers method not found in symbol tree"
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "formatMessage"
        ), "formatMessage method not found in symbol tree"
    ...
    @pytest.mark.parametrize("language_server", [Language.HAXE], indirect=True)
    def test_find_references_within_file(
        ...
        actual_locations = [
            {
                "uri_suffix": os.path.basename(
                    ref.get("relativePath", ref.get("uri", ""))
                ),
        ...
        assert (
            call_site in actual_locations
        ), f"Expected reference to greet at line 15 in Main.hx, got {actual_locations}"

        # All references should be within Main.hx (greet is not used in other files)
        assert all(
            loc["uri_suffix"] == "Main.hx" for loc in actual_locations
    ...
    def test_find_references_across_files(
        ...
        add_numbers_symbol = next(
            (s for s in all_symbols if s.get("name") == "addNumbers"), None
        )
        assert (
            add_numbers_symbol is not None
        ), "Could not find 'addNumbers' symbol in Helper.hx"

        sel_start = add_numbers_symbol["selectionRange"]["start"]
        refs = language_server.request_references(
            helper_path, sel_start["line"], sel_start["character"]
        )
        ...
        assert (
            call_site_1 in actual_locations
        ), f"Expected reference to addNumbers at line 16 in Main.hx, got {actual_locations}"

        # addNumbers is called on line 30 in Main.hx: `var sum = Helper.addNumbers(count, 20);`
        call_site_2 = {"uri_suffix": "Main.hx", "line": 30}
        assert (
            call_site_2 in actual_locations
        ), f"Expected reference to addNumbers at line 30 in Main.hx, got {actual_locations}"
        ...
        assert (
            len(main_refs) >= 2
        ), f"Expected at least 2 references in Main.hx (lines 16 and 30), got {main_refs}"

    @pytest.mark.parametrize("language_server", [Language.HAXE], indirect=True)
    def test_document_symbols_structure(
        self, language_server: SolidLanguageServer
    ) -> None:
        file_path = os.path.join("src", "Main.hx")
        ...
        assert main_symbol.get("kind") in (
            SymbolKind.Class,
            SymbolKind.Struct,
        ), f"Expected Main to be Class, got {main_symbol.get('kind')}"
        ...
        assert (
            "calculateResult" in child_names
        ), "calculateResult method not found in symbols"
    ...
    def test_workspace_symbol(self, language_server: SolidLanguageServer) -> None:
        ...
        assert any(
            "Helper" in str(s.get("name", "")) for s in result
        ), f"Expected at least one result containing 'Helper', got {result}"

    @pytest.mark.parametrize("language_server", [Language.HAXE], indirect=True)
    def test_go_to_definition_within_file(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        assert any(
            "Main.hx" in d.get("uri", d.get("relativePath", "")) for d in definitions
        ), f"Expected definition in Main.hx, got {definitions}"
        # greet is defined on line 23 (0-indexed: 22) in Main.hx
        definition_lines = [
            d["range"]["start"]["line"]
            for d in definitions
            if "Main.hx" in d.get("uri", d.get("relativePath", ""))
        ]
        assert (
            22 in definition_lines
        ), f"Expected definition of greet at line 22 in Main.hx, got lines {definition_lines}"
    ...
    def test_go_to_definition(self, language_server: SolidLanguageServer) -> None:
        ...
        definition_lines = [
            d["range"]["start"]["line"]
            for d in definitions
            if "Helper.hx" in d.get("uri", d.get("relativePath", ""))
        ]
        assert (
            18 in definition_lines
        ), f"Expected definition of addNumbers at line 18 in Helper.hx, got lines {definition_lines}"
    ...
    def test_hover(self, language_server: SolidLanguageServer) -> None:
        ...
        hover_str = str(hover)
        assert (
            "String" in hover_str or "greet" in hover_str
        ), f"Expected hover to contain type info, got {hover_str}"

    @pytest.mark.parametrize("language_server", [Language.HAXE], indirect=True)
    def test_hover_on_class_declaration(
        self, language_server: SolidLanguageServer
    ) -> None:
    ...
    def test_rename_symbol(self, language_server: SolidLanguageServer) -> None:
        ...
        sel_start = greet_symbol["selectionRange"]["start"]
        edits = language_server.request_rename_symbol_edit(
            file_path, sel_start["line"], sel_start["character"], "sayHello"
        )
        ...
        assert (
            "sayHello" in edits_str
        ), f"Expected new name 'sayHello' in rename edits, got {edits}"
    ...
    def test_completions(self, language_server: SolidLanguageServer) -> None:
        ...
        completion_texts = [
            c.get("completionText", c.get("label", "")) for c in completions
        ]
        assert (
            "addNumbers" in completion_texts
        ), f"Expected 'addNumbers' in completions after Helper., got {completion_texts[:10]}"

    @pytest.mark.parametrize("language_server", [Language.HAXE], indirect=True)
    def test_document_overview(self, language_server: SolidLanguageServer) -> None:
        overview = language_server.request_document_overview(
            os.path.join("src", "Main.hx")
        )
        assert overview, "Document overview returned empty list"
        symbol_names = [s.get("name", "") for s in overview]
        assert any(
            "Main" in name for name in symbol_names
        ), f"Expected 'Main' in overview, got {symbol_names}"
        main_entry = next((s for s in overview if s.get("name") == "Main"), None)
        assert (
            main_entry is not None
        ), f"Expected 'Main' entry in overview, got {symbol_names}"
        assert main_entry.get("kind") in (
            SymbolKind.Class,
            SymbolKind.Struct,
