...
class TestLuauLanguageServer:
    ...
    def test_find_symbols_in_init(self, language_server: SolidLanguageServer) -> None:
        ...
        assert (
            "createConfig" in symbol_names
        ), f"createConfig not found in symbols: {symbol_names}"
    ...
    def test_find_symbols_in_module(self, language_server: SolidLanguageServer) -> None:
        ...
        ), f"process not found in symbols: {symbol_names}"
    ...
    def test_find_references_within_file(
        ...
        assert (
            create_config_symbol is not None
        ), "createConfig function not found in init.luau"

        range_info = create_config_symbol.get(
            "selectionRange", create_config_symbol.get("range")
        )
        ...
        refs = language_server.request_references(
            "src/init.luau", range_start["line"], range_start["character"]
        )
        ...
        assert (
            len(refs) >= 2
        ), f"Should find at least 2 references to createConfig, found {len(refs)}"
        ...
        assert (
            "init.luau" in ref_files
        ), f"Expected references in init.luau, found in: {ref_files}"
    ...
    def test_find_references_across_files(
        ...
        range_start = range_info["start"]
        refs = language_server.request_references(
            "src/module.luau", range_start["line"], range_start["character"]
        )
        ...
        assert (
            len(refs) >= 1
        ), f"Should find at least 1 reference to process, found {len(refs)}"
        ...
        assert (
            "module.luau" in ref_info
        ), f"Expected references in module.luau, found in: {set(ref_info.keys())}"
    ...
    def test_find_definition(self, language_server: SolidLanguageServer) -> None:
        ...
        ), f"Expected non-empty definition list but got {definition_locations}"
        assert len(definition_locations) >= 1

        definition = definition_locations[0]
        assert definition["uri"].endswith(
            "init.luau"
        ), f"Definition should be in init.luau, got: {definition['uri']}"
        # createConfig is defined at line 8 (0-indexed): `local function createConfig(...)`
        assert (
            definition["range"]["start"]["line"] == 8
        ), f"Definition should be at line 8, got line {definition['range']['start']['line']}"
