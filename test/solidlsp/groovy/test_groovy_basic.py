...
class TestGroovyLanguageServer:
    ...
    def setup_class(cls):
        ...
        cls.language_server = SolidLanguageServer.create(
            config, repo_path, solidlsp_settings=solidlsp_settings
        )
    ...
    def test_find_symbol(self) -> None:
        ...
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "ModelUser"
        ), "ModelUser class not found in symbol tree"

    def test_find_referencing_class_symbols(self) -> None:
        assert self.language_server is not None
        file_path = os.path.join(
            "src", "main", "groovy", "com", "example", "Utils.groovy"
        )
        refs = self.language_server.request_references(file_path, 3, 6)
        assert any(
            "Main.groovy" in ref.get("relativePath", "") for ref in refs
        ), "Utils should be referenced from Main.groovy"

        file_path = os.path.join(
            "src", "main", "groovy", "com", "example", "Model.groovy"
        )
        symbols = self.language_server.request_document_symbols(
            file_path
        ...
        ), "Could not find 'Model' class symbol in Model.groovy"

        if "selectionRange" in model_symbol:
            sel_start = model_symbol["selectionRange"]["start"]
        else:
            sel_start = model_symbol["range"]["start"]
        refs = self.language_server.request_references(
            file_path, sel_start["line"], sel_start["character"]
        )

        main_refs = [
            ref for ref in refs if "Main.groovy" in ref.get("relativePath", "")
        ]
        assert (
            len(main_refs) >= 2
        ), f"Model should be referenced from Main.groovy at least 2 times, found {len(main_refs)}"

        model_user_refs = [
            ref for ref in refs if "ModelUser.groovy" in ref.get("relativePath", "")
        ]
        assert (
            len(model_user_refs) >= 1
        ), f"Model should be referenced from ModelUser.groovy at least 1 time, found {len(model_user_refs)}"
    ...
    def test_overview_methods(self) -> None:
        ...
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "ModelUser"
        ), "ModelUser missing from overview"
