...
pytestmark = [
    ...
    pytest.mark.skipif(
        ...
        reason=f"Erlang LS not available: {ERLANG_LS_UNAVAILABLE_REASON}",
    ),
]
...
@pytest.mark.xfail(
    reason="Known timeout issue on Ubuntu CI with Erlang LS server startup",
    strict=False,
)
...
def test_symbol_tree_ignores_dir(ls_with_ignored_dirs: SolidLanguageServer):
    ...
    ), f"Expected some dirs from {expected_dirs} to be in {children_names}"
    assert "_build" not in children_names, f"_build should not be in {children_names}"
    assert (
        "ignored_dir" not in children_names
    ), f"ignored_dir should not be in {children_names}"
...
def test_find_references_ignores_dir(ls_with_ignored_dirs: SolidLanguageServer):
    ...
    symbols = ls_with_ignored_dirs.request_document_symbols(
        definition_file
    ).get_all_symbols_and_roots()
    user_symbol = None
    for symbol_group in symbols:
        user_symbol = next(
            (s for s in symbol_group if "user" in s.get("name", "").lower()), None
        )
        if user_symbol:
            break
    ...
    references = ls_with_ignored_dirs.request_references(
        definition_file, sel_start["line"], sel_start["character"]
    )

    # Assert that _build and ignored_dir do not appear in the references
    assert not any(
        "_build" in ref["relativePath"] for ref in references
    ), "_build should be ignored"
    assert not any(
        "ignored_dir" in ref["relativePath"] for ref in references
...
def test_refs_and_symbols_with_glob_patterns(repo_path: Path) -> None:
    ...
    with start_ls_context(
        language=Language.ERLANG, repo_path=str(repo_path), ignored_paths=ignored_paths
    ) as ls:
        ...
        assert (
            "_build" not in children_names
        ), f"_build should not be in {children_names} (glob pattern)"
        assert (
            "ignored_dir" not in children_names
        ), f"ignored_dir should not be in {children_names} (glob pattern)"

        # Test that the refs and symbols with glob patterns are ignored
        definition_file = "include/records.hrl"

        # Find the user record definition
        symbols = ls.request_document_symbols(
            definition_file
        ).get_all_symbols_and_roots()
        user_symbol = None
        for symbol_group in symbols:
        ...
        if user_symbol and "selectionRange" in user_symbol:
            ...
            assert not any(
                "_build" in ref["relativePath"] for ref in references
            ), "_build should be ignored (glob)"
            assert not any(
                "ignored_dir" in ref["relativePath"] for ref in references
            ), "ignored_dir should be ignored (glob)"
...
def test_default_ignored_directories(language_server: SolidLanguageServer):
    ...
    assert language_server.is_ignored_dirname(
        "_checkouts"
    ), "_checkouts should be ignored"
    assert language_server.is_ignored_dirname(
        "node_modules"
    ...
    assert not language_server.is_ignored_dirname(
        "include"
    ), "include should not be ignored"
...
def test_symbol_tree_excludes_build_dirs(language_server: SolidLanguageServer):
    ...
    if symbol_tree:
        ...
        assert (
            len(found_ignored) == 0
        ), f"Found ignored directories in symbol tree: {found_ignored}"
        ...
        assert (
            len(found_important) > 0
        ), f"Expected to find important directories: {important_dirs}, got: {children_names}"
...
def test_ignore_compiled_files(language_server: SolidLanguageServer):
    ...
    assert language_server.is_ignored_filename(
        "module.beam"
    ), "BEAM files should be ignored"
    assert language_server.is_ignored_filename(
        "app.beam"
    ), "BEAM files should be ignored"

    # Test that source files are not ignored
    assert not language_server.is_ignored_filename(
        "module.erl"
    ), "Erlang source files should not be ignored"
    assert not language_server.is_ignored_filename(
        "records.hrl"
    ), "Header files should not be ignored"
...
def test_rebar_directories_ignored(language_server: SolidLanguageServer):
    ...
    assert language_server.is_ignored_dirname(
        "_build"
    ), "rebar3 _build should be ignored"
    assert language_server.is_ignored_dirname(
        "_checkouts"
    ), "rebar3 _checkouts should be ignored"
    assert language_server.is_ignored_dirname(
        ".rebar3"
    ), "rebar3 cache should be ignored"

    # Test that rebar.lock and rebar.config are not ignored (they are configuration files)
    assert not language_server.is_ignored_filename(
        "rebar.config"
    ), "rebar.config should not be ignored"
    assert not language_server.is_ignored_filename(
        "rebar.lock"
    ), "rebar.lock should not be ignored"
...
def test_erlang_specific_ignore_patterns(language_server: SolidLanguageServer):
    ...
    for dirname in erlang_important_dirs:
        assert not language_server.is_ignored_dirname(
            dirname
        ), f"{dirname} should not be ignored"
