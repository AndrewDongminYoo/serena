...
class TestErlangLanguageServerSymbols:
    ...
    def test_request_containing_symbol_module(
        ...
        containing_symbol = language_server.request_containing_symbol(
            file_path, module_line, 5
        )

        # Verify that we found the containing symbol
        if containing_symbol:
            assert (
                "models" in containing_symbol["name"]
                or "module" in containing_symbol["name"].lower()
            )
            assert (
    ...
    def test_request_containing_symbol_none(
        ...
        assert (
            ...
            or "models" in str(containing_symbol)
        )

    @pytest.mark.parametrize("language_server", [Language.ERLANG], indirect=True)
    def test_request_referencing_symbols_record(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        if ref_symbols:
            models_references = [
                ...
                and "models.erl" in symbol["location"]["uri"]
            ]
    ...
    def test_request_referencing_symbols_function(
        ...
        for symbol_group in symbols:
            create_user_symbol = next(
                (s for s in symbol_group if "create_user" in s.get("name", "")), None
            )
        ...
        if ref_symbols:
            ...
            service_references = [
                ...
                and "uri" in symbol["location"]
                and (
                    "services.erl" in symbol["location"]["uri"]
                    or "test" in symbol["location"]["uri"]
                )
            ]
    ...
    def test_request_defining_symbol_function_call(
        ...
        defining_symbol = language_server.request_defining_symbol(
            file_path, models_call_line, 20
        )

        if defining_symbol:
            assert "create_user" in defining_symbol.get(
                "name", ""
            ) or "models" in defining_symbol.get("name", "")
            if "location" in defining_symbol and "uri" in defining_symbol["location"]:
                assert "models.erl" in defining_symbol["location"]["uri"]

    @pytest.mark.parametrize("language_server", [Language.ERLANG], indirect=True)
    def test_request_defining_symbol_record_usage(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        defining_symbol = language_server.request_defining_symbol(
            file_path, record_usage_line, 10
        )
    ...
    @pytest.mark.parametrize("language_server", [Language.ERLANG], indirect=True)
    def test_request_defining_symbol_module_call(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        defining_symbol = language_server.request_defining_symbol(
            file_path, utils_call_line, 15
        )

        if defining_symbol:
            assert "validate" in defining_symbol.get(
                "name", ""
            ) or "email" in defining_symbol.get("name", "")
    ...
    def test_symbol_methods_integration(
        ...
        containing = language_server.request_containing_symbol(
            file_path, create_user_line + 2, 10
        )
    ...
    def test_symbol_tree_structure(self, language_server: SolidLanguageServer) -> None:
        ...
        if src_dir:
            ...
            found_modules = [
                ...
                if any(name in fname for fname in file_names)
            ]
            assert (
                len(found_modules) > 0
    ...
    def test_request_dir_overview(self, language_server: SolidLanguageServer) -> None:
        ...
        overview_keys = (
            list(src_overview.keys()) if hasattr(src_overview, "keys") else []
        )
        src_files = [
            key for key in overview_keys if key.startswith("src/") or "src" in key
        ]
        assert (
            len(src_files) > 0
        ), f"Expected to find src/ files in overview keys: {overview_keys}"
    ...
    @pytest.mark.parametrize("language_server", [Language.ERLANG], indirect=True)
    def test_containing_symbol_of_record_field(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        containing_symbol = language_server.request_containing_symbol(
            file_path, record_field_line, 10
        )
    ...
    @pytest.mark.parametrize("language_server", [Language.ERLANG], indirect=True)
    def test_containing_symbol_of_spec(
        self, language_server: SolidLanguageServer
    ) -> None:
    ...
    @pytest.mark.parametrize("language_server", [Language.ERLANG], indirect=True)
    def test_referencing_symbols_across_files(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        cross_file_refs = [
            ...
            if "location" in symbol
            and "uri" in symbol["location"]
            and not symbol["location"]["uri"].endswith("models.erl")
        ]
