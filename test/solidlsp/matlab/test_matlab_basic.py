...
class TestMatlabLanguageServerBasics:
    ...
    def test_matlab_language_server_initialization(
    ...
    @pytest.mark.parametrize("language_server", [Language.MATLAB], indirect=True)
    def test_matlab_request_document_symbols_class(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        method_symbols = [
            symbol for symbol in all_symbols if symbol.get("kind") in [6, 12]
        ]
        ...
        for method in expected_methods:
            ...
            ), f"Should find {method} method in Calculator class"

    @pytest.mark.parametrize("language_server", [Language.MATLAB], indirect=True)
    def test_matlab_request_document_symbols_function(
        self, language_server: SolidLanguageServer
    ) -> None:
    ...
    @pytest.mark.parametrize("language_server", [Language.MATLAB], indirect=True)
    def test_matlab_request_document_symbols_script(
        self, language_server: SolidLanguageServer
    ) -> None:
...
class TestMatlabLanguageServerReferences:
    ...
    @pytest.mark.parametrize("language_server", [Language.MATLAB], indirect=True)
    def test_matlab_find_references_within_file(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        references = language_server.request_references(
            "Calculator.m", 25, 12
        )  # 'result' in add method

        # Should find at least the definition
        assert references is not None

    @pytest.mark.parametrize("language_server", [Language.MATLAB], indirect=True)
    def test_matlab_find_references_cross_file(
        self, language_server: SolidLanguageServer
    ) -> None:
