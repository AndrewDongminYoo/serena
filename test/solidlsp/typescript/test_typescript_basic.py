...
class TestTypescriptLanguageServer:
    ...
    def test_find_symbol(self, language_server: SolidLanguageServer) -> None:
        ...
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "DemoClass"
        ), "DemoClass not found in symbol tree"
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "helperFunction"
        ), "helperFunction not found in symbol tree"
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "printValue"
        ), "printValue method not found in symbol tree"
    ...
    def test_find_referencing_symbols(
        ...
        ), "Could not find 'helperFunction' symbol in index.ts"
    ...
    if language_has_verified_implementation_support(Language.TYPESCRIPT):
        ...
        @pytest.mark.parametrize(
            "language_server", [Language.TYPESCRIPT], indirect=True
        )
        def test_find_implementations(
            ...
            pos = find_identifier_position(
                repo_path / "formatters.ts", "formatGreeting"
            )
            ...
            assert any(
                "formatters.ts" in implementation.get("relativePath", "")
    ...
    def test_tsx_symbol_range_not_truncated_by_jsx(
        ...
        jsx_component = next(
            (s for s in roots if s.get("name") == "JsxComponent"), None
        )
        assert (
            jsx_component is not None
        ), "JsxComponent not found at root level of jsx_component.tsx"
        ...
        assert any(
            s.get("name") == "trailingHelper" for s in roots
