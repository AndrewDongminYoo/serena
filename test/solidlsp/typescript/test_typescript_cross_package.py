...
CROSS_PKG_DIR = (
    Path(__file__).parent.parent.parent / "resources" / "repos" / "typescript"
)
...
class TestCrossPackageReferences:
        ...
        with start_ls_context(
            ...
            for sym in symbols[0]:
                if sym.get("name") == "sharedUtilityFunction":
                    ...
                    break
            assert (
                shared_fn is not None
            ), "Could not find 'sharedUtilityFunction' in shared_utils.ts"

            sel_start = shared_fn["selectionRange"]["start"]
            refs = ls.request_references(
                "shared_utils.ts", sel_start["line"], sel_start["character"]
            )

            ref_paths = [r.get("relativePath", "") for r in refs]
            cross_package_refs = [
                p for p in ref_paths if "cross_package_b" in p or "consumer.ts" in p
            ]
            assert (
                len(cross_package_refs) > 0
    ...
    def test_cross_package_referencing_symbols(self) -> None:
        ...
        with start_ls_context(
            ...
            ), "Could not find 'SharedClass' in shared_utils.ts"
            ...
            cross_refs = [
                p for p in ref_files if "cross_package_b" in p or "consumer.ts" in p
            ]
            assert (
                len(cross_refs) > 0
            ), f"Expected cross-package referencing symbol from package_b, but only found refs in: {ref_files}"
