...
class TestTomlSymbolRetrieval:
    ...
    def test_request_containing_symbol_behavior(
        ...
        ), "TOML LSP doesn't support containing symbol lookup"

    @pytest.mark.parametrize("language_server", [Language.TOML], indirect=True)
    @pytest.mark.parametrize("repo_path", [Language.TOML], indirect=True)
    def test_request_document_overview_cargo(
        self, language_server: SolidLanguageServer, repo_path: Path
    ) -> None:
        ...
        assert expected_tables.issubset(
            symbol_names
        ), f"Missing expected tables in overview: {expected_tables - symbol_names}"

    @pytest.mark.parametrize("language_server", [Language.TOML], indirect=True)
    @pytest.mark.parametrize("repo_path", [Language.TOML], indirect=True)
    def test_request_document_overview_pyproject(
        self, language_server: SolidLanguageServer, repo_path: Path
    ) -> None:
    ...
    @pytest.mark.parametrize("language_server", [Language.TOML], indirect=True)
    @pytest.mark.parametrize("repo_path", [Language.TOML], indirect=True)
    def test_request_full_symbol_tree(
        self, language_server: SolidLanguageServer, repo_path: Path
    ) -> None:
        ...
        assert (
            "Cargo" in child_names
            or "Cargo.toml" in child_names
            or any("cargo" in name.lower() for name in child_names)
        ), f"Should find Cargo.toml in tree, got: {child_names}"

    @pytest.mark.parametrize("language_server", [Language.TOML], indirect=True)
    @pytest.mark.parametrize("repo_path", [Language.TOML], indirect=True)
    def test_request_dir_overview(
        self, language_server: SolidLanguageServer, repo_path: Path
    ) -> None:
        ...
        assert any(
            "Cargo.toml" in path for path in file_paths
        ), f"Should find Cargo.toml in overview, got: {file_paths}"
        assert any(
            "pyproject.toml" in path for path in file_paths
        ), f"Should find pyproject.toml in overview, got: {file_paths}"

    @pytest.mark.parametrize("language_server", [Language.TOML], indirect=True)
    @pytest.mark.parametrize("repo_path", [Language.TOML], indirect=True)
    def test_symbol_hierarchy_in_cargo(
        self, language_server: SolidLanguageServer, repo_path: Path
    ) -> None:
        ...
        package_symbol = next(
            (s for s in root_symbols if s.get("name") == "package"), None
        )
        ...
        child_names = {
            child.get("name") for child in package_symbol.get("children", [])
        }
    ...
    @pytest.mark.parametrize("language_server", [Language.TOML], indirect=True)
    @pytest.mark.parametrize("repo_path", [Language.TOML], indirect=True)
    def test_symbol_hierarchy_in_pyproject(
        self, language_server: SolidLanguageServer, repo_path: Path
    ) -> None:
        ...
        project_symbol = next(
            (s for s in root_symbols if s.get("name") == "project"), None
        )
        ...
        child_names = {
            child.get("name") for child in project_symbol.get("children", [])
        }
    ...
    @pytest.mark.parametrize("language_server", [Language.TOML], indirect=True)
    @pytest.mark.parametrize("repo_path", [Language.TOML], indirect=True)
    def test_tool_section_hierarchy(
        self, language_server: SolidLanguageServer, repo_path: Path
    ) -> None:
        ...
        assert (
            has_ruff or has_mypy or has_pytest
        ), f"Should detect tool sections, got names: {all_names}"

    @pytest.mark.parametrize("language_server", [Language.TOML], indirect=True)
    @pytest.mark.parametrize("repo_path", [Language.TOML], indirect=True)
    def test_array_of_tables_symbol(
        self, language_server: SolidLanguageServer, repo_path: Path
    ) -> None:
        ...
        if "children" in bin_symbol:
            ...
            assert (
                len(bin_children) > 0
            ), "[[bin]] should have at least one child element"
            # First child is index '0'
            first_child = bin_children[0]
            assert (
                first_child.get("name") == "0"
            ), f"First array element should be named '0', got: {first_child.get('name')}"

            # The '0' element should contain name and path as grandchildren
            if "children" in first_child:
                grandchild_names = {
                    gc.get("name") for gc in first_child.get("children", [])
                }
                assert (
                    "name" in grandchild_names
                ), f"[[bin]] element should have 'name' field, got: {grandchild_names}"
                assert (
                    "path" in grandchild_names
                ), f"[[bin]] element should have 'path' field, got: {grandchild_names}"
