...
class TestTomlIgnoredDirectories:
    ...
    def test_important_directories_not_ignored(
        self, language_server: SolidLanguageServer
    ) -> None:
    ...
    def test_cargo_related_directories(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        assert language_server.is_ignored_dirname(
            "target"
        ), "target (Rust build) should be ignored"
        assert language_server.is_ignored_dirname(".cargo"), ".cargo should be ignored"

        # But important Rust directories should not be ignored
        assert not language_server.is_ignored_dirname(
            "benches"
        ), "benches should not be ignored"
        assert not language_server.is_ignored_dirname(
            "examples"
        ), "examples should not be ignored"

    def test_various_cache_directories(
        self, language_server: SolidLanguageServer
    ) -> None:
