...
def _find_symbol_by_name(
    language_server: SolidLanguageServer, file_path: str, name: str
) -> dict[str, Any] | None:
...
def _get_symbol_selection_start(
    language_server: SolidLanguageServer, file_path: str, name: str
) -> tuple[int, int]:
    ...
    ), f"Symbol '{name}' has no selectionRange in {file_path}"
...
class TestSystemVerilogSymbols:
    ...
    def test_find_symbol(self, language_server: SolidLanguageServer) -> None:
        ...
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "counter"
        ), "Module 'counter' not found in symbol tree"
    ...
    def test_find_top_module(self, language_server: SolidLanguageServer) -> None:
        ...
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "top"
        ), "Module 'top' not found in symbol tree"
...
class TestSystemVerilogDefinition:
    ...
    def test_goto_definition(self, language_server: SolidLanguageServer) -> None:
        ...
        assert (
            len(definitions) >= 1
        ), f"Expected at least 1 definition, got {len(definitions)}"
        def_in_counter = [
            d for d in definitions if "counter.sv" in (d.get("relativePath") or "")
        ]
        assert (
            len(def_in_counter) >= 1
        ), f"Expected definition in counter.sv, got: {[d.get('relativePath') for d in definitions]}"
        assert (
    ...
    def test_goto_definition_cross_file(
        ...
        assert any(
            "counter.sv" in p for p in def_paths
        ), f"Expected definition in counter.sv, got: {def_paths}"
        counter_defs = [
            d for d in definitions if "counter.sv" in (d.get("relativePath") or "")
        ]
        assert (
...
class TestSystemVerilogReferences:
    ...
    def test_find_references(self, language_server: SolidLanguageServer) -> None:
        ...
        assert (
            len(references) >= 1
        ), f"Expected at least 1 reference, got {len(references)}"
        ref_paths = [r.get("relativePath", "") for r in references]
        refs_in_counter = [
            r for r in references if "counter.sv" in (r.get("relativePath") or "")
        ]
        assert (
            len(refs_in_counter) >= 1
        ), f"Expected within-file references in counter.sv, got paths: {ref_paths}"
        ref_lines = sorted(r["range"]["start"]["line"] for r in refs_in_counter)
        # Line 13: count <= '0;  Line 15: count <= count + 1'b1; (two refs)
        assert (
            13 in ref_lines
        ), f"Expected reference at line 13 (count <= '0), got lines: {ref_lines}"
        assert (
            15 in ref_lines
        ), f"Expected reference at line 15 (count <= count + 1'b1), got lines: {ref_lines}"
    ...
    def test_find_references_cross_file(
        ...
        """
        line, char = _get_symbol_selection_start(
            language_server, "counter.sv", "counter"
        )
        references = language_server.request_references("counter.sv", line, char)
        ref_paths = [ref.get("relativePath", "") for ref in references]
        assert any(
            "top.sv" in p for p in ref_paths
        ), f"Expected reference from top.sv, got: {ref_paths}"
        refs_in_top = [
            r for r in references if "top.sv" in (r.get("relativePath") or "")
        ]
        # top.sv line 17 (0-indexed: 16): "    counter #(.WIDTH(8)) u_counter ("
        assert (
...
class TestSystemVerilogHover:
    ...
    def test_hover(self, language_server: SolidLanguageServer) -> None:
        ...
        assert (
            hover_info is not None
        ), "Hover should return information for counter module"
        ...
        assert (
            "counter" in hover_text.lower()
        ), f"Hover should mention 'counter', got: {hover_text}"
        assert (
            "module" in hover_text.lower()
        ), f"Hover should identify 'counter' as a module, got: {hover_text}"

    @pytest.mark.parametrize("language_server", [Language.SYSTEMVERILOG], indirect=True)
    def test_hover_includes_type_information(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        assert (
            hover_info is not None
        ), "Hover should return information for 'count' port"
        assert "contents" in hover_info, "Hover should have contents"
        hover_text = _extract_hover_text(hover_info)
        assert (
            "count" in hover_text.lower()
        ), f"Hover should mention 'count', got: {hover_text}"
        assert (
            "logic" in hover_text.lower()
        ), f"Hover should include type 'logic', got: {hover_text}"
...
class TestSystemVerilogRename:
    ...
    @pytest.mark.parametrize("language_server", [Language.SYSTEMVERILOG], indirect=True)
    def test_rename_signal_within_file(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        ), "Rename should be supported for port signal 'count'"

        changes = _extract_changes(workspace_edit)
        counter_edits = [edits for uri, edits in changes.items() if "counter.sv" in uri]
        assert (
            len(counter_edits) >= 1
        ), f"Should have edits for counter.sv, got: {list(changes.keys())}"

        edits = counter_edits[0]
        assert (
            len(edits) >= 2
        ), f"Expected at least 2 edits (declaration + usage), got {len(edits)}"
        edit_lines = sorted(e["range"]["start"]["line"] for e in edits)
        assert (
            7 in edit_lines
        ), f"Expected edit at line 7 (port declaration), got lines: {edit_lines}"
        assert (
            13 in edit_lines
        ), f"Expected edit at line 13 (count <= '0), got lines: {edit_lines}"
        assert (
            15 in edit_lines
        ), f"Expected edit at line 15 (count <= count + 1'b1), got lines: {edit_lines}"
        for edit in edits:
            assert (
                edit["newText"] == "cnt"
            ), f"Expected newText 'cnt', got {edit['newText']}"

    @pytest.mark.parametrize("language_server", [Language.SYSTEMVERILOG], indirect=True)
    def test_rename_signal_cross_file(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        """
        workspace_edit = language_server.request_rename_symbol_edit(
            "counter.sv", 13, 12, "cnt"
        )
        assert (
            workspace_edit is not None
        ), "Rename should be supported for signal 'count' from usage site"
        ...
        assert (
            len(counter_uris) >= 1
        ), f"Expected edits in counter.sv, got: {list(changes.keys())}"
        assert (
            len(top_uris) >= 1
        ), f"Expected cross-file edits in top.sv, got: {list(changes.keys())}"

        for uri, edits in changes.items():
            for edit in edits:
                assert (
                    edit["newText"] == "cnt"
                ), f"Expected 'cnt' in {uri}, got {edit['newText']}"
    ...
    def test_rename_module_name(self, language_server: SolidLanguageServer) -> None:
        ...
        workspace_edit = language_server.request_rename_symbol_edit(
            "counter.sv", line, char, "my_counter"
        )
        assert (
            workspace_edit is not None
        ), "Rename should be supported for module 'counter'"
        ...
        assert (
            1 in edit_lines
        ), f"Expected edit at line 1 (module declaration), got lines: {edit_lines}"
        ...
        for uri, file_edits in changes.items():
            for edit in file_edits:
                assert (
                    edit["newText"] == "my_counter"
                ), f"Expected 'my_counter', got {edit['newText']}"

    @pytest.mark.skipif(
        shutil.which("verible-verilog-ls") is None,
        reason="verible-verilog-ls is not available",
    )
    @pytest.mark.parametrize("language_server", [Language.SYSTEMVERILOG], indirect=True)
