...
class TestVeribleVerilogLsDetection:
    ...
    def test_detect_from_path_returns_system_verible(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            provider = SystemVerilogLanguageServer.DependencyProvider(
                custom_settings, temp_dir
            )
    ...
    def test_detect_from_path_logs_version(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch("shutil.which", return_value="/usr/bin/verible-verilog-ls"):
                with patch("subprocess.run") as mock_run:
                    mock_run.return_value = MagicMock(
                        returncode=0, stdout="Verible v0.0-4051-g9fdb4057", stderr=""
                    )
                    with patch(
                        "solidlsp.language_servers.systemverilog_server.log"
                    ) as mock_log:
                        result = provider._get_or_install_core_dependency()

            # Verify version check was called
            assert mock_run.call_args[0][0] == [
                "/usr/bin/verible-verilog-ls",
                "--version",
            ]
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch("shutil.which", return_value="/custom/bin/verible-verilog-ls"):
                with patch("subprocess.run") as mock_run:
                    # Version check fails
                    mock_run.return_value = MagicMock(
                        returncode=1, stdout="", stderr="Unknown option: --version"
                    )
                    result = provider._get_or_install_core_dependency()
    ...
    def test_detect_from_path_handles_version_timeout_gracefully(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch(
                "shutil.which", return_value="/opt/verible/bin/verible-verilog-ls"
            ):
                with patch("subprocess.run") as mock_run:
                    # Version check times out
                    mock_run.side_effect = subprocess.TimeoutExpired(
                        cmd=["verible-verilog-ls", "--version"], timeout=5
                    )
                    result = provider._get_or_install_core_dependency()
    ...
    def test_error_message_when_not_found_anywhere(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch("shutil.which", return_value=None):
                ...
                with patch(
                    "solidlsp.language_servers.systemverilog_server.RuntimeDependencyCollection"
                ) as mock_deps_class:
                    mock_deps = Mock()
                    mock_deps.get_single_dep_for_current_platform.side_effect = (
    ...
    def test_downloads_when_not_in_path(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            expected_path = os.path.join(
                temp_dir,
                "verible-ls",
                f"verible-{DEFAULT_VERIBLE_VERSION}",
                "bin",
                "verible-verilog-ls",
            )
            ...
            with patch("shutil.which", return_value=None):
                with patch(
                    ...
                    mock_deps = Mock()
                    mock_deps.get_single_dep_for_current_platform.return_value = (
                        mock_dep
                    )
    ...
    def test_detection_prefers_path_over_download(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch("shutil.which", return_value="/usr/bin/verible-verilog-ls"):
                with patch("subprocess.run") as mock_run:
                    mock_run.return_value = MagicMock(
                        returncode=0, stdout="Verible v0.0-4051", stderr=""
                    )
