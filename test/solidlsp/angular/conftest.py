...
def _install_angular_test_repo_node_modules() -> None:
    ...
    if ANGULAR_CORE_MARKER.exists():
        log.info(
            "Angular test repo node_modules already populated; skipping npm install"
        )
        return

    if shutil.which("npm") is None:
        pytest.skip(
            "npm is not available; cannot install Angular test repo dependencies"
        )
    ...
    with FileLock(str(INSTALL_LOCK)):
        ...
        if ANGULAR_CORE_MARKER.exists():
            log.info(
                "Angular test repo node_modules populated by another worker; skipping npm install"
            )
            return
        ...
        if proc.returncode != 0:
            log.error(
                "npm install failed (rc=%s).\nstdout:\n%s\nstderr:\n%s",
                proc.returncode,
                proc.stdout,
                proc.stderr,
            )
            pytest.skip(
                f"npm install failed in {REPO_ROOT} (rc={proc.returncode}); see logs for details"
            )

        if not ANGULAR_CORE_MARKER.exists():
            pytest.skip(
                f"npm install completed but {ANGULAR_CORE_MARKER} is missing; cannot run Angular tests"
            )
