...
class TestAngularLanguageServerBasics:
    ...
    @pytest.mark.parametrize("language_server", [Language.ANGULAR], indirect=True)
    def test_component_class_symbols(
        ...
        for expected in (
            ...
            assert (
                expected in names
            ), f"Expected '{expected}' in component symbols: {names}"
    ...
    def test_service_class_symbols(self, language_server: SolidLanguageServer) -> None:
        ...
        for expected in ("GreetingService", "greet", "defaultName"):
            assert (
                expected in names
            ), f"Expected '{expected}' in service symbols: {names}"
    ...
    def test_full_symbol_tree_includes_all_files(
        ...
        for f in (
            ...
            assert (
                f in relative_paths
            ), f"Expected {f} to appear in symbol tree, got {relative_paths}"

    @pytest.mark.parametrize("language_server", [Language.ANGULAR], indirect=True)
    def test_template_definition_to_component_method(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        assert (
            definitions
        ), f"Expected non-empty cross-file definition for template->component method, got {definitions}"
        target_uris = [d["uri"] for d in definitions]
        assert any(
            uri.endswith("app.component.ts") for uri in target_uris
        ), f"Expected definition to resolve into app.component.ts, got URIs: {target_uris}"

    @pytest.mark.parametrize("language_server", [Language.ANGULAR], indirect=True)
    def test_plain_html_document_symbols(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        for expected in ("html", "head", "body", "app-root"):
            assert (
                expected in names
            ), f"Expected '{expected}' in plain-HTML symbol list, got: {names}"

    @pytest.mark.parametrize("language_server", [Language.ANGULAR], indirect=True)
    def test_template_html_document_symbols(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        for expected in ("section", "h1", "p", "input", "ul", "app-item-card"):
            assert (
                expected in names
            ), f"Expected '{expected}' in template HTML symbol list, got: {names}"

    @pytest.mark.parametrize("language_server", [Language.ANGULAR], indirect=True)
    def test_ts_method_references_include_template_usage(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        coords = find_text_coordinates(
            read_repo_file(language_server, src_path), r"(setName)\(name:"
        )
        assert (
            coords is not None
        ), "Could not locate setName declaration in app.component.ts"
        # +1 puts the cursor inside the identifier rather than on its leading boundary.
        refs = language_server.request_references(src_path, coords.line, coords.col + 1)
        ref_paths = {r.get("relativePath", "") for r in refs}
        assert any(
            p.endswith("app.component.html") for p in ref_paths
...
class TestAngularHover:
    ...
    @pytest.mark.parametrize("language_server", [Language.ANGULAR], indirect=True)
    def test_hover_on_template_method_call(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        assert (
            hover is not None
        ), f"Expected hover info for greeting() in {path}, got None"
        contents = hover.get("contents")
        assert contents, f"Expected non-empty hover contents, got: {hover}"
...
class TestAngularDefinitionRouting:
    ...
    @pytest.mark.parametrize("language_server", [Language.ANGULAR], indirect=True)
    def test_definition_from_property_binding(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        assert (
            definitions
        ), f"Expected non-empty definition for userName binding, got {definitions}"
        target_uris = [d["uri"] for d in definitions]
        assert any(
            uri.endswith("app.component.ts") for uri in target_uris
        ), f"Expected definition to resolve into app.component.ts, got URIs: {target_uris}"
    ...
    @pytest.mark.parametrize("language_server", [Language.ANGULAR], indirect=True)
    def test_definition_from_event_binding(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        assert (
            definitions
        ), f"Expected non-empty definition for setName binding, got {definitions}"
        target_uris = [d["uri"] for d in definitions]
        assert any(
            uri.endswith("app.component.ts") for uri in target_uris
        ), f"Expected definition to resolve into app.component.ts, got URIs: {target_uris}"
    ...
    @pytest.mark.parametrize("language_server", [Language.ANGULAR], indirect=True)
    def test_definition_service_import_in_component(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        )  # cursor on 'G' of GreetingService
        definitions = language_server.request_definition(path, line, col)
        assert (
            definitions
        ), f"Expected definition for GreetingService, got {definitions}"
        target_uris = [d["uri"] for d in definitions]
        assert any(
            uri.endswith("greeting.service.ts") for uri in target_uris
        ), f"Expected definition to resolve into greeting.service.ts, got URIs: {target_uris}"

    @pytest.mark.parametrize("language_server", [Language.ANGULAR], indirect=True)
    def test_definition_from_child_component_selector(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        assert (
            definitions
        ), f"Expected definition for <app-item-card>, got {definitions}"
        target_uris = [d["uri"] for d in definitions]
        assert any(
            uri.endswith("item-card.component.ts") for uri in target_uris
...
class TestAngularRename:
    ...
    @pytest.mark.parametrize("language_server", [Language.ANGULAR], indirect=True)
    def test_rename_method_returns_edits_for_ts_and_template(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        coords = find_text_coordinates(
            read_repo_file(language_server, path), r"(setName)\(name:"
        )
        assert (
            coords is not None
        ), "Could not locate setName declaration in app.component.ts"
        ...
        edit = language_server.request_rename_symbol_edit(
            path, coords.line, coords.col + 1, "updateName"
        )
        ...
        for dc in document_changes:
            ...
            if uri:
                all_uris.add(uri)
        assert any(
            uri.endswith("app.component.ts") for uri in all_uris
        ), f"Expected rename edits in app.component.ts, got URIs: {all_uris}"
        assert any(
            uri.endswith("app.component.html") for uri in all_uris
...
class TestAngularSymbolStructure:
    ...
    @pytest.mark.parametrize("language_server", [Language.ANGULAR], indirect=True)
    def test_component_class_has_methods_and_fields_as_children(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        component = next(
            (s for s in all_symbols if s.get("name") == "AppComponent"), None
        )
        ...
        for expected in ("greeting", "setName", "title", "userName", "items"):
            assert (
                expected in child_names
            ), f"Expected '{expected}' as child of AppComponent, got: {child_names}"

    @pytest.mark.parametrize("language_server", [Language.ANGULAR], indirect=True)
    def test_pipe_class_in_symbol_tree(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        for expected in (
            ...
            assert (
                expected in names
            ), f"Expected '{expected}' in full symbol tree, got: {sorted(names)}"
...
class TestAngularImplementations:
    ...
    @pytest.mark.parametrize("language_server", [Language.ANGULAR], indirect=True)
    def test_find_implementations_of_interface_method(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        assert (
            implementations
        ), f"Expected at least one implementation of Greeter.greet, got {implementations}"
