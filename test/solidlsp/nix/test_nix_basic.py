...
pytestmark = pytest.mark.skipif(
    platform.system() == "Windows", reason="Nix and nil are not available on Windows"
)
...
class TestNixLanguageServer:
    ...
    def test_find_symbols_in_default_nix(
        ...
        ), f"Expected exactly {expected_attrs}, found {found_attrs}"
    ...
    def test_find_symbols_in_utils(self, language_server: SolidLanguageServer) -> None:
        ...
        ), f"Expected exactly {expected_modules}, found {found_modules}"
    ...
    def test_find_symbols_in_flake(self, language_server: SolidLanguageServer) -> None:
        ...
        assert (
            "inputs" in symbol_names or "outputs" in symbol_names
        ), "Flake must have inputs or outputs"
    ...
    def test_find_symbols_in_module(self, language_server: SolidLanguageServer) -> None:
        ...
        assert (
            "options" in symbol_names or "config" in symbol_names
        ), "Module must have options or config"
    ...
    def test_find_references_within_file(
        ...
        refs = language_server.request_references(
            "default.nix", range_start["line"], range_start["character"]
        )
        ...
        assert (
            len(refs) >= 1
        ), f"Should find at least 1 reference to makeGreeting, found {len(refs)}"
        ...
        if refs:
            ...
            assert (
                66 in ref_lines
            ), f"Should find makeGreeting inherit at line 67, found at lines {[l + 1 for l in ref_lines]}"

    @pytest.mark.xfail(
        is_ci, reason="Test is flaky"
    )  # TODO: Re-enable if the hover test becomes more stable (#1040)
    @pytest.mark.parametrize("language_server", [Language.NIX], indirect=True)
    def test_hover_information(self, language_server: SolidLanguageServer) -> None:
        ...
        if isinstance(hover_info, dict) and len(hover_info) > 0:
            ...
            ), "Hover should have contents or value"

    @pytest.mark.parametrize("language_server", [Language.NIX], indirect=True)
    def test_cross_file_references_utils_import(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        assert (
            len(default_refs) >= 2
        ), f"Should find at least 2 references to utils in default.nix, found {len(default_refs)}"
    ...
    def test_verify_imports_exist(self, language_server: SolidLanguageServer) -> None:
        ...
        symbol_names = {sym.get("name") for sym in symbol_list if isinstance(sym, dict)}
        assert (
            "makeGreeting" in symbol_names
        ), "makeGreeting should be found in default.nix"

        # Verify lib/utils.nix has the expected structure
        utils_symbols = language_server.request_document_symbols(
            "lib/utils.nix"
        ).get_all_symbols_and_roots()
        assert utils_symbols is not None
        utils_list = (
            utils_symbols[0] if isinstance(utils_symbols, tuple) else utils_symbols
        )
    ...
    def test_go_to_definition_cross_file(
        ...
        if len(definitions) > 0:
            ...
            assert any(
                "utils" in def_item.get("uri", "")
                or "default.nix" in def_item.get("uri", "")
                for def_item in definitions
            ), "Definition should relate to utils import or utils.nix file"

    @pytest.mark.parametrize("language_server", [Language.NIX], indirect=True)
    def test_definition_navigation_in_flake(
        self, language_server: SolidLanguageServer
    ) -> None:
    ...
    def test_bare_symbol_names(self, language_server) -> None:
        ...
        for s in all_symbols:
            ...
            if s["name"] in {
                ...
                continue
            if (
                s["kind"] in {SymbolKind.Field, SymbolKind.Property}
                and "." in s["name"]
            ):
                continue
