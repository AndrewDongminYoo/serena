...
def test_request_rename_symbol_edit_opens_file_before_rename(tmp_path) -> None:
    ...
    notify = MagicMock()
    notify.did_open_text_document.side_effect = lambda *_args, **_kwargs: events.append(
        "didOpen"
    )
    notify.did_close_text_document.side_effect = (
        lambda *_args, **_kwargs: events.append("didClose")
    )
