...
def _php_supports_phpactor() -> bool:
    completed_process = subprocess.run(
        ["php", "-m"], capture_output=True, text=True, check=False
    )
    installed_extensions = {
        line.strip().lower() for line in completed_process.stdout.splitlines()
    }
...
class TestPhpLanguageServers:
    ...
    @pytest.mark.parametrize("language_server", _php_servers, indirect=True)
    @pytest.mark.parametrize("repo_path", [Language.PHP], indirect=True)
    def test_find_definition_within_file(
        self, language_server: SolidLanguageServer, repo_path: Path
    ) -> None:
        # In index.php:
        ...
        if language_server.language_server.language == Language.PHP_PHPACTOR:
            definition_location_list = language_server.request_definition(
                str(repo_path / "index.php"), 11, 6
            )
        else:
            definition_location_list = language_server.request_definition(
                str(repo_path / "index.php"), 10, 6
            )

        assert (
            definition_location_list
        ), f"Expected non-empty definition_location_list but got {definition_location_list=}"
        if language_server.language_server.language == Language.PHP_PHPACTOR:
    ...
    def test_find_definition_across_files(
        ...
        if language_server.language_server.language == Language.PHP_PHPACTOR:
            definition_location_list = language_server.request_definition(
                str(repo_path / "index.php"), 13, 5
            )
        else:
            definition_location_list = language_server.request_definition(
                str(repo_path / "index.php"), 12, 5
            )
    ...
    @pytest.mark.parametrize("language_server", _php_servers, indirect=True)
    @pytest.mark.parametrize("repo_path", [Language.PHP], indirect=True)
    def test_find_definition_simple_variable(
        self, language_server: SolidLanguageServer, repo_path: Path
    ) -> None:
        ...
        )  # Definition of $localVar (0-indexed)
        if language_server.language_server.language != Language.PHP_PHPACTOR:
            assert (
                definition_location["range"]["start"]["character"] == 0
            )  # $localVar (0-indexed)
    ...
    def test_find_references_within_file(
        ...
        assert (
            references
        ), f"Expected non-empty references for $greeting but got {references=}"
        ...
        if language_server.language_server.language == Language.PHP_PHPACTOR:
            ...
            matching = [
                ...
                if loc["uri_suffix"] == "index.php" and loc["line"] == 11
            ]
            assert (
                matching
            ), f"Expected reference to $greeting on line 11 of index.php, got {actual_locations}"
        else:
            # Intelephense, when asked for references from usage, seems to only return the usage itself.
            assert (
                len(references) == 1
            ), "Expected to find 1 reference for $greeting (the usage itself)"

            expected_locations = [
                {"uri_suffix": "index.php", "line": 11, "character": 5}
            ]  # Usage: echo $greeting (points to $)
    ...
    def test_find_references_across_files(
        ...
        references = language_server.request_references(
            helper_php_path, 2, len("function ")
        )

        assert (
            references
        ), f"Expected non-empty references for helperFunction but got {references=}"
        ...
        if language_server.language_server.language == Language.PHP_PHPACTOR:
            ...
            matching = [
                loc
                for loc in actual_locations_comparable
                if loc["uri_suffix"] == "index.php" and loc["line"] == 13
            ]
            assert (
                matching
            ), f"Usage of helperFunction in index.php (line 13) not found in {actual_locations_comparable}"
        else:
            ...
            assert (
                usage_in_index_php in actual_locations_comparable
            ), "Usage of helperFunction in index.php not found"
    ...
    def test_find_symbol(self, language_server: SolidLanguageServer) -> None:
        ...
        ), "helperFunction not found in symbol tree"
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "greet"
        ), "greet function not found in symbol tree"
    ...
    def test_document_symbols(self, language_server: SolidLanguageServer) -> None:
        ...
        assert (
            "helperFunction" in symbol_names
        ), f"helperFunction not found in document symbols. Found: {symbol_names}"

    @pytest.mark.parametrize("language_server", [Language.PHP], indirect=True)
    def test_document_symbols_hierarchical_structure(
        ...
        assert (
            "Animal" in root_names
        ), f"Animal class not found at root level. Roots: {root_names}"
        assert (
            "Dog" in root_names
        ), f"Dog class not found at root level. Roots: {root_names}"
        assert (
            "Cat" in root_names
        ), f"Cat class not found at root level. Roots: {root_names}"
        ...
        assert (
            not missing
        ), f"Dog class missing expected methods: {missing}. Children found: {dog_child_names}"

        # Methods must NOT appear at root level (that would indicate the flat fallback format).
        assert (
            "greet" not in root_names
        ), f"greet should be a child of Dog, not at root level. Roots: {root_names}"
        assert (
            "fetch" not in root_names
        ), f"fetch should be a child of Dog, not at root level. Roots: {root_names}"

    @pytest.mark.parametrize("language_server", [Language.PHP], indirect=True)
    def test_full_symbol_tree_within_file(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        symbols = language_server.request_full_symbol_tree(
            within_relative_path="sample.php"
        )

        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "Dog"
        ), "Dog not found in sample.php symbol tree"
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "Animal"
        ), "Animal not found in sample.php symbol tree"
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "greet"
        ), "greet method not found in sample.php symbol tree"
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "fetch"
        ), "fetch method not found in sample.php symbol tree"
        ...
        if dog_root is not None:
            assert SymbolUtils.symbol_tree_contains_name(
                [dog_root], "greet"
            ), "greet should be nested under Dog in symbol tree"
