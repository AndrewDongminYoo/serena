...
class TestMslDocumentSymbols:
    ...
    def test_document_symbols_main(self, language_server: SolidLanguageServer) -> None:
        ...
        assert (
            "calculate.doubloons" in symbol_names
        ), f"calculate.doubloons alias not found. Found: {symbol_names}"
        assert (
            "show.player.info" in symbol_names
        ), f"show.player.info alias not found. Found: {symbol_names}"

    @pytest.mark.parametrize("language_server", [Language.MSL], indirect=True)
    def test_document_symbols_events(
        ...
        assert (
            len(on_events) >= 2
        ), f"Expected at least 2 event handlers. Found: {on_events}"
        # Check for raw event
        raw_events = [n for n in symbol_names if n.startswith("raw ")]
        assert (
            len(raw_events) >= 1
        ), f"Expected at least 1 raw event handler. Found: {raw_events}"
        # Check for menu
    ...
    def test_document_symbols_utils(self, language_server: SolidLanguageServer) -> None:
        ...
        symbol_names = [s.get("name") for s in all_symbols if s.get("name")]
        assert (
            "format.coins" in symbol_names
        ), f"format.coins alias not found. Found: {symbol_names}"
        assert (
            "is.admin" in symbol_names
        ), f"is.admin alias not found. Found: {symbol_names}"
        assert (
            "welcome.message" in symbol_names
        ), f"welcome.message alias not found. Found: {symbol_names}"

    @pytest.mark.parametrize("language_server", [Language.MSL], indirect=True)
    def test_document_symbols_dialog_and_ctcp(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        symbol_names = [s.get("name") for s in all_symbols if s.get("name")]
        assert (
            "dialog settings" in symbol_names
        ), f"dialog settings not found. Found: {symbol_names}"
        # Check for CTCP handler
        ctcp_events = [n for n in symbol_names if n.startswith("ctcp ")]
        assert (
            len(ctcp_events) >= 1
        ), f"Expected at least 1 ctcp handler. Found: {ctcp_events}"
    ...
    def test_find_symbol(self, language_server: SolidLanguageServer) -> None:
        ...
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "greet"
        ), "greet not found in symbol tree"
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "format.coins"
        ), "format.coins not found in symbol tree"
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "show.player.info"
        ), "show.player.info not found in symbol tree"
    ...
    def test_bare_symbol_names(self, language_server: SolidLanguageServer) -> None:
        ...
        for s in all_symbols:
            ...
            if has_malformed_name(
                s, period_allowed=True, colon_allowed=True, whitespace_allowed=True
            ):
    ...
    def test_find_references_within_file(
        ...
        ), f"Expected reference to greet at line 13 in main.mrc, got {actual_locations}"
    ...
    def test_find_references_across_files(
        ...
        fc_symbol = next(
            (s for s in all_symbols if s.get("name") == "format.coins"), None
        )
        assert (
            fc_symbol is not None
        ), "Could not find 'format.coins' symbol in utils.mrc"
        ...
        assert (
            len(main_refs) >= 1
        ), f"Expected at least 1 reference in main.mrc, got {main_refs}"
    ...
    def test_workspace_symbol(self, language_server: SolidLanguageServer) -> None:
        ...
        assert any(
            "greet" in str(s.get("name", "")) for s in result
        ), f"Expected at least one result containing 'greet', got {result}"
