...
@pytest.mark.skipif(
    shutil.which("terraform") is None and not is_ci,
    reason="Terraform CLI is not available",
)
...
class TestLanguageServerBasics:
    ...
    @pytest.mark.parametrize("language_server", [Language.TERRAFORM], indirect=True)
    def test_request_references_aws_instance(
        ...
        aws_instance_symbol = next(
            (
                s
                for s in symbols[0]
                if s.get("name") == 'resource "aws_instance" "web_server"'
            ),
            None,
        )
    ...
    @pytest.mark.parametrize("language_server", [Language.TERRAFORM], indirect=True)
    def test_request_references_variable(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        var_symbol = next(
            (s for s in symbols[0] if s.get("name") == 'variable "instance_type"'), None
        )
