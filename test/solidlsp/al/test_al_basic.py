...
pytestmark = [
    pytest.mark.al,
    pytest.mark.skipif(
        not language_tests_enabled(Language.AL), reason="AL tests are disabled"
    ),
]
...
class TestExtractALDisplayName:
    ...
    def test_table_with_quoted_name(self) -> None:
        ...
        assert (
            ALLanguageServer._extract_al_display_name('Table 50000 "TEST Customer"')
            == "TEST Customer"
        )
    ...
    def test_page_with_quoted_name(self) -> None:
        ...
        assert (
            ALLanguageServer._extract_al_display_name('Page 50001 "TEST Customer Card"')
            == "TEST Customer Card"
        )
    ...
    def test_codeunit_unquoted(self) -> None:
        ...
        assert (
            ALLanguageServer._extract_al_display_name("Codeunit 50000 CustomerMgt")
            == "CustomerMgt"
        )
    ...
    def test_enum_unquoted(self) -> None:
        ...
        assert (
            ALLanguageServer._extract_al_display_name("Enum 50000 CustomerType")
            == "CustomerType"
        )
    ...
    def test_interface_no_id(self) -> None:
        ...
        assert (
            ALLanguageServer._extract_al_display_name("Interface IPaymentProcessor")
            == "IPaymentProcessor"
        )
    ...
    def test_table_extension(self) -> None:
        ...
        assert (
            ALLanguageServer._extract_al_display_name(
                'TableExtension 50000 "Ext Customer"'
            )
            == "Ext Customer"
        )
    ...
    def test_page_extension(self) -> None:
        ...
        assert (
            ALLanguageServer._extract_al_display_name(
                'PageExtension 50000 "My Page Ext"'
            )
            == "My Page Ext"
        )
    ...
    def test_non_al_object_unchanged(self) -> None:
        ...
        assert (
            ALLanguageServer._extract_al_display_name("CreateCustomer")
            == "CreateCustomer"
        )
    ...
    def test_report_with_quoted_name(self) -> None:
        ...
        assert (
            ALLanguageServer._extract_al_display_name('Report 50000 "Sales Invoice"')
            == "Sales Invoice"
        )
    ...
    def test_query_unquoted(self) -> None:
        ...
        assert (
            ALLanguageServer._extract_al_display_name("Query 50000 CustomerQuery")
            == "CustomerQuery"
        )
...
class TestALLanguageServer:
    ...
    def test_symbol_names_are_normalized(
        ...
        assert (
            customer_table is not None
        ), "Could not find 'TEST Customer' table symbol (name should be normalized)"
        # Name should be just "TEST Customer", not "Table 50000 'TEST Customer'"
        assert (
            customer_table["name"] == "TEST Customer"
        ), f"Expected normalized name 'TEST Customer', got '{customer_table['name']}'"
    ...
    def test_find_symbol_exact_match(
        ...
        for root in symbols.root_symbols:
            ...
            if matches:
                assert (
                    len(matches) >= 1
                ), "Should find at least one match for 'TEST Customer'"
                assert (
                    matches[0].name == "TEST Customer"
                ), f"Expected 'TEST Customer', got '{matches[0].name}'"
                return

        pytest.fail("Could not find 'TEST Customer' symbol by exact name match")

    @pytest.mark.parametrize("language_server", [Language.AL], indirect=True)
    def test_find_codeunit_exact_match(
        self, language_server: SolidLanguageServer
    ) -> None:
    ...
    def test_find_symbol(self, language_server: SolidLanguageServer) -> None:
        ...
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "TEST Customer"
        ), "TEST Customer table not found in symbol tree"

        # Check for page symbols
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "TEST Customer Card"
        ), "TEST Customer Card page not found in symbol tree"
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "TEST Customer List"
        ), "TEST Customer List page not found in symbol tree"

        # Check for codeunit symbols
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "CustomerMgt"
        ), "CustomerMgt codeunit not found in symbol tree"
        assert SymbolUtils.symbol_tree_contains_name(
        ...
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "CustomerType"
        ), "CustomerType enum not found in symbol tree"

        # Check for interface symbol
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "IPaymentProcessor"
    ...
    def test_find_table_fields(self, language_server: SolidLanguageServer) -> None:
        ...
        if "children" in customer_table:
            ...
            if "children" in fields_group:
                field_names = [
                    child.get("name", "") for child in fields_group.get("children", [])
                ]
                assert any(
                    "Name" in name for name in field_names
                ), f"Name field not found. Fields: {field_names}"
                assert any(
                    "Balance" in name for name in field_names
                ), f"Balance field not found. Fields: {field_names}"
    ...
    def test_find_procedures(self, language_server: SolidLanguageServer) -> None:
        ...
        if "children" in codeunit_symbol:
            procedure_names = [
                child.get("name", "") for child in codeunit_symbol.get("children", [])
            ]
            assert any(
                "CreateCustomer" in name for name in procedure_names
            ), "CreateCustomer procedure not found"
            assert any(
                "TestNoSeries" in name for name in procedure_names
            ), "TestNoSeries procedure not found"
    ...
    def test_find_referencing_symbols(
        ...
        if customer_symbol and "selectionRange" in customer_symbol:
            ...
            refs = language_server.request_references(
                table_file, sel_start["line"], sel_start["character"]
            )
    ...
    def test_cross_file_symbols(self, language_server: SolidLanguageServer) -> None:
        ...
        assert (
            "TEST Customer" in al_object_names
        ), f"TEST Customer not found in: {al_object_names}"
        assert (
            "CustomerMgt" in al_object_names
        ), f"CustomerMgt not found in: {al_object_names}"
        assert (
            "CustomerType" in al_object_names
        ), f"CustomerType not found in: {al_object_names}"
...
class TestALHoverInjection:
    ...
    def _get_symbol_hover(
        self, language_server: SolidLanguageServer, file_path: str, symbol_name: str
    ...
    @pytest.mark.parametrize("language_server", [Language.AL], indirect=True)
    def test_hover_table_injects_full_name(
        self, language_server: SolidLanguageServer
    ) -> None:
        """Test that hovering over a Table symbol shows the full object name with ID."""
        file_path = os.path.join("src", "Tables", "Customer.Table.al")
        hover, value = self._get_symbol_hover(
            language_server, file_path, "TEST Customer"
        )
        ...
        ), f"Hover should contain full Table name with ID. Got: {value[:200]}"

    @pytest.mark.parametrize("language_server", [Language.AL], indirect=True)
    def test_hover_page_injects_full_name(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        hover, value = self._get_symbol_hover(
            language_server, file_path, "TEST Customer Card"
        )

        assert hover is not None, "Hover should return a result for Page symbol"
        assert value is not None, "Hover should have content"
        assert (
            '**Page 50001 "TEST Customer Card"**' in value
        ), f"Hover should contain full Page name with ID. Got: {value[:200]}"

    @pytest.mark.parametrize("language_server", [Language.AL], indirect=True)
    def test_hover_codeunit_injects_full_name(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        assert value is not None, "Hover should have content"
        assert (
            "**Codeunit 50000 CustomerMgt**" in value
        ), f"Hover should contain full Codeunit name with ID. Got: {value[:200]}"

    @pytest.mark.parametrize("language_server", [Language.AL], indirect=True)
    def test_hover_enum_injects_full_name(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        hover, value = self._get_symbol_hover(
            language_server, file_path, "CustomerType"
        )

        assert hover is not None, "Hover should return a result for Enum symbol"
        assert value is not None, "Hover should have content"
        assert (
            "**Enum 50000 CustomerType**" in value
        ), f"Hover should contain full Enum name with ID. Got: {value[:200]}"

    @pytest.mark.parametrize("language_server", [Language.AL], indirect=True)
    def test_hover_interface_injects_full_name(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        hover, value = self._get_symbol_hover(
            language_server, file_path, "IPaymentProcessor"
        )

        assert hover is not None, "Hover should return a result for Interface symbol"
        assert value is not None, "Hover should have content"
        assert (
            "**Interface IPaymentProcessor**" in value
        ), f"Hover should contain full Interface name. Got: {value[:200]}"

    @pytest.mark.parametrize("language_server", [Language.AL], indirect=True)
    def test_hover_procedure_no_injection(
        self, language_server: SolidLanguageServer
    ) -> None:
        """Test that hovering over a procedure does NOT inject object name (procedures are not normalized)."""
        file_path = os.path.join("src", "Codeunits", "CustomerMgt.Codeunit.al")
        hover, value = self._get_child_symbol_hover(
            language_server, file_path, "CustomerMgt", "CreateCustomer"
        )
        ...
        assert not value.startswith(
            "**"
        ), f"Procedure hover should not have injected name. Got: {value[:200]}"
        # But should contain procedure info
        assert (
            "CreateCustomer" in value
        ), f"Hover should contain procedure name. Got: {value[:200]}"

    @pytest.mark.parametrize("language_server", [Language.AL], indirect=True)
    def test_hover_field_no_injection(
        self, language_server: SolidLanguageServer
    ) -> None:
                    ...
                    if child.get("name") == "fields":
                        for field in child.get("children", []):
                            if "Name" in field.get("name", ""):
                                ...
                                line = start.get("line", 0)
                                char = start.get("character", 0)
                                hover = language_server.request_hover(
                                    file_path, line, char
                                )

                                assert (
                                    hover is not None
                                ), "Hover should return a result for field"
                                value = hover.get("contents", {}).get("value", "")
                                # Field hover should NOT start with ** (no injection)
                                assert not value.startswith(
                                    "**"
                                ), f"Field hover should not have injected name. Got: {value[:200]}"
                                return

        pytest.fail("Could not find a field to test hover on")

    @pytest.mark.parametrize("language_server", [Language.AL], indirect=True)
    def test_hover_multiple_objects_correct_injection(
        self, language_server: SolidLanguageServer
    ) -> None:
        """Test that multiple AL objects each get their correct full name injected."""
        test_cases = [
            (
                os.path.join("src", "Tables", "Customer.Table.al"),
                "TEST Customer",
                'Table 50000 "TEST Customer"',
            ),
            (
                os.path.join("src", "Codeunits", "CustomerMgt.Codeunit.al"),
                "CustomerMgt",
                "Codeunit 50000 CustomerMgt",
            ),
            (
                os.path.join("src", "Enums", "CustomerType.Enum.al"),
                "CustomerType",
        ...
        for file_path, symbol_name, expected_full_name in test_cases:
            hover, value = self._get_symbol_hover(
                language_server, file_path, symbol_name
            )
    ...
    @pytest.mark.parametrize("language_server", [Language.AL], indirect=True)
    def test_hover_contains_separator_after_injection(
        self, language_server: SolidLanguageServer
    ) -> None:
    ...
    @pytest.mark.parametrize("language_server", [Language.AL], indirect=True)
    def test_hover_preserves_original_content(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        assert (
            "```al" in value
        ), f"Hover should contain original AL code block. Got: {value[:500]}"
        assert (
            'Table "TEST Customer"' in value
        ), f"Hover should contain original table definition. Got: {value[:500]}"
...
class TestALPathNormalization:
    ...
    @pytest.mark.parametrize("language_server", [Language.AL], indirect=True)
    def test_hover_with_forward_slash_path(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        for sym in symbols.root_symbols:
            if sym.get("name") == "TEST Customer":
                ...
                assert (
                    '**Table 50000 "TEST Customer"**' in value
                ), f"Hover should have injection. Got: {value[:200]}"
                return

        pytest.fail("Could not find TEST Customer symbol")

    @pytest.mark.parametrize("language_server", [Language.AL], indirect=True)
    def test_hover_with_backslash_path(
        self, language_server: SolidLanguageServer
    ) -> None:
    ...
    @pytest.mark.parametrize("language_server", [Language.AL], indirect=True)
    def test_hover_with_mixed_path_formats_symbols_backslash_hover_forward(
        self, language_server: SolidLanguageServer
    ) -> None:
    ...
    @pytest.mark.parametrize("language_server", [Language.AL], indirect=True)
    def test_hover_with_mixed_path_formats_symbols_forward_hover_backslash(
        self, language_server: SolidLanguageServer
    ) -> None:
    ...
    @pytest.mark.parametrize("language_server", [Language.AL], indirect=True)
    def test_hover_caching_multiple_files_different_path_formats(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        for symbols_path, hover_path, symbol_name, expected_injection in test_cases:
            ...
            for sym in symbols.root_symbols:
                if sym.get("name") == symbol_name:
                    ...
                    assert (
                        hover is not None
                    ), f"Hover should return a result for {symbol_name}"
                    value = hover.get("contents", {}).get("value", "")
                    assert (
    ...
    def test_bare_symbol_names(self, language_server) -> None:
        ...
        for s in all_symbols:
            is_al_display_symbol = (
                s["name"].endswith(
                    (".Codeunit", ".Page", ".Table", ".TableExt", ".Enum", ".Interface")
                )
