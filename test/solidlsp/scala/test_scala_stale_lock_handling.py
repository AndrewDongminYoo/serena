...
class TestStaleLockHandling:
    ...
    def test_auto_clean_mode_cleans_stale_lock(
        ...
        with (
            ...
            patch.object(
                ScalaLanguageServer, "__init__", lambda self, *args, **kwargs: None
            ),
        ):
            # Create instance without calling __init__
            ls = object.__new__(ScalaLanguageServer)
            settings = SolidLSPSettings(
                ls_specific_settings={Language.SCALA: {"on_stale_lock": "auto-clean"}}
            )
    ...
    def test_warn_mode_logs_warning_without_cleanup(
        ...
        with (
            ...
            ls = object.__new__(ScalaLanguageServer)
            settings = SolidLSPSettings(
                ls_specific_settings={Language.SCALA: {"on_stale_lock": "warn"}}
            )
            ...
            assert any(
                "Stale Metals lock detected" in record.message
    ...
    def test_fail_mode_raises_exception(
        ...
        with (
            ...
            ls = object.__new__(ScalaLanguageServer)
            settings = SolidLSPSettings(
                ls_specific_settings={Language.SCALA: {"on_stale_lock": "fail"}}
            )
    ...
    def test_active_instance_logs_info_when_enabled(
        ...
        with (
            ...
            assert any(
                "Another Metals instance detected" in record.message
                for record in caplog.records
            )
    ...
    def test_no_database_proceeds_silently(
        ...
        with (
            ...
            assert not any(
                "Another Metals instance" in record.message for record in caplog.records
            )
