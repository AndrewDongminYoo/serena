...
class TestNuGetOrgDownload:
    ...
    def test_download_nuget_package_uses_direct_url(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            captured_calls: list[
                tuple[str, str, str, str | None, tuple[str, ...] | list[str] | None]
            ] = []
            ...
            def fake_download_and_extract(
                ...
                captured_calls.append(
                    (url, target_path, archive_type, expected_sha256, allowed_hosts)
                )
            ...
            with patch(
                ...
                package_dir = dependency_provider._download_nuget_package(
                    test_dependency
                )
            ...
            assert (
                ...
                == Path(temp_dir)
                / "temp_downloads"
                / "roslyn-language-server.linux-x64.5.5.0-2.26078.4"
            )
            ...
            assert (
                called_url == test_dependency.url
            ), f"Should use URL from RuntimeDependency: {test_dependency.url}"
    ...
    def test_runtime_dependencies_use_nuget_org_urls(self):
        ...
        lang_server_deps = [
            dep for dep in _RUNTIME_DEPENDENCIES if dep.id == "CSharpLanguageServer"
        ]

        assert (
            len(lang_server_deps) == 6
        ), "Should have 6 language server platform variants"

        for dep in lang_server_deps:
            # Verify package name uses roslyn-language-server
            assert (
                dep.package_name is not None
            ), f"Package name should be set for {dep.platform_id}"
            ...
            assert (
                dep.package_version == "5.5.0-2.26078.4"
            ), f"Should use NuGet.org version 5.5.0-2.26078.4, got: {dep.package_version}"

            # Verify URL points to NuGet.org
            assert dep.url is not None, f"URL should be set for {dep.platform_id}"
            assert (
                "nuget.org" in dep.url
            ), f"URL should point to nuget.org, got: {dep.url}"
            assert (
                "azure" not in dep.url.lower()
            ), f"URL should not point to Azure feed, got: {dep.url}"
