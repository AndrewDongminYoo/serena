...
class TestCSharpLanguageServer:
    ...
    def test_find_symbol(self, language_server: SolidLanguageServer) -> None:
        ...
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "Program"
        ), "Program class not found in symbol tree"
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "Calculator"
    ...
    def test_get_document_symbols(self, language_server: SolidLanguageServer) -> None:
        ...
        class_names = [
            s.get("name") for s in symbols if s.get("kind") == 5
        ]  # 5 is class
    ...
    def test_find_referencing_symbols(
        ...
        ), "Could not find 'Add' method symbol in Program.cs"
    ...
    @pytest.mark.parametrize("language_server", [Language.CSHARP], indirect=True)
    def test_nested_namespace_symbols(
        self, language_server: SolidLanguageServer
    ) -> None:
    ...
    def test_find_referencing_symbols_across_files(
        ...
        assert (
            subtract_symbol is not None
        ), "Could not find 'Subtract' method symbol in Program.cs"
        ...
        refs_second_call = language_server.request_references(
            file_path, sel_start["line"], sel_start["character"] + 1
        )
        assert (
            refs_second_call == refs
        ), "Second call to request_references should return the same results"
    ...
    def test_hover_includes_type_information(
        ...
        ), "Hover should return information for Name property"
        ...
        assert (
            "string" in hover_text
        ), f"Hover should include 'string' type, got: {hover_text}"
        assert (
            "Name" in hover_text
        ), f"Hover should include 'Name' property name, got: {hover_text}"
        ...
        assert (
            hover_method is not None
        ), "Hover should return information for IsAdult method"
        ...
        assert (
            "bool" in method_hover_text
        ), f"Hover should include 'bool' return type, got: {method_hover_text}"
        assert (
            "IsAdult" in method_hover_text
        ), f"Hover should include 'IsAdult' method name, got: {method_hover_text}"
    ...
    if language_has_verified_implementation_support(Language.CSHARP):
        ...
        def test_find_implementations(
            ...
            pos = find_identifier_position(
                repo_path / "Services" / "IGreeter.cs", "FormatGreeting"
            )
            ...
            implementations = language_server.request_implementation(
                "Services/IGreeter.cs", *pos
            )
            assert (
                implementations
            ), "Expected at least one implementation of IGreeter.FormatGreeting"
            assert any(
                "ConsoleGreeter.cs" in implementation.get("relativePath", "")
        ...
        def test_request_implementing_symbols(
            ...
            assert any(
                ...
                and "ConsoleGreeter.cs" in symbol["location"].get("relativePath", "")
