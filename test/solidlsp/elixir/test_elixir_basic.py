...
class TestElixirBasic:
    ...
    def test_request_references_function_definition(
        ...
        for symbol in symbols[0]:  # Top level symbols
            if symbol.get("name") == "User" and symbol.get("kind") == 2:  # Module
                for child in symbol.get("children", []):
                    if (
                        child.get("name", "").startswith("def new(")
                        and child.get("kind") == 12
                    ):  # Function
    ...
    @pytest.mark.parametrize("language_server", [Language.ELIXIR], indirect=True)
    def test_request_references_create_user_function(
        self, language_server: SolidLanguageServer
    ):
        ...
        for symbol in symbols[0]:  # Top level symbols
            if (
                symbol.get("name") == "UserService" and symbol.get("kind") == 2
            ):  # Module
                for child in symbol.get("children", []):
                    if (
                        child.get("name", "").startswith("def create_user(")
                        and child.get("kind") == 12
                    ):  # Function
        ...
        if not create_user_symbol or "selectionRange" not in create_user_symbol:
            pytest.skip(
                "UserService.create_user function or its selectionRange not found"
            )
