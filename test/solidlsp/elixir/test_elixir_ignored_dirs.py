...
pytestmark = [
    pytest.mark.elixir,
    pytest.mark.skipif(
        EXPERT_UNAVAILABLE, reason=f"Expert not available: {EXPERT_UNAVAILABLE_REASON}"
    ),
]
...
def test_symbol_tree_ignores_dir(ls_with_ignored_dirs: SolidLanguageServer):
    ...
    assert expected_dirs.issubset(
        children_names
    ), f"Expected {expected_dirs} to be in {children_names}"
...
def test_find_references_ignores_dir(ls_with_ignored_dirs: SolidLanguageServer):
    ...
    assert not any(
        "scripts" in ref["relativePath"] for ref in references
...
def test_refs_and_symbols_with_glob_patterns(repo_path: Path) -> None:
    ...
    with start_ls_context(
        language=Language.ELIXIR, repo_path=str(repo_path), ignored_paths=ignored_paths
    ) as ls:
        ...
        assert (
            "scripts" not in children_names
        ), f"scripts should not be in {children_names} (glob pattern)"
        assert (
            "ignored_dir" not in children_names
        ...
        if user_symbol and "selectionRange" in user_symbol:
            ...
            assert not any(
                "scripts" in ref["relativePath"] for ref in references
            ), "scripts should be ignored (glob)"
            assert not any(
                "ignored_dir" in ref["relativePath"] for ref in references
