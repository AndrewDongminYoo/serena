...
@pytest.mark.skipif(
    sys.platform == "win32",
    reason="ZLS is disabled on Windows - cross-file references don't work reliably. Reason unknown.",
)
class TestZigLanguageServer:
    ...
    @pytest.mark.parametrize("language_server", [Language.ZIG], indirect=True)
    def test_find_symbols_in_calculator(
        ...
        ), f"Expected exactly {expected_methods}, found: {found_methods}"

    @pytest.mark.parametrize("language_server", [Language.ZIG], indirect=True)
    def test_find_symbols_in_math_utils(
        self, language_server: SolidLanguageServer
    ) -> None:
    ...
    def test_find_references_within_file(
        ...
        sel_range = calculator_symbol.get(
            "selectionRange", calculator_symbol.get("range")
        )
        ...
        for line in test_lines:
            assert (
                line in ref_lines
            ), f"Should find Calculator reference at line {line + 1}, found at lines {[l + 1 for l in ref_lines]}"

    @pytest.mark.parametrize("language_server", [Language.ZIG], indirect=True)
    @pytest.mark.skipif(
        sys.platform == "win32",
        reason="ZLS cross-file references don't work reliably on Windows - URI path handling issues",
    )
    def test_cross_file_references_with_open_files(
        self, language_server: SolidLanguageServer
    ) -> None:
        """
        ...
        with language_server.open_file("build.zig"):
            with language_server.open_file(os.path.join("src", "main.zig")):
                with language_server.open_file(os.path.join("src", "calculator.zig")):
                    ...
                    assert (
                        sel_range is not None
                    ), "Calculator symbol has no range information"
                    ...
                    main_refs = [
                        ref for ref in refs if "main.zig" in ref.get("uri", "")
                    ]

                    assert (
                        len(main_refs) >= 1
                    ), f"Should find at least 1 Calculator reference in main.zig, found {len(main_refs)}"
    ...
    @pytest.mark.parametrize("language_server", [Language.ZIG], indirect=True)
    def test_cross_file_references_within_file(
        self, language_server: SolidLanguageServer
    ) -> None:
        """
    ...
    def test_go_to_definition_cross_file(
        self, language_server: SolidLanguageServer
    ) -> None:
        """
        ...
        assert "calculator.zig" in calc_def.get(
            "uri", ""
        ), "Definition should be in calculator.zig"
    ...
    def test_cross_file_function_usage(
        self, language_server: SolidLanguageServer
    ) -> None:
        ...
        definitions = language_server.request_definition(
            os.path.join("src", "main.zig"), 22, 40
        )  # Position of "factorial"
    ...
    @pytest.mark.parametrize("language_server", [Language.ZIG], indirect=True)
    def test_verify_cross_file_imports(
        self, language_server: SolidLanguageServer
    ) -> None:
        """Verify that our test files have proper cross-file imports."""
        # Verify main.zig imports
        main_symbols = language_server.request_document_symbols(
            os.path.join("src", "main.zig")
        ).get_all_symbols_and_roots()
        ...
        calc_symbols = language_server.request_document_symbols(
            os.path.join("src", "calculator.zig")
        ).get_all_symbols_and_roots()
        ...
        assert (
            "Calculator" in calc_names
        ), "Calculator struct should be in calculator.zig"

        # Verify math_utils.zig exports functions
        math_symbols = language_server.request_document_symbols(
        ...
        assert (
            "factorial" in math_names
        ), "factorial function should be in math_utils.zig"
    ...
    def test_hover_information(self, language_server: SolidLanguageServer) -> None:
        ...
        hover_info = language_server.request_hover(
            file_path, 4, 8
        )  # Position of "main" function

        assert (
            hover_info is not None
        ), "Should provide hover information for main function"

        # Hover info could be a dict with 'contents' or a string
        if isinstance(hover_info, dict):
            assert (
                "contents" in hover_info or "value" in hover_info
