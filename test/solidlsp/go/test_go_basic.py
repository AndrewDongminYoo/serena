...
class TestGoLanguageServer:
    ...
    def test_find_symbol(self, language_server: SolidLanguageServer) -> None:
        ...
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "Helper"
        ), "Helper function not found in symbol tree"
        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "DemoStruct"
        ), "DemoStruct not found in symbol tree"

    @pytest.mark.parametrize("language_server", [Language.GO], indirect=True)
    def test_find_symbol_matches_go_method_by_bare_name(
        self, language_server: SolidLanguageServer
    ) -> None:
        symbols = language_server.request_full_symbol_tree(
            within_relative_path="main.go"
        )

        assert SymbolUtils.symbol_tree_contains_name(
            symbols, "Value"
        ), "Expected Go method name to be normalized to bare name"
        assert not SymbolUtils.symbol_tree_contains_name(
            symbols, "(*DemoStruct).Value"
        ), "Expected receiver-qualified Go method name to be normalized away"

        bare_name_matches = [
            match
            for root in symbols
            for match in LanguageServerSymbol(root).find("Value")
        ]
    ...
    def test_find_referencing_symbols(
        ...
        for sym in symbols[0]:
            if sym.get("name") == "Helper":
                ...
                break
        assert (
            helper_symbol is not None
        ), "Could not find 'Helper' function symbol in main.go"
        ...
        assert any(
            "main.go" in ref.get("uri", "") for ref in refs
        ), "Expected at least one reference result to point at main.go"
    ...
    if language_has_verified_implementation_support(Language.GO):
        ...
        def test_find_implementations(
            ...
            assert (
                implementations
            ), "Expected at least one implementation of Greeter.FormatGreeting"
            assert any(
                "main.go" in implementation.get("relativePath", "")
        ...
        def test_request_implementing_symbols(
            ...
            ), "Expected implementing symbols for Greeter.FormatGreeting"
            assert any(
                symbol.get("name") == "FormatGreeting"
                and "main.go" in symbol["location"].get("relativePath", "")
                for symbol in implementing_symbols
            ), f"Expected FormatGreeting symbol, got: {implementing_symbols}"


def _filter_symbols_by_name_in_repo(
    symbols: list | None, target_name: str, repo_name: str = "test_repo"
) -> list:
    ...
    return [
        ...
        and repo_name in s.get("location", {}).get("uri", "")
    ]
...
class TestGoBuildTags:
    ...
    def test_default_context_contains_xnotfoo(self, tmp_path: Path) -> None:
        ...
        with start_ls_context(
            Language.GO, repo_path=str(repo_path), solidlsp_dir=tmp_path
        ) as ls:
            xnotfoo_symbols = ls.request_workspace_symbol("XNotFoo")
            xfoo_symbols = ls.request_workspace_symbol("XFoo")

            xnotfoo_matches = _filter_symbols_by_name_in_repo(
                xnotfoo_symbols, "XNotFoo"
            )
    ...
    def test_foo_context_contains_xfoo(self, tmp_path: Path) -> None:
        ...
        with start_ls_context(
            ...
            ls_specific_settings=ls_settings,
    ...
    def test_disk_cache_is_invalidated_on_build_context_switch(
        self, tmp_path: Path
    ) -> None:
        ...
        def _assert_caches_loaded_and_clean(ls: SolidLanguageServer) -> None:
            ...
            assert (
                ls._raw_document_symbols_cache
            ), "Expected raw document-symbol cache to load from disk"
            assert (
                ls._document_symbols_cache
            ), "Expected document-symbol cache to load from disk"
        ...
        with start_ls_context(
            ...
            assert (
                cache_files
            ), f"Expected SolidLSP to create cache artifacts under {cache_dir}"
            ...
            assert (
                versioned_cache_files
            ), f"Expected at least one SolidLSP cache file with a __cache_version under {cache_dir}"
            saved_versions = {v for _, v in versioned_cache_files}
            assert (
                default_raw_cache_version in saved_versions
                or default_doc_cache_version in saved_versions
