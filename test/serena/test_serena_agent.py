...
class DiagnosticCase(BaseCase):
    ...
    def assert_matches(self, tool_output: dict) -> None:
        ...
        """
        assert (
            self.relative_path in tool_output
        ...
        for expected_fragment in [self.message_fragment1, self.message_fragment2]:
            if expected_fragment is not None:
                assert any(
                    expected_fragment in message for message in diagnostic_messages
                ), diagnostic_messages
...
FIND_DEFINING_SYMBOL_CASES = [
    ...
    FindDefiningSymbolCase(
        ...
        pytest.mark.xfail(
            reason="F# language server cannot reliably resolve defining symbols"
        ),
    ),
]
...
FIND_IMPLEMENTATION_CASES = [
    ...
    FindImplementationCase(
        ...
        definition_file=os.path.join(
            "src", "main", "java", "test_repo", "Greeter.java"
        ),
        implementation_file=os.path.join(
            "src", "main", "java", "test_repo", "ConsoleGreeter.java"
        ),
        expected_symbol_name="formatGreeting",
    ).to_pytest_param(),
    FindImplementationCase(
...
FIND_SYMBOL_REFERENCES_CASES = [
    ...
    FindSymbolCase(
        ...
        symbol_name="Model",
        expected_kind="Class",
        expected_file="Model.java",
    ).to_pytest_param(),
    FindSymbolCase(
        language=Language.KOTLIN,
        id="kotlin_model_struct",
        symbol_name="Model",
        expected_kind="Struct",
        expected_file="Model.kt",
    ).to_pytest_param(),
    FindSymbolCase(
        language=Language.TYPESCRIPT,
...
FIND_REFERENCE_CASES = [
    ...
    FindReferenceCase(
        ...
        pytest.mark.xfail(False, reason="TypeScript language server is unreliable")
    ),
    FindReferenceCase(
        language=Language.FSHARP,
        id="fsharp_add_refs",
...
FIND_SYMBOL_NAME_PATH_NO_MATCH_CASES = [
    FindSymbolNoMatchCase(
        language=Language.PYTHON,
        id="nested_class_not_top_level",
        name_path="/NestedClass",
    ).to_pytest_param(),
    FindSymbolNoMatchCase(
        language=Language.PYTHON,
        id="nested_class_missing_parent",
        name_path="/NoSuchParent/NestedClass",
    ).to_pytest_param(),
]
...
SAFE_DELETE_BLOCKED_CASES = [
    ...
    SafeDeleteCase(
        ...
        *(
            [pytest.mark.skip(reason="Kotlin LSP JVM crashes on restart in CI")]
            if is_ci
            else []
        ),
    ),
...
SAFE_DELETE_SUCCEEDS_CASES = [
    ...
    SafeDeleteCase(
        ...
        relative_path=os.path.join(
            "src", "main", "java", "test_repo", "ModelUser.java"
        ),
    ).to_pytest_param(),
    SafeDeleteCase(
        language=Language.KOTLIN,
        ...
        name_path="ModelUser",
        relative_path=os.path.join(
            "src", "main", "kotlin", "test_repo", "ModelUser.kt"
        ),
    ).to_pytest_param(
        *(
            [pytest.mark.skip(reason="Kotlin LSP JVM crashes on restart in CI")]
...
def serena_config():
    config = SerenaConfig(
        gui_log_window=False, web_dashboard=False, log_level=logging.ERROR
    )
...
@contextmanager
def project_file_modification_context(
    serena_agent: SerenaAgent, relative_path: str
) -> Iterator[None]:
...
@pytest.fixture
def serena_agent(
    request: pytest.FixtureRequest, serena_config
) -> Iterator[SerenaAgent]:
...
class TestSerenaAgent:
    ...
    def _assert_symbol_info_present(
        ...
        if symbol[
            "kind"
        ] == SymbolKind.Class.name and serena_agent.get_active_lsp_languages() == [
            Language.JAVA
        ]:
            assert (
                "A simple model class" in symbol_info
            ), f"Java class docstring not found in symbol info: {symbol}"

    @pytest.mark.parametrize(
        "serena_agent,case", FIND_SYMBOL_REFERENCES_CASES, indirect=["serena_agent"]
    )
    def test_find_symbol(self, serena_agent: SerenaAgent, case: FindSymbolCase) -> None:
        agent = serena_agent
        find_symbol_tool = agent.get_tool(FindSymbolTool)
        result = find_symbol_tool.apply(
            name_path_pattern=case.symbol_name, include_info=True
        )
    ...
    def test_find_symbol_references(
        self, serena_agent: SerenaAgent, case: FindReferenceCase
    ) -> None:
        # Find the symbol location first
        find_symbol_tool = serena_agent.get_tool(FindSymbolTool)
        result = find_symbol_tool.apply(
            name_path_pattern=case.symbol_name, relative_path=case.definition_file
        )
        ...
        result = find_refs_tool.apply(
            name_path=def_symbol["name_path"], relative_path=def_symbol["relative_path"]
        )
        ...
        assert contains_ref_with_relative_path(
            refs, case.reference_file
        ), f"Expected to find reference to {case.symbol_name} in {case.reference_file}. refs={refs}"

    @pytest.mark.parametrize(
        "serena_agent,case", FIND_DEFINING_SYMBOL_REGEX_CASES, indirect=["serena_agent"]
    )
    def test_find_declaration(
        self, serena_agent: SerenaAgent, case: RegexDefiningSymbolCase
    ) -> None:
        ...
        ), f"Expected defining symbol for regex {case.regex!r} in {case.relative_path}"
        assert defining_symbol.get("relative_path") is not None
        assert (
            case.expected_definition_file in defining_symbol["relative_path"]
        ), f"Expected defining symbol in {case.expected_definition_file!r}, got: {defining_symbol}"
        assert self._symbol_matches_expected_name(
            defining_symbol, case.expected_name
    ...
    def test_get_diagnostics_for_file(
        self, serena_agent: SerenaAgent, diagnostic_case: DiagnosticCase
    ) -> None:
        ...
        pos1 = find_identifier_pos(
            project_root / diagnostic_case.relative_path, diagnostic_case.symbol1_id_str
        )
        pos2 = find_identifier_pos(
            project_root / diagnostic_case.relative_path,
            cast(str, diagnostic_case.symbol2_id_str),
        )
    ...
    @pytest.mark.parametrize(
        "serena_agent,case", FIND_IMPLEMENTATION_CASES, indirect=["serena_agent"]
    )
    def test_find_symbol_implementations(
        self, serena_agent: SerenaAgent, case: FindImplementationCase
    ) -> None:
        agent = serena_agent
        find_symbol_tool = agent.get_tool(FindSymbolTool)
        result = find_symbol_tool.apply(
        ...
        assert (
            symbols
        ), f"Expected to find symbol {case.symbol_name} in {case.definition_file}"
        def_symbol = symbols[0]
        find_impl_tool = agent.get_tool(FindImplementationsTool)
        result = find_impl_tool.apply(
            name_path=def_symbol["name_path"],
            relative_path=def_symbol["relative_path"],
            include_info=True,
        )
        implementations = json.loads(result)
        assert any(
            case.implementation_file in implementation["relative_path"]
            and self._symbol_matches_expected_name(
                implementation, case.expected_symbol_name
            )
    ...
    def test_find_symbol_name_path(
        self, serena_agent: SerenaAgent, case: FindSymbolNamePathCase
    ) -> None:
        agent = serena_agent
    ...
    def test_find_symbol_name_path_no_match(
        self, serena_agent: SerenaAgent, case: FindSymbolNoMatchCase
    ) -> None:
        agent = serena_agent
        ...
        assert (
            not symbols
        ), f"Expected to find no symbols for {case.name_path}. Symbols found: {symbols}"
    ...
    def test_find_symbol_overloaded_function(
        self, serena_agent: SerenaAgent, case: FindSymbolOverloadedCase
    ) -> None:
        """
    ...
    def test_non_unique_symbol_reference_error(
        ...
        with pytest.raises(ValueError, match=case.expected_error_fragment):
            find_refs_tool.apply(
                name_path=case.name_path, relative_path=case.relative_path
            )

        replace_symbol_body_tool = serena_agent.get_tool(ReplaceSymbolBodyTool)
        with pytest.raises(ValueError, match=case.expected_error_fragment):
            replace_symbol_body_tool.apply(
                name_path=case.name_path, relative_path=case.relative_path, body=""
            )
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(
                Language.TYPESCRIPT,
                marks=get_pytest_markers(Language.TYPESCRIPT),
                id="typescript_backslashes",
            ),
        ],
        indirect=["serena_agent"],
    )
    @pytest.mark.parametrize(
        "mode", ["literal", "regex"], ids=["literal_mode", "regex_mode"]
    )
    def test_replace_content_with_backslashes(
        ...
        with project_file_modification_context(serena_agent, relative_path):
            ...
            assert result == SUCCESS_RESULT
            new_content = read_project_file(
                serena_agent.get_active_project(), relative_path
            )
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(
                Language.PYTHON,
                marks=get_pytest_markers(Language.PYTHON),
                id="python_services",
            ),
            pytest.param(
                Language.PYTHON_TY,
                marks=get_pytest_markers(Language.PYTHON_TY),
                id="python_ty_services",
            ),
        ],
        indirect=["serena_agent"],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(
                Language.PYTHON,
                marks=get_pytest_markers(Language.PYTHON),
                id="python_container_body",
            ),
            pytest.param(
                Language.PYTHON_TY,
                marks=get_pytest_markers(Language.PYTHON_TY),
                id="python_ty_container_body",
            ),
        ],
        indirect=["serena_agent"],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(
                Language.TYPESCRIPT,
                marks=get_pytest_markers(Language.TYPESCRIPT),
                id="typescript_ambiguous_regex",
            ),
        ],
        indirect=["serena_agent"],
    )
    def test_replace_content_regex_with_wildcard_ambiguous(
        self, serena_agent: SerenaAgent
    ):
        """
    ...
    def test_safe_delete_symbol_blocked_by_references(
        ...
        with project_file_modification_context(serena_agent, case.relative_path):
            ...
            result = safe_delete_tool.apply(
                name_path_pattern=case.name_path, relative_path=case.relative_path
            )
            assert (
                "Cannot delete" in result
            ), f"Expected deletion to be blocked due to existing references, but got: {result}"
            assert (
                "referenced in" in result
            ), f"Expected reference information in result, but got: {result}"
    ...
    def test_safe_delete_symbol_succeeds_when_no_references(
        ...
        with project_file_modification_context(serena_agent, case.relative_path):
            ...
            ), f"Expected successful deletion, but got: {result}"

            # verify the symbol was actually removed from the file
            file_content = read_project_file(
                serena_agent.get_active_project(), case.relative_path
            )
            assert (
                case.name_path not in file_content
...
class TestPromptProvision:
    ...
    @staticmethod
    def _assert_activation_message(
        result: str, project_name: str, present: bool
    ) -> None:
        ...
        if present:
            assert (
                match is not None
            ), f"Expected project activation message in result:\n{result}"
        else:
            assert (
                match is None
            ), f"Expected no project activation message in result:\n{result}"
    ...
    def test_initial_instructions_provide_project_activation_message_once_per_session(
        ...
        result1 = self._call_tool(
            serena_agent, InitialInstructionsTool, session_id=session1
        )
        self._assert_activation_message(result1, project_name, present=True)

        result2 = self._call_tool(
            serena_agent, InitialInstructionsTool, session_id=session2
        )
        self._assert_activation_message(result2, project_name, present=True)

        result3 = self._call_tool(
            serena_agent, InitialInstructionsTool, session_id=session1
        )
        self._assert_activation_message(result3, project_name, present=False)

    @pytest.mark.parametrize("serena_agent", [Language.PYTHON], indirect=True)
    def test_dynamically_activated_mode_is_provided_once_per_session(
        self, serena_agent: SerenaAgent
    ) -> None:
        """
        ...
        assert (
            expected_new_mode_message in result2
        ...
        assert (
            expected_new_mode_message not in result3
        ...
        result4 = self._call_tool(
            serena_agent, InitialInstructionsTool, session_id=session2
        )
        assert (
            expected_new_mode_message in result4
    ...
    @pytest.mark.parametrize("serena_agent", [Language.PYTHON], indirect=True)
    def test_activate_project_tool_always_returns_activation_message(
        self, serena_agent: SerenaAgent
    ) -> None:
        project_name = "test_repo_python"
        session = "session1"
        ...
        result1 = self._call_tool(
            serena_agent, ActivateProjectTool, project=project_name, session_id=session
        )
        self._assert_activation_message(result1, project_name, present=True)
