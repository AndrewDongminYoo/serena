...
class TestHookClientDetection:
    ...
    def test_claude_code_client(self, tmp_path: Path):
        ...
        with patch("sys.stdin", _make_stdin(stdin_data)), patch(
...
class TestPreToolUseRemindAboutSerenaHook:
    ...
    def test_grep_tool_detection_claude_code(self, tmp_path: Path):
        ...
        for name, expected in [
            ...
            with patch("sys.stdin", _make_stdin(_base_input(tool_name=name))), patch(
                "serena.hooks.serena_home_dir", str(tmp_path)
            ):
                hook = PreToolUseRemindAboutSerenaHook(HookClient.CLAUDE_CODE)
            assert (
                hook.is_grep_call() == expected
            ), f"is_grep_tool() wrong for {name} (claude-code)"
    ...
    def test_grep_tool_detection_non_claude_code(self, tmp_path: Path):
        ...
        for name, expected in [
            ...
            assert (
                hook.is_grep_call() == expected
            ), f"is_grep_tool() wrong for {name} (vscode)"
        ...
        for tool_name, tool_input, expected in cases:
            with (
                patch(
                    "sys.stdin",
                    _make_stdin(
                        _base_input(tool_name=tool_name, tool_input=tool_input)
                    ),
                ),
            ...
            assert (
                hook.is_grep_call() == expected
            ), f"is_grep_tool() wrong for {tool_name} / {tool_input}"
    ...
    def test_read_file_tool_detection_claude_code(self, tmp_path: Path):
        ...
        for name, expected in [
            ...
            ), f"is_read_file_tool() wrong for {name} (claude-code)"
    ...
    def test_read_file_tool_detection_non_claude_code(self, tmp_path: Path):
        ...
        for name, expected in cases:
            ...
            assert (
                hook.is_read_file_call() == expected
            ), f"is_read_file_tool() wrong for {name} (vscode)"
    ...
    def test_read_non_code_file_counts_as_file_read(self, tmp_path: Path):
        ...
        with patch("sys.stdin", _make_stdin(payload)), patch(
            "serena.hooks.serena_home_dir", str(tmp_path)
        ):
            hook = PreToolUseRemindAboutSerenaHook(HookClient.CLAUDE_CODE)
    ...
    def test_read_code_file_detection(self, tmp_path: Path):
        ...
        cases = [
            (
                ...
                _read_input(tool_name="read", file_path="README.md"),
                False,
            ),
            (
                HookClient.CLAUDE_CODE,
                _read_input(tool_name="read", file_path="src/foo.py"),
                True,
            ),
            (
                HookClient.CODEX,
            ...
            (
                ...
                _base_input(
                    "functions.shell_command",
                    tool_input={"command": "Get-Content src/foo.py"},
                ),
                True,
            ),
        ]
        for client, payload, expected in cases:
            ...
            assert (
                hook.is_read_code_file_call() == expected
            ), f"is_read_code_file_call() wrong for {client} / {payload}"
    ...
    def test_serena_tool_detection(self, tmp_path: Path):
        for name, expected in [
            ("mcp_serena_find_symbol", True),
            ("serena_overview", True),
            ("grep_search", False),
        ]:
            with patch("sys.stdin", _make_stdin(_base_input(tool_name=name))), patch(
                "serena.hooks.serena_home_dir", str(tmp_path)
                ...
                hook = PreToolUseRemindAboutSerenaHook(HookClient.CLAUDE_CODE)
            assert (
                hook.is_serena_tool() == expected
            ), f"is_serena_tool() wrong for {name}"
    ...
    def test_no_output_below_threshold(
        ...
        for _ in range(ToolUseCounter._GREP_USES_THRESHOLD - 1):
            with patch("sys.stdin", _make_stdin(_base_input("grep"))), patch(
                "serena.hooks.serena_home_dir", str(tmp_path)
            ):
                PreToolUseRemindAboutSerenaHook(HookClient.CLAUDE_CODE).execute()
        assert capsys.readouterr().out == ""

    def test_deny_output_after_threshold_greps_claude_code(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ):
    ...
    def test_deny_output_after_threshold_greps_vscode(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ):
        """After reaching the grep threshold, the hook should output a deny for VS Code."""
        for _ in range(ToolUseCounter._GREP_USES_THRESHOLD):
            with patch("sys.stdin", _make_stdin(_base_input("grep_search"))), patch(
                "serena.hooks.serena_home_dir", str(tmp_path)
            ):
    ...
    def test_deny_output_after_threshold_greps_codex_shell_command(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ):
        """After reaching the grep threshold, Codex ``functions.shell_command`` emits a deny."""
        payload = _base_input(
            "functions.shell_command", tool_input={"command": "rg -n foo README.md"}
        )
        for _ in range(ToolUseCounter._GREP_USES_THRESHOLD):
            with patch("sys.stdin", _make_stdin(payload)), patch(
                "serena.hooks.serena_home_dir", str(tmp_path)
    ...
    def test_deny_output_after_threshold_reads(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ):
        ...
        for _ in range(ToolUseCounter._READ_FILE_USES_THRESHOLD):
            with patch("sys.stdin", _make_stdin(_read_input("read"))), patch(
                "serena.hooks.serena_home_dir", str(tmp_path)
            ):
                PreToolUseRemindAboutSerenaHook(HookClient.CLAUDE_CODE).execute()
    ...
    def test_no_deny_after_threshold_get_content_markdown_codex(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ):
        """PowerShell ``Get-Content`` calls on markdown files do not count for the code-read reminder."""
        payload = _base_input(
            "functions.shell_command", tool_input={"command": "Get-Content README.md"}
        )
        for _ in range(ToolUseCounter._READ_FILE_USES_THRESHOLD):
            with patch("sys.stdin", _make_stdin(payload)), patch(
                "serena.hooks.serena_home_dir", str(tmp_path)
    ...
    def test_deny_output_after_threshold_get_content_code_file_codex(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ):
        """PowerShell ``Get-Content`` calls on source files count for the code-read reminder."""
        payload = _base_input(
            "functions.shell_command", tool_input={"command": "Get-Content src/foo.py"}
        )
        for _ in range(ToolUseCounter._READ_FILE_USES_THRESHOLD):
            with patch("sys.stdin", _make_stdin(payload)), patch(
                "serena.hooks.serena_home_dir", str(tmp_path)
    ...
    def test_serena_tool_resets_counters(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ):
        ...
        with patch(
            "sys.stdin", _make_stdin(_base_input("mcp_serena_find_symbol"))
        ), patch("serena.hooks.serena_home_dir", str(tmp_path)):
            PreToolUseRemindAboutSerenaHook(HookClient.CLAUDE_CODE).execute()
    ...
    def test_counter_resets_after_deny(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ):
    ...
    def test_rate_limit_gates_entire_hook_within_interval(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ):
        ...
        stub_for_path.session_persistence_dir = str(
            tmp_path / "hook_data" / _base_input()["session_id"]
        )
        ...
        ), "gated hook must not mutate the persisted counter"

    def test_rate_limit_allows_deny_after_interval(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ):
        ...
        counter.last_deny_timestamp -= timedelta(
            seconds=ToolUseCounter._MIN_DENY_INTERVAL_SECONDS + 1
        )
        ...
        assert (
            second_output
        ), "after the rate-limit window elapsed, a new burst should emit a deny"
        result = json.loads(second_output)
        assert result["hookSpecificOutput"]["permissionDecision"] == "deny"

    def test_non_symbolic_deny_emitted_when_combined_threshold_tripped(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ):
        ...
        with patch("sys.stdin", _make_stdin(_base_input("Edit"))), patch(
            "serena.hooks.serena_home_dir", str(tmp_path)
        ):
            PreToolUseRemindAboutSerenaHook(HookClient.CLAUDE_CODE).execute()
...
class TestToolUseCounter:
    ...
    def test_update_resets_grep_outside_period(self):
        counter = ToolUseCounter()
        now = datetime.now()
        counter.last_grep_use_timestamp = now - timedelta(
            seconds=ToolUseCounter._GREP_RESET_PERIOD_SECONDS + 1
        )
    ...
    def test_update_resets_read_file_outside_period(self):
        counter = ToolUseCounter()
        now = datetime.now()
        counter.last_read_file_use_timestamp = now - timedelta(
            seconds=ToolUseCounter._READ_FILE_RESET_PERIOD_SECONDS + 1
        )
    ...
    def test_load_returns_fresh_counter_on_missing_file(self, tmp_path: Path):
        hook_stub = type(
            "HookStub", (), {"session_persistence_dir": str(tmp_path / "nonexistent")}
        )()
        loaded = ToolUseCounter.load(hook_stub)  # type: ignore[arg-type]
...
class TestPreToolUseAutoApproveSerenaHook:
    ...
    def test_approves_serena_tool_in_accept_edits_mode(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ):
    ...
    def test_approves_serena_tool_in_auto_mode(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ):
    ...
    def test_accepts_camel_case_permission_mode(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ):
    ...
    def test_stays_silent_for_non_serena_tool(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ):
    ...
    def test_stays_silent_in_default_mode(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ):
    ...
    def test_stays_silent_in_plan_mode(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ):
    ...
    def test_stays_silent_in_bypass_permissions_mode(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ):
    ...
    def test_stays_silent_in_dont_ask_mode(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ):
    ...
    def test_stays_silent_when_permission_mode_missing(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ):
...
class TestSessionEndCleanupHook:
    def test_removes_session_dir(self, tmp_path: Path):
        ...
        (session_dir / "tool_use_counter.pkl").write_bytes(
            pickle.dumps(ToolUseCounter())
        )
...
class TestHookCli:
    ...
    def test_cleanup_command(self, tmp_path: Path):
        ...
        with patch("serena.hooks.serena_home_dir", str(tmp_path)):
            result = runner.invoke(
                hook_commands, ["cleanup", "--client", "claude-code"], input=stdin_json
            )
    ...
    def test_remind_command(self, tmp_path: Path):
        ...
        for _ in range(ToolUseCounter._GREP_USES_THRESHOLD):
            stdin_json = json.dumps(
                {"session_id": "cli-remind", "tool_name": "grep", "tool_input": {}}
            )
            with patch("serena.hooks.serena_home_dir", str(tmp_path)):
                result = runner.invoke(
                    hook_commands,
                    ["remind", "--client", "claude-code"],
                    input=stdin_json,
                )
            assert result.exit_code == 0
    ...
    def test_auto_approve_command(self, tmp_path: Path):
        ...
        with patch("serena.hooks.serena_home_dir", str(tmp_path)):
            result = runner.invoke(
                hook_commands,
                ["auto-approve", "--client", "claude-code"],
                input=stdin_json,
            )
        assert result.exit_code == 0
        output = json.loads(result.output)
    ...
    def test_invalid_client_rejected(self, tmp_path: Path):
        ...
        with patch("serena.hooks.serena_home_dir", str(tmp_path)):
            result = runner.invoke(
                hook_commands, ["activate", "--client", "invalid"], input=stdin_json
            )
        assert result.exit_code != 0
    ...
    def test_invalid_stdin_exits_nonzero(self, tmp_path: Path):
        ...
        with patch("serena.hooks.serena_home_dir", str(tmp_path)):
            result = runner.invoke(
                hook_commands, ["activate", "--client", "claude-code"], input="not json"
            )
        assert result.exit_code != 0
