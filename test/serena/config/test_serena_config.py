...
class TestProjectConfigAutogenerate:
    ...
    def test_autogenerate_empty_directory(self):
        ...
        config = ProjectConfig.autogenerate(
            self.project_path, self.serena_config, save_to_disk=False
        )
    ...
    def test_autogenerate_empty_directory_logs_warning(self, caplog):
        ...
        assert any(
            "No source files for supported language servers were found" in msg
    ...
    def test_autogenerate_saves_to_disk(self):
        ...
        config = ProjectConfig.autogenerate(
            self.project_path, self.serena_config, save_to_disk=True
        )
    ...
    def test_autogenerate_nonexistent_path(self):
        ...
        with pytest.raises(FileNotFoundError) as exc_info:
            ProjectConfig.autogenerate(
                non_existent, self.serena_config, save_to_disk=False
            )
...
class TestProjectConfig:
    def test_template_is_complete(self):
        ...
        assert (
            is_complete
        ), "Project template YAML is incomplete; all fields must be present (with descriptions)."
...
class TestEffectiveLanguageBackend:
    ...
    def test_default_backend_is_global(self):
        ...
        config, name = _make_config_with_project(
            "test_proj", language_backend=None, global_backend=LanguageBackend.LSP
        )
    ...
    def test_project_overrides_global_backend(self):
        ...
        config, name = _make_config_with_project(
            "test_jetbrains",
            language_backend=LanguageBackend.JETBRAINS,
            global_backend=LanguageBackend.LSP,
        )
        agent = SerenaAgent(project=name, serena_config=config)
        try:
    ...
    def test_activate_project_rejects_backend_mismatch(self):
        ...
        config, name = _make_config_with_project(
            "lsp_proj", language_backend=None, global_backend=LanguageBackend.LSP
        )
...
class TestGetConfiguredProjectSerenaFolder:
    ...
    def test_rejects_unknown_placeholder(self):
        ...
        with pytest.raises(
            SerenaConfigError, match=r"Unknown placeholder '\$unknownVar'"
        ):
    ...
    def test_rejects_typo_projectDirs(self):
        ...
        with pytest.raises(
            SerenaConfigError, match=r"Unknown placeholder '\$projectDirs'"
        ):
            config.get_configured_project_serena_folder("/home/user/myproject")
    ...
    def test_rejects_typo_projectfoldername_lowercase(self):
        ...
        with pytest.raises(
            SerenaConfigError, match=r"Unknown placeholder '\$projectfoldername'"
        ):
            config.get_configured_project_serena_folder("/home/user/myproject")
    ...
    def test_error_message_lists_supported_placeholders(self):
        ...
        with pytest.raises(
            SerenaConfigError,
            match=r"\$projectDir.*\$projectFolderName|\$projectFolderName.*\$projectDir",
        ):
            config.get_configured_project_serena_folder("/home/user/myproject")
...
class TestProjectSerenaDataFolder:
    ...
    def test_custom_location_creates_outside_project(self):
        ...
        config = SerenaConfig(
            gui_log_window=False,
            web_dashboard=False,
            project_serena_folder_location=str(custom_base)
            + "/$projectFolderName/.serena",
        )
        project = self._make_project(config)
...
class TestSerenaConfigFromConfigFileRobustness:
    ...
    def test_malformed_project_is_skipped_with_warning(self, caplog, monkeypatch):
        ...
        assert registered_roots == {
            good_project.resolve()
        }, f"Expected only the good project to be registered, got {registered_roots}"
        assert any(
            "Failed to load project configuration" in msg
            and str(bad_project.resolve()) in msg
            for msg in caplog.messages
