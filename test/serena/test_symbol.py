...
class TestSymbolNameMatching:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                "foo", ["foo"], False, True, id="'foo' matches 'foo' exactly (simple)"
            ),
            pytest.param(
                "foo/", ["foo"], False, True, id="'foo/' matches 'foo' exactly (simple)"
            ),
            pytest.param(
                ...
                False,
                True,
                id="'foo' matches ['bar', 'foo'] exactly (simple, last element)",
            ),
            pytest.param(
                "foo",
                ["foobar"],
                False,
                False,
                id="'foo' does not match 'foobar' exactly (simple)",
            ),
            pytest.param(
                "foo",
                ["bar", "foobar"],
                False,
                False,
                id="'foo' does not match ['bar', 'foobar'] exactly (simple, last element)",
            ),
            pytest.param(
                "foo",
                ["path", "to", "foo"],
                False,
                True,
                id="'foo' matches ['path', 'to', 'foo'] exactly (simple, last element)",
            ),
            ...
            pytest.param(
                ...
                False,
                True,
                id="'/foo' matches ['foo'] exactly (absolute simple)",
            ),
            pytest.param(
                "/foo",
                ["foo", "bar"],
                False,
                False,
                id="'/foo' does not match ['foo', 'bar'] (absolute simple, len mismatch)",
            ),
            pytest.param(
                "/foo",
                ["bar"],
                False,
                False,
                id="'/foo' does not match ['bar'] (absolute simple, name mismatch)",
            ),
            pytest.param(
                "/foo",
                ["bar", "foo"],
                False,
                False,
                id="'/foo' does not match ['bar', 'foo'] (absolute simple, position mismatch)",
            ),
            ...
            pytest.param(
                ...
                True,
                True,
                id="'foo' matches 'foobar' as substring (simple)",
            ),
            pytest.param(
                "foo",
                ["bar", "foobar"],
                True,
                True,
                id="'foo' matches ['bar', 'foobar'] as substring (simple, last element)",
            ),
            pytest.param(
                "foo",
                ["barfoo"],
                True,
                True,
                id="'foo' matches 'barfoo' as substring (simple)",
            ...
            pytest.param(
                ...
                True,
                False,
                id="'foo' does not match 'baz' as substring (simple)",
            ),
            pytest.param(
                "foo",
                ["bar", "baz"],
                True,
                False,
                id="'foo' does not match ['bar', 'baz'] as substring (simple, last element)",
            ),
            pytest.param(
                "foo",
                ["my_foobar_func"],
                True,
                True,
                id="'foo' matches 'my_foobar_func' as substring (simple)",
            ),
            pytest.param(
                "foo",
            ...
            pytest.param(
                ...
                True,
                False,
                id="'foo' does not match 'my_bar_func' as substring (simple)",
            ),
            ...
            pytest.param(
                ...
                True,
                True,
                id="'/foo' matches ['foobar'] as substring (absolute simple)",
            ),
            pytest.param(
                "/foo/",
                ["foobar"],
                True,
                True,
                id="'/foo/' matches ['foobar'] as substring (absolute simple, last element)",
            ),
            pytest.param(
                "/foo",
                ["barfoobaz"],
                True,
                True,
                id="'/foo' matches ['barfoobaz'] as substring (absolute simple)",
            ),
            pytest.param(
                "/foo",
                ["foo", "bar"],
                True,
                False,
                id="'/foo' does not match ['foo', 'bar'] as substring (absolute simple, len mismatch)",
            ),
            pytest.param(
                "/foo",
                ["bar"],
                True,
                False,
                id="'/foo' does not match ['bar'] (absolute simple, no substr)",
            ),
            pytest.param(
                "/foo",
                ["bar", "foo"],
    ...
    def test_match_simple_name(
        self, name_path_pattern, symbol_name_path_parts, is_substring_match, expected
    ):
        """Tests matching for simple names (no '/' in pattern)."""
        symbol_name_path_components = [
            NamePathComponent(part) for part in symbol_name_path_parts
        ]
        result = NamePathMatcher(
            name_path_pattern, is_substring_match
        ).matches_reversed_components(reversed(symbol_name_path_components))
        error_msg = self._create_assertion_error_message(
            name_path_pattern,
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                ...
                False,
                True,
                id="R: 'bar/foo' matches ['bar', 'foo'] exactly",
            ),
            pytest.param(
                "bar/foo",
                ["mod", "bar", "foo"],
                False,
                True,
                id="R: 'bar/foo' matches ['mod', 'bar', 'foo'] exactly (suffix)",
            ),
            pytest.param(
                "bar/foo",
                ["bar", "foo", "baz"],
                False,
                False,
                id="R: 'bar/foo' does not match ['bar', 'foo', 'baz'] (pattern shorter)",
            ),
            pytest.param(
                "bar/foo",
                ["bar"],
                False,
                False,
                id="R: 'bar/foo' does not match ['bar'] (pattern longer)",
            ),
            pytest.param(
                "bar/foo",
                ["baz", "foo"],
                False,
                False,
                id="R: 'bar/foo' does not match ['baz', 'foo'] (first part mismatch)",
            ),
            pytest.param(
                "bar/foo",
                ["bar", "baz"],
                False,
                False,
                id="R: 'bar/foo' does not match ['bar', 'baz'] (last part mismatch)",
            ),
            pytest.param(
                "bar/foo",
                ["foo"],
                False,
                False,
                id="R: 'bar/foo' does not match ['foo'] (pattern longer)",
            ),
            pytest.param(
                "bar/foo",
                ["other", "foo"],
                False,
                False,
                id="R: 'bar/foo' does not match ['other', 'foo'] (first part mismatch)",
            ),
            pytest.param(
                "bar/foo",
                ["bar", "otherfoo"],
                False,
                False,
                id="R: 'bar/foo' does not match ['bar', 'otherfoo'] (last part mismatch)",
            ),
            ...
            pytest.param(
                ...
                True,
                True,
                id="R: 'bar/foo' matches ['bar', 'foobar'] as substring",
            ),
            pytest.param(
                "bar/foo",
                ["mod", "bar", "foobar"],
                True,
                True,
                id="R: 'bar/foo' matches ['mod', 'bar', 'foobar'] as substring (suffix)",
            ),
            pytest.param(
                "bar/foo",
                ["bar", "bazfoo"],
                True,
                True,
                id="R: 'bar/foo' matches ['bar', 'bazfoo'] as substring",
            ),
            pytest.param(
                "bar/fo",
                ["bar", "foo"],
                True,
                True,
                id="R: 'bar/fo' matches ['bar', 'foo'] as substring",
            ),  # codespell:ignore
            pytest.param(
                "bar/foo",
                ["bar", "baz"],
                True,
                False,
                id="R: 'bar/foo' does not match ['bar', 'baz'] (last no substr)",
            ),
            pytest.param(
                "bar/foo",
                ["baz", "foobar"],
                True,
                False,
                id="R: 'bar/foo' does not match ['baz', 'foobar'] (first part mismatch)",
            ),
            pytest.param(
                "bar/foo",
                ["bar", "my_foobar_method"],
                True,
                True,
                id="R: 'bar/foo' matches ['bar', 'my_foobar_method'] as substring",
            ),
            pytest.param(
                "bar/foo",
            ...
            pytest.param(
                ...
                True,
                True,
                id="R: 'bar/f' matches ['bar', 'foo'] as substring",
            ),
            # Exact matches, absolute patterns (is_substring_match=False)
            pytest.param(
                "/bar/foo",
                ...
                False,
                True,
                id="A: '/bar/foo' matches ['bar', 'foo'] exactly",
            ),
            pytest.param(
                "/bar/foo",
                ["bar", "foo", "baz"],
                False,
                False,
                id="A: '/bar/foo' does not match ['bar', 'foo', 'baz'] (pattern shorter)",
            ),
            pytest.param(
                "/bar/foo",
                ["bar"],
                False,
                False,
                id="A: '/bar/foo' does not match ['bar'] (pattern longer)",
            ),
            pytest.param(
                "/bar/foo",
                ["baz", "foo"],
                False,
                False,
                id="A: '/bar/foo' does not match ['baz', 'foo'] (first part mismatch)",
            ),
            pytest.param(
                "/bar/foo",
                ["baz", "bar", "foo"],
            ...
            pytest.param(
                ...
                False,
                False,
                id="A: '/bar/foo' does not match ['bar', 'baz'] (last part mismatch)",
            ),
            ...
            pytest.param(
                ...
                True,
                True,
                id="A: '/bar/foo' matches ['bar', 'foobar'] as substring",
            ),
            pytest.param(
                "/bar/foo",
                ["bar", "bazfoo"],
                True,
                True,
                id="A: '/bar/foo' matches ['bar', 'bazfoo'] as substring",
            ),
            pytest.param(
                "/bar/fo",
                ["bar", "foo"],
                True,
                True,
                id="A: '/bar/fo' matches ['bar', 'foo'] as substring",
            ),  # codespell:ignore
            pytest.param(
                "/bar/foo",
                ["bar", "baz"],
                True,
                False,
                id="A: '/bar/foo' does not match ['bar', 'baz'] (last no substr)",
            ),
            pytest.param(
                "/bar/foo",
                ["baz", "foobar"],
                True,
                False,
                id="A: '/bar/foo' does not match ['baz', 'foobar'] (first part mismatch)",
            ),
        ],
    )
    def test_match_name_path_pattern_path_len_2(
        self, name_path_pattern, symbol_name_path_parts, is_substring_match, expected
    ):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                "bar/foo",
                [NamePathComponent("bar", 0), NamePathComponent("foo")],
                True,
                id="R: 'bar/foo' matches ['bar[0]', 'foo']",
            ),
            pytest.param(
                "bar/foo",
                [NamePathComponent("bar", 0), NamePathComponent("foo", 1)],
                True,
                id="R: 'bar/foo' matches ['bar[0]', 'foo[1]']",
            ),
            pytest.param(
                "bar[0]/foo",
                [NamePathComponent("bar", 0), NamePathComponent("foo")],
                True,
                id="R: 'bar[0]/foo' matches ['bar[0]', 'foo']",
            ),
            pytest.param(
                "bar[0]/foo[1]",
                [NamePathComponent("bar", 0), NamePathComponent("foo", 1)],
    ...
    def test_match_name_path_pattern_with_overload_idx(
        self, name_path_pattern, symbol_name_path_components, expected
    ):
        """Tests matching for qualified names (e.g. 'module/class/func')."""
        matcher = NamePathMatcher(name_path_pattern, False)
        result = matcher.matches_reversed_components(
            reversed(symbol_name_path_components)
        )
        error_msg = self._create_assertion_error_message(
            name_path_pattern, symbol_name_path_components, False, expected, result
        )
        assert result == expected, error_msg
...
class TestLanguageServerSymbolRetriever:
    ...
    def test_request_info(self, project_with_ls: Project):
        ...
        create_user_method_symbol = symbol_retriever.find(
            "UserService/create_user", within_relative_path="test_repo/services.py"
        )[0]
        create_user_method_symbol_info = symbol_retriever.request_info_for_symbol(
            create_user_method_symbol
        )
...
class TestSymbolDictTypes:
    ...
    def check_key_type(dict_type: type, key_type: type):
        ...
        for expected_key in key_type.__args__:  # type: ignore
            assert (
                expected_key in dict_type_keys
            ), f"Expected key '{expected_key}' not found in {dict_type}"

    def test_ls_symbol_dict_type(self):
        self.check_key_type(
            LanguageServerSymbol.OutputDict, LanguageServerSymbol.OutputDictKey
        )
...
def _make_mock_symbols(
    count: int, *, relative_path: str = "test_repo/services.py"
) -> list[MagicMock]:
...
class TestHoverBudget:
    ...
    @pytest.mark.parametrize("project_with_ls", PYTHON_BACKEND_LANGUAGES, indirect=True)
    def test_budget_not_exceeded_all_lookups_performed(
    ...
    @pytest.mark.parametrize("project_with_ls", PYTHON_BACKEND_LANGUAGES, indirect=True)
    def test_budget_exceeded_partial_info(
        self, project_with_ls: Project, monkeypatch: pytest.MonkeyPatch
    ):
    ...
    @pytest.mark.parametrize("project_with_ls", PYTHON_BACKEND_LANGUAGES, indirect=True)
    def test_project_budget_overrides_global(
        self, project_with_ls: Project, monkeypatch: pytest.MonkeyPatch
    ):
    ...
    @pytest.mark.parametrize("project_with_ls", PYTHON_BACKEND_LANGUAGES, indirect=True)
    def test_project_null_inherits_global(
        self, project_with_ls: Project, monkeypatch: pytest.MonkeyPatch
    ):
