...
SEPARATOR = "=" * 80
PYTHON_TEST_REPO = (
    Path(REPO_ROOT) / "test" / "resources" / "repos" / "python" / "test_repo"
)
SERVICES_FILE = Path("test_repo") / "services.py"


def make_agent(
    project_root: Path, language: Language, project_name: str
) -> SerenaAgent:
...
def find_identifier_occurrence_position(
    file_path: Path, identifier: str, occurrence_index: int = 0
) -> tuple[int, int]:
    ...
    raise ValueError(
        f"Could not find occurrence {occurrence_index} of {identifier!r} in {file_path}"
    )
