...
"""
    ...
    try:
        ...
        create_result = agent.execute_task(
            lambda: create_text_file_tool.apply(
                relative_path=relative_path, content=initial_content
            )
        )
        print(create_result)
        ...
        initial_diagnostics_result = agent.execute_task(
            lambda: get_diagnostics_for_file_tool.apply(
                relative_path=relative_path, min_severity=2
            )
        )
        ...
        assert "missing_one" in json.dumps(
            first_edit_diagnostics
        ), first_edit_diagnostics
        ...
        diagnostics_after_first_edit = parse_json_result(
            diagnostics_after_first_edit_result
        )
        assert "missing_one" in json.dumps(
            diagnostics_after_first_edit
        ), diagnostics_after_first_edit
        ...
        diagnostics_after_second_edit = parse_json_result(
            diagnostics_after_second_edit_result
        )
        diagnostics_after_second_edit_json = json.dumps(diagnostics_after_second_edit)
        assert (
            "missing_one" in diagnostics_after_second_edit_json
        ), diagnostics_after_second_edit
        assert (
            "missing_two" in diagnostics_after_second_edit_json
        ), diagnostics_after_second_edit
    finally:
