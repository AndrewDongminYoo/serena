...
_INIT_VERSION_PATTERN = re.compile(
    r'^(?P<before>__version__\s*=\s*")(?P<version>\d+\.\d+\.\d+)(?P<after>"\s*)$',
    re.MULTILINE,
)
...
@click.option(
    ...
    "major",
    is_flag=True,
    help="Bump the major version and reset minor and patch to 0.",
)
@click.option(
    "--minor",
    "minor",
    is_flag=True,
    help="Bump the minor version and reset patch to 0.",
)
...
@click.option(
    "--dry-run", is_flag=True, help="Show what would change without writing any files."
)
def bump_version(
    major: bool, minor: bool, patch: bool, target_version: str | None, dry_run: bool
) -> None:
    ...
    log.info(
        "bump_version called: major=%s, minor=%s, patch=%s, target_version=%s",
        major,
        minor,
        patch,
        target_version,
    )
    version_part = resolve_version_selection(
        major=major, minor=minor, patch=patch, target_version=target_version
    )
...
def resolve_version_selection(
    *, major: bool, minor: bool, patch: bool, target_version: str | None
) -> VersionPart | None:
    bump_flags_selected = sum([major, minor, patch])
    if target_version is not None and bump_flags_selected > 0:
        raise click.ClickException(
            "Use either --version or one of --major/--minor/--patch, not both."
        )
    ...
    if patch:
        return "patch"
    raise click.ClickException(
        "No version bump selected. Use --major, --minor, --patch or --version."
    )
...
def bump_repo_version(
    ...
    *,
    ...
    current_version = extract_version(
        pyproject_text, _PYPROJECT_VERSION_PATTERN, "pyproject.toml"
    )
    init_version = extract_version(
        init_text, _INIT_VERSION_PATTERN, "src/serena/__init__.py"
    )
    log.info(
        "pyproject.toml version: %s, __init__.py version: %s",
        current_version,
        init_version,
    )
    ...
    new_pyproject_text = replace_version(
        pyproject_text, _PYPROJECT_VERSION_PATTERN, new_version, "pyproject.toml"
    )
    new_init_text = replace_version(
        init_text, _INIT_VERSION_PATTERN, new_version, "src/serena/__init__.py"
    )
...
def replace_version(
    text: str, pattern: re.Pattern[str], new_version: str, file_label: str
) -> str:
    match = pattern.search(text)
    if match is None:
        raise click.ClickException(f"Could not update version in {file_label}.")
    return (
        f"{text[: match.start('version')]}{new_version}{text[match.end('version') :]}"
    )
...
def update_changelog(changelog_text: str, new_version: str) -> str:
    if not changelog_text.startswith(_UNRELEASED_HEADER):
        raise click.ClickException(
            "CHANGELOG.md must start with '# Unreleased (main)'."
        )

    next_header_index = changelog_text.find("\n# ", len(_UNRELEASED_HEADER))
    unreleased_section_end = (
        len(changelog_text) if next_header_index == -1 else next_header_index + 1
    )
...
def split_unreleased_body(unreleased_body: str) -> tuple[str, str]:
    ...
    if not seen_content:
        raise click.ClickException(
            "Could not determine the introduction paragraph in CHANGELOG.md."
        )
