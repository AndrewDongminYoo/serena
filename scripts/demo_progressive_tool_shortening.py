...
def run_with_shrinking(
    agent: SerenaAgent, label: str, fn, char_limits: list[int]
) -> None:
...
def run_lsp_tools(agent: SerenaAgent) -> None:
    ...
    run_with_shrinking(
        ...
        lambda lim: lsp_find.apply(
            "Tool", relative_path=REF_FILE, depth=1, max_answer_chars=lim
        ),
