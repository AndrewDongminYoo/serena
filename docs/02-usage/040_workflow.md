...
- the set of programming languages for which language servers are spawned (not relevant when using the JetBrains plugin)  
  Note that you can dynamically add/remove language servers while Serena is running via the [Dashboard](./060_dashboard.md).
...
- while in a conversation, by telling the LLM to activate a project, e.g.,
  ...
  which it isn't in single-project [contexts](./050_configuration.md#contexts) like `ide` or `claude-code` _if_ a project is provided at startup.
