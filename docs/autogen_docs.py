...
def index_template(
    package_name: str, doc_references: Optional[list[str]] = None, text_prefix=""
):
...
def make_rst(src_root, rst_root, clean=False, overwrite=False, package_prefix=""):
    ...
    for root, dirnames, filenames in os.walk(src_root):
        ...
        for dirname in dirnames:
            ...
            write_to_file(
                index_template(
                    package_qualname, doc_references=module_names + subdir_refs
                ),
                package_index_rst_path,
            )
        ...
        for filename in filenames:
            ...
            if ext == ".py" and not filename.startswith("_"):
                ...
                module_rst_path = os.path.join(
                    rst_root, base_package_relpath, f"{base_name}.rst"
                )
...
def autogen_tool_list(target_filename="01-about/035_tools.md"):
    ...
    with open(target_file, "w") as f:
        ...
        priority_modules = {
            "serena.tools.symbol_tools": 1,
            "serena.tools.jetbrains_tools": 2,
        }

        text = TextBuilder()
        sorted_modules = sorted(
            tools_by_module.keys(), key=lambda m: (priority_modules.get(m, 3), m)
        )
        for module in sorted_modules:
            ...
            for tool in tools:
                ...
                if tool.is_beta:
                    info += " [BETA]"
                text.with_line(
                    f"* `{tool.tool_name}`{info}: {tool.class_docstring}", indent=2
                )
        f.write(text.build())
...
def autogen_about_intro_features():
    ...
    match = re.search(
        r"<h3.*?>(.*?)</h3>(.*?)^## Programming.*?^(## Features.*?)^## ",
        readme_contents,
        re.DOTALL | re.MULTILINE,
    )
    assert (
        match
    ), f"Failed to extract about texts from README.md. {__file__} probably needs to be updated."
    ...
    with open(
        Path(__file__).parent / "01-about" / "000_intro.md", "w", encoding="utf-8"
    ) as f:
        ...
        about_text = re.subn(
            r"^.*\[Quick Start.*?$", r"", about_text, flags=re.MULTILINE
        )[0]

        # remove callouts
        about_text = re.subn(
            r"^> \[!\w+\]\s+(^>.*?$)+", "", about_text, flags=re.MULTILINE
        )[0]
    ...
    with open(
        Path(__file__).parent / "01-about" / "025_features.md", "w", encoding="utf-8"
    ) as f:
        f.write(autogen_info)
        features_text = re.subn(r"^#", r"", features_text, flags=re.MULTILINE)[0]
        features_text = re.subn(r"</?details>", "", features_text, flags=re.MULTILINE)[
            0
        ]
        features_text = re.subn(
            r"<summary>.*?</summary>", "", features_text, flags=re.MULTILINE
        )[0]
        features_text = re.sub(
            r'<a href="https://plugins.jetbrains.com.*?</a>',
            jetbrains_marketplace_link,
            features_text,
            flags=re.DOTALL,
        )
