...
class Language(str, Enum):
    ...
    def iter_all(
        ...
        include_non_programming_languages: bool = True,
    ) -> Iterable[Self]:
        for lang in cls:
    ...
    def is_programming_language(self) -> bool:
        ...
        """
        return self not in frozenset(
            (self.MARKDOWN, self.JSON, self.TOML, self.YAML, self.ANSIBLE)
        )
    ...
    def get_ls_class(self) -> type["SolidLanguageServer"]:
        match self:
            ...
            case self.CRYSTAL:
                from solidlsp.language_servers.crystal_language_server import (
                    CrystalLanguageServer,
                )
            ...
            case self.JSON:
                from solidlsp.language_servers.json_language_server import (
                    JsonLanguageServer,
                )
            ...
            case self.OCAML:
                from solidlsp.language_servers.ocaml_lsp_server import (
                    OcamlLanguageServer,
                )
            ...
            case self.HAXE:
                from solidlsp.language_servers.haxe_language_server import (
                    HaxeLanguageServer,
                )

                return HaxeLanguageServer
            case self.LEAN4:
                from solidlsp.language_servers.lean4_language_server import (
                    Lean4LanguageServer,
                )
            ...
            case self.HLSL:
                from solidlsp.language_servers.hlsl_language_server import (
                    HlslLanguageServer,
                )

                return HlslLanguageServer
            case self.SYSTEMVERILOG:
                from solidlsp.language_servers.systemverilog_server import (
                    SystemVerilogLanguageServer,
                )

                return SystemVerilogLanguageServer
            case self.SOLIDITY:
                from solidlsp.language_servers.solidity_language_server import (
                    SolidityLanguageServer,
                )

                return SolidityLanguageServer
            case self.ANSIBLE:
                from solidlsp.language_servers.ansible_language_server import (
                    AnsibleLanguageServer,
                )

                return AnsibleLanguageServer
            case self.MSL:
                from solidlsp.language_servers.msl_language_server import (
                    MslLanguageServer,
                )
            ...
            case self.SCSS:
                from solidlsp.language_servers.some_sass_language_server import (
                    SomeSassLanguageServer,
                )
