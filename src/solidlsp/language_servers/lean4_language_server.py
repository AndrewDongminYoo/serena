...
class Lean4LanguageServer(SolidLanguageServer):
    ...
    class DependencyProvider(LanguageServerDependencyProviderSinglePath):
        ...
        def create_launch_command_env(self) -> dict[str, str]:
            ...
            if lake_path is None:
                log.warning(
                    "lake not found on PATH; cross-file references may not work"
                )
                return env
            try:
                ...
                if result.returncode == 0:
                    ...
                    log.warning(
                        f"lake env failed (exit {result.returncode}): {result.stderr[:200]}"
                    )
    ...
    def _create_dependency_provider(self) -> LanguageServerDependencyProvider:
        return self.DependencyProvider(
            self._custom_settings, self._ls_resources_dir, self.repository_root_path
        )
