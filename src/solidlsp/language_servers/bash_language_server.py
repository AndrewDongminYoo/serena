...
_SHELLCHECK_RELEASE_BASE = (
    f"https://github.com/koalaman/shellcheck/releases/download/v{_SHELLCHECK_VERSION}"
)
...
class BashLanguageServer(SolidLanguageServer):
    ...
    class DependencyProvider(LanguageServerDependencyProviderSinglePath):
        def _get_or_install_core_dependency(self) -> str:
            ...
            bash_language_server_version = self._custom_settings.get(
                "bash_language_server_version", DEFAULT_BASH_LANGUAGE_SERVER_VERSION
            )
        ...
        def _install_shellcheck_if_missing(bash_ls_dir: str) -> None:
            ...
            log.info(
                f"Downloading ShellCheck v{_SHELLCHECK_VERSION} for {PlatformUtils.get_platform_id().value}"
            )
