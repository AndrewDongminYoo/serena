    ...
    * ``ansible_settings`` (dict) — full settings dict, deep-merged on top of defaults.
    ...
    class DependencyProvider(LanguageServerDependencyProviderSinglePath):
        def _get_or_install_core_dependency(self) -> str:
            ...
            ansible_language_server_version = self._custom_settings.get(
                ...
                DEFAULT_ANSIBLE_LANGUAGE_SERVER_VERSION,
            )
            ...
            ls_dirname = (
                "ansible-lsp"
                if ansible_language_server_version
                == INITIAL_ANSIBLE_LANGUAGE_SERVER_VERSION
                else f"ansible-lsp-{ansible_language_server_version}"
            )
            ansible_ls_dir = os.path.join(self._ls_resources_dir, ls_dirname)
            ansible_executable_path = os.path.join(
                ansible_ls_dir, "node_modules", ".bin", "ansible-language-server"
            )
            ...
            if not os.path.exists(ansible_executable_path):
                log.info(
                    f"Ansible Language Server executable not found at {ansible_executable_path}. Installing..."
                )
    ...
    def _get_initialize_params(self, repository_absolute_path: str) -> InitializeParams:
        ...
        ansible_settings: dict[str, Any] = {
            ...
            "python": {
                "interpreterPath": self._custom_settings.get(
                    "python_interpreter_path", "python3"
                ),
                "activationScript": self._custom_settings.get(
    ...
    def _start_server(self) -> None:
        ...
        def show_message_request_handler(params: Any) -> None:
            ...
            """
            log.info(
                f"LSP: window/showMessageRequest (dismissed): {params.get('message', params)}"
            )
            return
        ...
        log.debug(
            f"Ansible server capabilities: {list(init_response['capabilities'].keys())}"
        )
