...
INITIAL_VSHAXE_SHA256 = (
    "104d785e3f7b57a7f3debf520d9751f7e7abf3a7e78d203db1a8ff3dc7ca30e2"
)
DEFAULT_VSHAXE_VERSION = "2.34.2"
DEFAULT_VSHAXE_SHA256 = (
    "104d785e3f7b57a7f3debf520d9751f7e7abf3a7e78d203db1a8ff3dc7ca30e2"
)
...
class HaxeLanguageServer(SolidLanguageServer):
    ...
    class DependencyProvider(LanguageServerDependencyProviderSinglePath):
        ...
        def _get_or_install_core_dependency(self) -> str:
            ...
            if system_haxe_ls:
                log.info(
                    f"Found system-installed haxe-language-server at {system_haxe_ls}"
                )
            ...
            if vscode_server_path:
                log.info(
                    f"Found Haxe Language Server in VSCode extension at {vscode_server_path}"
                )
        ...
        def _find_vscode_extension_server() -> str | None:
            ...
            search_paths = [
                os.path.expanduser(
                    "~/.vscode/extensions/nadako.vshaxe-*/bin/server.js"
                ),
                os.path.expanduser(
                    "~/.vscode-server/extensions/nadako.vshaxe-*/bin/server.js"
                ),
                os.path.expanduser(
                    "~/.vscode-insiders/extensions/nadako.vshaxe-*/bin/server.js"
                ),
            ]
        ...
        def _download_from_open_vsx(cls, target_dir: str, version: str) -> str | None:
            ...
            try:
                ...
                log.info(
                    "Downloading Haxe Language Server v%s from Open VSX...", version
                )
                ...
                if expected_sha is not None:
                    ...
                    log.info(
                        "Using custom version %s — skipping SHA256 verification",
                        version,
                    )
                ...
                if os.path.exists(server_js_path):
                    log.info(
                        f"Haxe Language Server v{version} installed to {server_js_path}"
                    )
                    return server_js_path
        ...
        for root, dirs, files in os.walk(repository_absolute_path):
            ...
            for f in files:
                if f.endswith(".hxml") and "haxe_libraries" not in root:
                    hxml_path = os.path.relpath(
                        os.path.join(root, f), repository_absolute_path
                    )
                    log.info(
    ...
    def _start_server(self) -> None:
        ...
        def diagnostics_handler(params: dict) -> None:
            ...
            if errors:
                log.warning(
                    "Haxe LSP diagnostics for %s: %d errors: %s",
                    uri,
                    len(errors),
                    errors,
                )
                ...
                log.info(
                    "Haxe LSP diagnostics for %s: clean (%d total)", uri, len(diags)
                )

            with self._progress_lock:
                if not self._active_progress_tokens:
                    log.info(
                        "Haxe LSP: no active progress tokens — signalling compilation complete"
                    )
                    self._server_ready.set()
                else:
                    log.info(
    ...
    def request_hover(
        ...
        try:
            ...
            log.warning(
                "Hover request failed for %s:%d:%d",
                relative_file_path,
                line,
                column,
                exc_info=True,
