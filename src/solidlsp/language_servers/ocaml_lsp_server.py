...
class OcamlLanguageServer(SolidLanguageServer):
    ...
    def _ensure_ocaml_lsp_installed(repository_root_path: str) -> str:
        ...
        try:
            ...
            if (
                "ocaml-lsp-server" not in result.stdout
                or "# No matches found" in result.stdout
            ):
                raise RuntimeError(
            ...
            raise RuntimeError(
                f"Failed to check ocaml-lsp-server installation: {e.stderr}"
            )
        ...
        try:
            ...
            if not os.path.exists(executable_path):
                raise RuntimeError(
                    f"ocaml-lsp-server executable not found at {executable_path}"
                )
    ...
    def _build_ocaml_index_static(repository_root_path: str) -> bool:
        ...
        """
        log.info(
            "Building OCaml index for cross-file references (dune build @ocaml-index)..."
        )
        try:
            result = subprocess.run(
    ...
    def _start_server(self) -> None:
        ...
        def lang_status_handler(params: dict[str, Any]) -> None:
            if (
                ...
                and params.get("message") == "ServiceReady"
            ):
