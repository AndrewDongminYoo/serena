...
class PowerShellLanguageServer(SolidLanguageServer):
    ...
    def _get_pses_path(cls, solidlsp_settings: SolidLSPSettings) -> str | None:
        ...
        install_dir = _pses_install_dir(
            cls.ls_resources_dir(solidlsp_settings), pses_version
        )
        start_script = (
    ...
    def _setup_runtime_dependency(
        ...
        psscriptanalyzer_version = solidlsp_settings.get_ls_specific_settings(
            Language.POWERSHELL
        ).get("psscriptanalyzer_version", PSSCRIPTANALYZER_VERSION)
        psscriptanalyzer_path = (
            Path(bundled_modules_path) / "PSScriptAnalyzer" / psscriptanalyzer_version
        )
        if not psscriptanalyzer_path.exists():
            log.info(
                f"PSScriptAnalyzer {psscriptanalyzer_version} not found. Installing..."
            )
            subprocess.run(
                [
                    pwsh_path,
