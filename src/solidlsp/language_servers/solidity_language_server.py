...
class SolidityLanguageServer(SolidLanguageServer):
    ...
    class DependencyProvider(LanguageServerDependencyProviderSinglePath):
        def _get_or_install_core_dependency(self) -> str:
            ...
            forge_version = self._custom_settings.get(
            ...
            deps = RuntimeDependencyCollection(
                [
                    ...
                    RuntimeDependency(
                        ...
                        command=build_npm_install_command(
                            self._get_forge_npm_package(), forge_version, npm_registry
                        ),
                        platform_id="any",
                    ),
                ]
            ...
            solidity_ls_dir = self._resolve_solidity_ls_dir(
                solidity_language_server_version, forge_version
            )
            managed_bin_dir = os.path.join(solidity_ls_dir, "node_modules", ".bin")
            solidity_executable_path = os.path.join(
                managed_bin_dir, "nomicfoundation-solidity-language-server"
            )
            ...
            if not os.path.exists(solidity_executable_path) or not os.path.exists(
                forge_executable_path
            ):
                log.info("Solidity Language Server dependencies missing. Installing...")
                deps.install(solidity_ls_dir)
                log.info(
                    "Solidity language server dependencies installed successfully."
                )
        ...
        def _get_forge_npm_package() -> str:
            ...
            if machine in ("x86_64", "amd64"):
                ...
                raise RuntimeError(
                    f"Unsupported architecture for foundry forge: {machine}"
                )

            if os.name == "nt":
                if arch != "amd64":
                    raise RuntimeError(
                        f"foundry forge does not publish a Windows {arch} npm package"
                    )
        ...
        def _resolve_solidity_ls_dir(
            self, solidity_language_server_version: str, forge_version: str
        ) -> str:
            # legacy unversioned dir reserved for INITIAL pair; any other combination goes into a versioned subdir
            is_initial = (
                solidity_language_server_version
                == INITIAL_SOLIDITY_LANGUAGE_SERVER_VERSION
                and forge_version == INITIAL_FORGE_VERSION
            )
            ls_dirname = (
                "solidity-lsp"
                if is_initial
                else f"solidity-lsp-{solidity_language_server_version}-{forge_version}"
            )
    ...
    def _start_server(self) -> None:
        ...
        sol_files = glob.glob(
            os.path.join(self.repository_root_path, "**", "*.sol"), recursive=True
        )
        ...
        def on_file_indexed(params: Any) -> None:
            ...
            log.debug(
                f"Solidity LSP: file indexed ({indexed_count[0]}/{expected_count}): {uri}"
            )
        ...
        if expected_count > 0:
            log.info(
                f"Waiting for Solidity LSP to index {expected_count} .sol file(s)…"
            )
            completed = all_indexed.wait(timeout=60)
            if completed:
                log.info(
                    f"Solidity LSP indexing complete ({indexed_count[0]}/{expected_count} files indexed)"
                )
            else:
                log.warning(
                ...
                sleep(30)
                log.info(
                    f"Additional wait complete ({indexed_count[0]}/{expected_count} files indexed)"
                )
        else:
    ...
    def request_text_document_diagnostics(
        ...
        self._validate_text_document_diagnostics_request(
