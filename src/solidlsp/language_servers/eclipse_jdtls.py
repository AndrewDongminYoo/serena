...
class VsixResourcePaths:
    ...
    jre_version: (
        str  # e.g. "21.0.7"; expanded to extension/jre/{jre_version}-{platform-suffix}
    )
    lombok_jar_basename: str  # e.g. "lombok-1.18.36.jar"
    equinox_launcher_basename: (
        str  # e.g. "org.eclipse.equinox.launcher_1.7.0.v20250424-1814.jar"
    )
...
class EclipseJDTLS(SolidLanguageServer):
    ...
    class DependencyProvider(LanguageServerDependencyProvider):
        def __init__(
            ...
            self.runtime_dependency_paths = self._setup_runtime_dependencies(
                ls_resources_dir, custom_settings
            )
        ...
        def _setup_runtime_dependencies(
            ...
            * **Default vscode-java VSIX mode** (activated otherwise): downloads the platform-specific
            ...
            if jdtls_path or lombok_path:
                ...
                return EclipseJDTLS.DependencyProvider._setup_from_existing_install(
                    str(jdtls_path), str(lombok_path), custom_settings
                )

            platformId = PlatformUtils.get_platform_id()
            gradle_version = custom_settings.get("gradle_version", "8.14.2")
            vscode_java_version = custom_settings.get(
                "vscode_java_version", DEFAULT_VSCODE_JAVA_VERSION
            )
            vscode_java_tag = f"v{vscode_java_version.rsplit('-', 1)[0]}"
            intellicode_version = custom_settings.get(
                "intellicode_version", DEFAULT_INTELLICODE_VERSION
            )
            ...
            vscode_java_dirname = (
                ...
                else f"vscode-java-{vscode_java_version}"
            )
            intellicode_dirname = (
                "intellicode"
                if intellicode_version == INITIAL_INTELLICODE_VERSION
                else f"intellicode-{intellicode_version}"
            )
            ...
            if not os.path.exists(gradle_path):
                ...
                FileUtils.download_and_extract_archive_verified(
                    ...
                    allowed_hosts=cast(
                        tuple[str, ...], gradle_dependency["allowed_hosts"]
                    ),
                )
            ...
            vscode_java_path = str(
                PurePath(
                    ls_resources_dir, cast(str, dependency["relative_extraction_path"])
                )
            )
            os.makedirs(vscode_java_path, exist_ok=True)
            jre_home_path = str(
                PurePath(vscode_java_path, cast(str, dependency["jre_home_path"]))
            )
            jre_path = str(
                PurePath(vscode_java_path, cast(str, dependency["jre_path"]))
            )
            lombok_jar_path = str(
                PurePath(vscode_java_path, cast(str, dependency["lombok_jar_path"]))
            )
            jdtls_launcher_jar_path = str(
                PurePath(
                    vscode_java_path, cast(str, dependency["jdtls_launcher_jar_path"])
                )
            )
            jdtls_readonly_config_path = str(
                PurePath(
                    vscode_java_path,
                    cast(str, dependency["jdtls_readonly_config_path"]),
                )
            )
            if not all(
                [
            ...
            intellicode_jar_path = str(
                PurePath(
                    ...
                    cast(str, dependency["intellicode_jar_path"]),
                )
            )
            intellisense_members_path = str(
                PurePath(
                    intellicode_directory_path,
                    cast(str, dependency["intellisense_members_path"]),
                )
            )
            if not all(
                [
        ...
        def _setup_from_existing_install(
            ...
            launcher_jar = EclipseJDTLS.DependencyProvider._resolve_launcher_jar(
                plugins_dir
            )
        ...
        def _resolve_launcher_jar(plugins_dir: Path) -> Path:
            ...
            matches = sorted(
                p
                for p in plugins_dir.glob("org.eclipse.equinox.launcher_*.jar")
        ...
        def _resolve_system_jdk(
            ...
            if explicit_home := custom_settings.get("java_home"):
                ...
                if os.path.exists(candidate):
                    ...
                    log.warning(
                        f"JAVA_HOME='{env_home}' invalid (no '{candidate}'), falling back to PATH."
                    )
            ...
            real_jdk_home, major_version = (
                EclipseJDTLS.DependencyProvider._inspect_java(java_exe)
            )
            ...
            log.info(
                f"Resolved JDK {major_version} via {source}; java.home={real_jdk_home}; java_exe={java_exe}."
            )
        ...
        def _inspect_java(java_exe: str) -> tuple[str, int]:
            ...
            try:
                ...
                raise SolidLSPException(
                    f"Failed to run '{java_exe} -XshowSettings:properties -version': {exc}"
                ) from exc
        ...
        def _compute_workspace_hash(
            ...
            if custom_settings.get("jdtls_path"):
                ws_hash_input = (
                    repository_root_path + "|" + jdtls_launcher_jar_path
                ).encode()
            else:
        ...
        def create_launch_command_env(self) -> dict[str, str]:
            use_system_java_home = self._custom_settings.get(
                "use_system_java_home", False
            )
            if use_system_java_home:
                ...
                if system_java_home:
                    ...
                    log.warning(
                        "use_system_java_home is set but JAVA_HOME is not set in environment, falling back to bundled JRE"
                    )
    ...
    def _get_initialize_params(self, repository_absolute_path: str) -> InitializeParams:
        ...
        gradle_wrapper_enabled = self._custom_settings.get(
        ...
        if gradle_java_home is not None:
            ...
            log.info(
                f"Using bundled JRE for Gradle: {self.runtime_dependency_paths.jre_path}"
            )
        ...
        gradle_settings["java"] = {
            "home": (
                gradle_java_home
                if gradle_java_home is not None
                else self.runtime_dependency_paths.jre_path
            )
        }
