...
class LuaLanguageServer(SolidLanguageServer):
    ...
    def _get_lua_ls_path(
        ...
        if solidlsp_settings is not None:
            ...
            lua_ls_version = lua_settings.get(
                "lua_language_server_version", DEFAULT_LUA_LS_VERSION
            )
            ls_resource_dir = _lua_ls_install_dir(
                LuaLanguageServer.ls_resources_dir(solidlsp_settings), lua_ls_version
            )
    ...
    def _download_lua_ls(solidlsp_settings: SolidLSPSettings) -> str:
        ...
        install_dir = _lua_ls_install_dir(
            LuaLanguageServer.ls_resources_dir(solidlsp_settings), lua_ls_version
        )
