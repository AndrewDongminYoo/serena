...
def _get_taplo_download_url(version: str = DEFAULT_TAPLO_VERSION) -> tuple[str, str]:
    ...
    return (
        f"https://github.com/tamasfe/taplo/releases/download/{version}/{filename}",
        executable,
    )
...
class TaploServer(SolidLanguageServer):
    ...
    class DependencyProvider(LanguageServerDependencyProviderSinglePath):
        def _get_or_install_core_dependency(self) -> str:
            ...
            taplo_version = self._custom_settings.get(
                "taplo_version", DEFAULT_TAPLO_VERSION
            )
            ls_dirname = (
                "taplo"
                if taplo_version == INITIAL_TAPLO_VERSION
                else f"taplo-{taplo_version}"
            )
            ...
            if os.path.exists(taplo_executable) and os.access(
                taplo_executable, os.X_OK
            ):
        ...
        def _download_taplo(
            ...
            install_dir: str,
            executable_path: str,
            version: str = DEFAULT_TAPLO_VERSION,
        ) -> None:
            ...
            if expected_hash is None and version in (
                INITIAL_TAPLO_VERSION,
                DEFAULT_TAPLO_VERSION,
            ):
                raise RuntimeError(
                    f"No SHA256 checksum configured for Taplo archive: {archive_filename}"
                )
