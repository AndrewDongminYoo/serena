...
class PhpactorServer(SolidLanguageServer):
    ...
    class DependencyProvider(LanguageServerDependencyProviderSinglePath):
        def _get_or_install_core_dependency(self) -> str:
            ...
            """
            phpactor_version = self._custom_settings.get(
                "phpactor_version", DEFAULT_PHPACTOR_VERSION
            )
            ...
            result = subprocess.run(
                ["php", "--version"], capture_output=True, text=True, check=False
            )
            ...
            if version_match:
                ...
                if major < 8 or (major == 8 and minor < 1):
                    raise RuntimeError(
                        f"PHP {major}.{minor} detected, but Phpactor requires PHP 8.1+. Please upgrade PHP."
                    )
            else:
                log.warning(
                    "Could not parse PHP version from output. Continuing anyway."
                )
            ...
            if phpactor_version == INITIAL_PHPACTOR_VERSION:
                ...
                phar_dir = os.path.join(
                    self._ls_resources_dir, f"phpactor-{phpactor_version}"
                )
            ...
            assert os.path.exists(
                phpactor_phar_path
            ), f"phpactor PHAR not found at {phpactor_phar_path}, download may have failed."
    ...
    def __init__(
        ...
        log.info(
            f"Ignoring the following directories for PHP (Phpactor): {', '.join(sorted(self._ignored_dirnames))}"
        )
    ...
    def _start_server(self) -> None:
        ...
        assert init_response["capabilities"].get(
            "definitionProvider"
        ), "Phpactor did not advertise definition support"
