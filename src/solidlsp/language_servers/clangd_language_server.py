...
class ClangdLanguageServer(SolidLanguageServer):
    ...
    def _compile_commands_fingerprint(self) -> str | None:
        compile_db_path = os.path.join(
            self.repository_root_path, "compile_commands.json"
        )
    ...
    def _prepare_compile_commands(self) -> str | None:
        ...
        try:
            ...
            for entry in compile_commands:
                ...
                if directory and not os.path.isabs(directory):
                    ...
                    entry["directory"] = os.path.abspath(
                        os.path.join(self.repository_root_path, directory)
                    )
            ...
            cpp_settings: dict[str, Any] = self._custom_settings or {}
            compile_commands_rel_dir = cpp_settings.get(
                "compile_commands_dir", ".serena"
            )
            compile_commands_dir = os.path.join(
                self.repository_root_path, compile_commands_rel_dir
            )
            ...
            log.info(
                f"Created serena compilation database with absolute paths at {compile_commands_path}"
            )
            ...
            deps = RuntimeDependencyCollection(
                [
                    RuntimeDependency(
                        ...
                        sha256=(
                            "7c09614eff857d590e4502ef516f035ff94cfb8b795de14ece5afbc53a206caf"
                            if default_version
                            else None
                        ),
                    ...
                    RuntimeDependency(
                        ...
                        sha256=(
                            "5b6ceb0f85d63fa0c2c9aab31c29bebd41dc11da1f160ef21bc2fea93270a20d"
                            if default_version
                            else None
                        ),
                        allowed_hosts=CLANGD_ALLOWED_HOSTS,
                    ...
                    RuntimeDependency(
                        ...
                        binary_name=f"clangd_{clangd_version}/bin/clangd",
                        sha256=(
                            "d3b329b3f58602c57ca6501d255147af1bccad3691b1cb0c12c258fcd2da1be3"
                            if default_version
                            else None
                        ),
                        allowed_hosts=CLANGD_ALLOWED_HOSTS,
