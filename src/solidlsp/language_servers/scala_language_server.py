...
def _get_scala_settings(solidlsp_settings: SolidLSPSettings) -> dict[str, object]:
    ...
    try:
        ...
        log.warning(
            f"Invalid on_stale_lock value '{on_stale_lock_str}', using '{DEFAULT_ON_STALE_LOCK}'"
        )
        on_stale_lock = StaleLockMode.AUTO_CLEAN
    ...
    return {
        ...
        "log_multi_instance_notice": scala_settings.get(
            "log_multi_instance_notice", DEFAULT_LOG_MULTI_INSTANCE_NOTICE
        ),
    }
...
class ScalaLanguageServer(SolidLanguageServer):
    ...
    def _check_metals_db_status(
        self, repository_root_path: str, solidlsp_settings: SolidLSPSettings
    ) -> None:
        """
        ...
        if status == MetalsDbStatus.ACTIVE_INSTANCE:
            ...
            lock_path = (
                lock_info.lock_path
                if lock_info
                else project_path / ".metals" / "metals.mv.db.lock.db"
            )
