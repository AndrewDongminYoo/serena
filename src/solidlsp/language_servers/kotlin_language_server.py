...
class KotlinLanguageServer(SolidLanguageServer):
    ...
    class DependencyProvider(LanguageServerDependencyProviderSinglePath):
        ...
        def _get_or_install_core_dependency(self) -> str:
            ...
            assert (
                kotlin_suffix
            ), f"Unsupported platform for Kotlin LSP: {platform_id.value}"

            # Setup paths for dependencies; legacy unversioned dir reserved for INITIAL only
            kotlin_lsp_version = self._custom_settings.get(
                "kotlin_lsp_version", DEFAULT_KOTLIN_LSP_VERSION
            )
            ls_dirname = (
    ...
    def _start_server(self) -> None:
        ...
        if self._indexing_complete.wait(timeout=_INDEXING_TIMEOUT):
            ...
            log.warning(
                "Kotlin LSP did not signal indexing completion within %.0fs; proceeding anyway",
                _INDEXING_TIMEOUT,
            )
