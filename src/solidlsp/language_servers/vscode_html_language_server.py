...
class VsCodeHtmlLanguageServer(SolidLanguageServer):
    ...
    class DependencyProvider(LanguageServerDependencyProviderSinglePath):
        def _get_or_install_core_dependency(self) -> str:
            ...
            package_name = self._custom_settings.get(
                "vscode_langservers_package", DEFAULT_PACKAGE_NAME
            )
            package_version = self._custom_settings.get(
                "vscode_langservers_version", DEFAULT_PACKAGE_VERSION
            )
            ...
            executable_path = os.path.join(
                install_dir, "node_modules", ".bin", LS_BIN_NAME
            )
            if os.name == "nt":
                executable_path += ".cmd"
                    ...
                    [
                        RuntimeDependency(
                            ...
                            command=build_npm_install_command(
                                package_name, package_version, npm_registry
                            ),
                            platform_id="any",
                        ),
                    ]
    ...
    def _start_server(self) -> None:
        ...
        assert (
            "completionProvider" in init_response["capabilities"]
        ), "HTML LSP did not advertise completionProvider"
