...
class TerraformLS(SolidLanguageServer):
    ...
    def _setup_runtime_dependencies(cls, solidlsp_settings: SolidLSPSettings) -> str:
        ...
        terraform_settings = solidlsp_settings.get_ls_specific_settings(
            Language.TERRAFORM
        )
        terraform_ls_version = terraform_settings.get(
            "terraform_ls_version", DEFAULT_TERRAFORM_LS_VERSION
        )
        ...
        install_dir = (
            ...
            if terraform_ls_version == INITIAL_TERRAFORM_LS_VERSION
                ...
                cls.ls_resources_dir(solidlsp_settings),
                f"terraform-ls-{terraform_ls_version}",
            )
        )
