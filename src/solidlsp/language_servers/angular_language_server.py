...
from solidlsp.language_servers.vscode_html_language_server import (
...
class AngularLanguageServer(SolidLanguageServer):
    ...
    def __init__(
        ...
        (
            ng_executable,
            self._tsdk_path,
            self._ts_ls_executable,
            self._angular_plugin_path,
            self._install_dir,
    ...
    def _setup_runtime_dependencies(
        ...
        ls_version = ng_settings.get(
            "angular_language_server_version", DEFAULT_ANGULAR_LANGUAGE_SERVER_VERSION
        )
        svc_version = ng_settings.get(
            "angular_language_service_version", DEFAULT_ANGULAR_LANGUAGE_SERVICE_VERSION
        )
        ts_version = ng_settings.get(
            "typescript_version",
            ts_settings.get("typescript_version", DEFAULT_TYPESCRIPT_VERSION),
        )
        ...
        ls_dirname = (
            f"angular-lsp-{ls_version}-{svc_version}-{ts_version}-{tsls_version}"
        )
        ...
        angular_plugin_path = os.path.join(
            install_dir, "node_modules", "@angular", "language-service"
        )
                ...
                [
                    RuntimeDependency(
                        ...
                        command=build_npm_install_command(
                            "typescript", ts_version, npm_registry
                        ),
                        platform_id="any",
                    ),
                    RuntimeDependency(
                        ...
                        command=build_npm_install_command(
                            "@angular/language-service", svc_version, npm_registry
                        ),
                        platform_id="any",
                    ),
                    RuntimeDependency(
                        ...
                        command=build_npm_install_command(
                            "@angular/language-server", ls_version, npm_registry
                        ),
                        platform_id="any",
                    ),
                    RuntimeDependency(
                        ...
                        command=build_npm_install_command(
                            "typescript-language-server", tsls_version, npm_registry
                        ),
                        platform_id="any",
                    ),
                ]
        ...
        for path, label in (
            ...
            (
                os.path.join(tsdk_path, "tsserverlibrary.js"),
                "typescript/lib/tsserverlibrary.js",
            ),
        ):
            if not os.path.exists(path):
                raise FileNotFoundError(
                    f"Expected {label} at {path} after install, but it was not found."
                )
    ...
    def _start_typescript_server(self) -> None:
        try:
            ts_config = LanguageServerConfig(
                code_language=Language.TYPESCRIPT, trace_lsp_communication=False
            )
            ...
            if not self._ts_server.server_ready.wait(
                ...
                log.warning(
                    "Companion TS server ready timeout (%s s); proceeding anyway",
                    self.TS_SERVER_READY_TIMEOUT,
                )
    ...
    def _start_html_server(self) -> None:
        ...
        """
        try:
            html_config = LanguageServerConfig(
                code_language=Language.HTML, trace_lsp_communication=False
            )
            ...
            if not self._html_server.server_ready.wait(
                timeout=self.HTML_SERVER_READY_TIMEOUT
            ):
                log.warning(
                    "Companion HTML server ready timeout (%s s); proceeding anyway",
                    self.HTML_SERVER_READY_TIMEOUT,
                )
            ...
            log.exception(
                "Error starting companion HTML server; .html documentSymbol will return []"
            )
    ...
    def _find_angular_core_install(self) -> str | None:
        ...
        for parent in [cur, *cur.parents]:
            ...
            if start_dev is not None:
                try:
                    if parent.stat().st_dev != start_dev:
                        log.debug(
                            "Stopping @angular/core probe at %s after %d step(s) (mount-point change)",
                            parent,
                            steps,
                        )
                        break
            ...
            if candidate.exists():
                log.debug(
                    "Found @angular/core after %d step(s) at %s", steps, candidate
                )
                ...
                try:
                    with open(workspace_pkg, encoding="utf-8") as f:
                        if "workspaces" in json.load(f):
                            log.debug(
                                "Stopping @angular/core probe at workspace root %s after %d step(s)",
                                parent,
                                steps,
                            )
                            break
                    ...
                    log.debug(
                        "Could not parse %s as JSON dict (%s); ignoring as workspace marker",
                        workspace_pkg,
                        e,
                    )
        else:
            log.debug(
                "@angular/core probe walked %d ancestor(s) without finding an install or a workspace root",
                steps,
            )
        return None
    ...
    def _start_server(self) -> None:
        ...
        self.server.on_notification(
            "angular/projectLoadingFinish", angular_project_loading_finish
        )
        ...
        try:
            ...
            try:
                ...
                log.warning(
                    "Error stopping ngserver during startup-failure cleanup: %s", e
                )
            raise
    ...
    def _request_document_symbols(
        ...
        if self._ts_server is not None and self._is_typescript_file(relative_file_path):
            with self._ts_server.open_file(relative_file_path):
                return self._ts_server._request_document_symbols(
                    relative_file_path, file_data=None
                )
        ...
        if self._is_html_template_file(relative_file_path):
            if self._html_server is not None and self._html_server_started:
                with self._html_server.open_file(relative_file_path):
                    return self._html_server._request_document_symbols(
                        relative_file_path, file_data=None
                    )
            log.debug(
                "HTML companion unavailable for %s; returning None", relative_file_path
            )
            return None
    ...
    def request_definition(
        ...
        if self._ts_server is not None and self._is_typescript_file(relative_file_path):
            with self._ts_server.open_file(relative_file_path):
                return self._ts_server.request_definition(
    ...
    def request_rename_symbol_edit(
        ...
        return super().request_rename_symbol_edit(
            relative_file_path, line, column, new_name
        )
    ...
    def request_hover(
        ...
        if self._ts_server is not None and self._is_typescript_file(relative_file_path):
            with self._ts_server.open_file(relative_file_path):
                return self._ts_server.request_hover(
                    relative_file_path, line, column, file_buffer=file_buffer
                )
        return super().request_hover(
            relative_file_path, line, column, file_buffer=file_buffer
        )
    ...
    def request_implementation(
        ...
        if self._ts_server is not None and self._is_typescript_file(relative_file_path):
            with self._ts_server.open_file(relative_file_path):
                return self._ts_server.request_implementation(
                    relative_file_path, line, column
                )
        log.debug(
