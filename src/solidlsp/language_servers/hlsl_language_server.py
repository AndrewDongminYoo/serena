...
class HlslLanguageServer(SolidLanguageServer):
    ...
    class DependencyProvider(LanguageServerDependencyProviderSinglePath):
        def _get_or_install_core_dependency(self) -> str:
            ...
            if system_binary:
                log.info(
                    f"Using system-installed shader-language-server at {system_binary}"
                )
            ...
            deps = RuntimeDependencyCollection(
                [
                    RuntimeDependency(
                        ...
                        sha256=_hlsl_sha(
                            version, "shader-language-server-x86_64-pc-windows-msvc.zip"
                        ),
            ...
            ls_dirname = (
                ...
                if version == _INITIAL_VERSION
                else f"shader-language-server-{version}"
            )
            ...
            if not os.path.exists(executable_path):
                log.info(
                    f"shader-language-server not found. Downloading from {dep.url}"
                )
                _ = deps.install(install_dir)

            if not os.path.exists(executable_path):
                raise FileNotFoundError(
                    f"shader-language-server not found at {executable_path}"
                )
    ...
    def _start_server(self) -> None:
        ...
        def on_log_message(params: Any) -> None:
            message = (
                params.get("message", "") if isinstance(params, dict) else str(params)
            )
        ...
        ), "shader-language-server must support textDocumentSync"
        if "documentSymbolProvider" not in capabilities:
            log.warning(
                "shader-language-server does not advertise documentSymbolProvider"
            )
