...
class SystemVerilogLanguageServer(SolidLanguageServer):
    ...
    def __init__(
        ...
        super().__init__(
            config, repository_root_path, None, "systemverilog", solidlsp_settings
        )
    ...
    class DependencyProvider(LanguageServerDependencyProviderSinglePath):
        def _get_or_install_core_dependency(self) -> str:
            ...
            if system_verible:
                ...
                try:
                    ...
                    if result.returncode == 0:
                        ...
                        log.info(
                            f"Using system-installed verible-verilog-ls: {version_info}"
                        )
                    else:
                        log.info(
                            f"Using system-installed verible-verilog-ls at {system_verible}"
                        )
                except Exception:
                    log.info(
                        f"Using system-installed verible-verilog-ls at {system_verible}"
            ...
            verible_version = self._custom_settings.get(
            ...
            if not os.path.exists(executable_path):
                raise FileNotFoundError(
                    f"verible-verilog-ls not found at {executable_path}"
                )
    ...
    def _start_server(self) -> None:
        ...
        ), "verible-verilog-ls must support textDocumentSync"
