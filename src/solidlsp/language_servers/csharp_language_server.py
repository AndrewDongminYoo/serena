...
class CSharpLanguageServer(SolidLanguageServer):
    ...
    def request_hover(
        ...
        original_name = self._original_symbol_names.get(
    ...
    class DependencyProvider(LanguageServerDependencyProvider):
        ...
        def create_launch_command(self) -> list[str]:
            ...
            if solution_or_project:
                ...
                log.warning(
                    "No .sln/.slnx or .csproj file found, language server will attempt auto-discovery"
                )
        ...
        def _ensure_dotnet_runtime(self) -> str:
            ...
            return DotNETUtil(
                "10.0", allow_higher_version=True
            ).get_dotnet_path_or_raise()
        ...
        def _ensure_language_server(self, lang_server_dep: RuntimeDependency) -> str:
            ...
            log.info(
                f"Downloading {package_name} version {package_version} from NuGet.org..."
            )
        ...
        def _download_nuget_package(self, dependency: RuntimeDependency) -> Path:
            ...
            if url is None:
                raise SolidLSPException(
                    f"No URL specified for package {package_name} version {package_version}"
                )
            ...
            try:
                ...
                log.info(
                    f"Successfully downloaded and extracted {package_name} version {package_version} from NuGet.org"
                )
                return package_extract_dir

            except Exception as e:
                raise SolidLSPException(
                    f"Failed to download package {package_name} version {package_version} from NuGet.org: {e}"
                ) from e
