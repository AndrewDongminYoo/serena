...
class VueLanguageServer(SolidLanguageServer):
    ...
    def _warm_up_ls_operational_state(self) -> None:
        ...
        try:
            ...
            if not self.server_started:
                log.debug(
                    "Skipping Vue language server operational warm-up because the server is stopping"
                )
                return
            log.exception(
                "Error while warming up Vue language server operational state"
            )
        except Exception:
            log.exception(
                "Error while warming up Vue language server operational state"
    ...
    def _ensure_vue_files_indexed_on_ts_server(self) -> None:
        ...
        log.info(
            "Vue file indexing on TypeScript server complete, waiting for TS server to finish processing"
        )
    ...
    def _wait_for_ts_indexing_complete(self) -> None:
        ...
        if self._ts_server.wait_for_indexing(timeout=timeout):
            log.info(
                "TypeScript server finished indexing Vue files (signaled via $/progress)"
            )
        else:
            log.warning(
                f"Timeout ({timeout}s) waiting for TypeScript server to finish indexing Vue files, proceeding anyway"
            )
    ...
    def _forward_edit_to_ts_server_if_needed(
        self, relative_file_path: str, edit_fn: Callable[[], object]
    ) -> None:
        """
    ...
    def insert_text_at_position(
        ...
        """
        result = super().insert_text_at_position(
    ...
    def delete_text_between_positions(
        ...
        :return: deleted text
        """
        deleted_text = super().delete_text_between_positions(
    ...
    def _setup_runtime_dependencies(
        ...
        npm_registry = vue_config.get(
            "npm_registry", typescript_config.get("npm_registry")
        )
