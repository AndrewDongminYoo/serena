...
class LuauLanguageServer(SolidLanguageServer):
    ...
    class DependencyProvider(LanguageServerDependencyProviderSinglePath):
        ...
        def _download_luau_lsp(self) -> str:
            luau_lsp_version = self._custom_settings.get(
                "luau_lsp_version", DEFAULT_LUAU_LSP_VERSION
            )
        ...
        def _resolve_support_files(self) -> tuple[str | None, str | None]:
            ...
            security_level = LuauLanguageServer._get_roblox_security_level(
        ...
        def _download_roblox_support_files(
        ...
        def _download_auxiliary_file(
            path: Path, url: str, description: str
        ) -> str | None:
            if path.exists():
                return str(path)
            ...
            try:
                ...
                FileUtils.download_file_verified(
                    url, str(path), allowed_hosts=LUAU_DOCS_ALLOWED_HOSTS
                )
                return str(path)
    ...
    def _get_platform_type(custom_settings: SolidLSPSettings.CustomLSSettings) -> str:
        ...
        if platform_type not in SUPPORTED_PLATFORMS:
            raise ValueError(
                f"Unsupported Luau platform: {platform_type}. Expected one of: {', '.join(sorted(SUPPORTED_PLATFORMS))}"
            )
        return platform_type

    @staticmethod
    def _get_roblox_security_level(
    ...
    @classmethod
    def _get_workspace_configuration(
        cls, custom_settings: SolidLSPSettings.CustomLSSettings
    ...
    def _start_server(self) -> None:
        ...
        def window_log_message(msg: dict) -> None:
            ...
            if (
                "workspace ready" in message_text.lower()
                or "initialized" in message_text.lower()
            ):
        ...
        if self.server_ready.wait(timeout=5.0):
            ...
            log.warning(
                "Timeout waiting for Luau language server readiness, proceeding anyway"
            )
            self.server_ready.set()
