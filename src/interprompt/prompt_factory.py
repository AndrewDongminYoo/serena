...
class PromptFactoryBase:
    ...
    def __init__(
        ...
        self._prompt_collection = MultiLangPromptCollection(
            prompts_dir, fallback_mode=fallback_mode
        )
    ...
    def get_prompt_template(self, prompt_name: str) -> PromptTemplate:
        return self._prompt_collection.get_prompt_template(
            prompt_name, lang_code=self.lang_code
        )
...
def autogenerate_prompt_factory_module(
    prompts_dir: str, target_module_path: str
) -> None:
    """
...
"""
    ...
    for template_name in prompt_collection.get_prompt_template_names():
        template_parameters = prompt_collection.get_prompt_template_parameters(
            template_name
        )
        if len(template_parameters) == 0:
            method_params_str = ""
        else:
            method_params_str = ", *, " + ", ".join(
                [f"{param}: Any" for param in template_parameters]
            )
        generated_code += f"""
