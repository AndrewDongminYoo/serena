...
class DotNETUtil:
    def __init__(self, required_version: str, allow_higher_version: bool = True):
        ...
        self._required_version_components = [
            int(c) for c in required_version.split(".")
        ]
        ...
        if self._system_dotnet:
            try:
                result = subprocess.run(
                    [self._system_dotnet, "--list-runtimes"],
                    capture_output=True,
                    text=True,
                    check=True,
                )
                version_strings = re.findall(
                    r"Microsoft.NETCore.App\s+([^\s]+)", result.stdout
                )
                log.info("Installed .NET runtime versions: %s", version_strings)
                return [Version(v) for v in version_strings]
            except:
                log.warning(
                    "Failed to run 'dotnet --list-runtimes' to check .NET version; assuming no installed .NET versions"
                )
                return []
        else:
            log.info(
                "Found no `dotnet` on system PATH; assuming no installed .NET versions"
            )
            return []
    ...
    def is_required_version_available(self) -> bool:
        ...
        """
        required_version_str = ".".join(
            str(c) for c in self._required_version_components
        )
        for v in self._installed_versions:
            if self._allow_higher_version:
                if v.is_at_least(*self._required_version_components):
                    log.info(
                        f"Found installed .NET runtime version {v} which satisfies requirement of {required_version_str} or higher"
                    )
                    return True
            else:
                if v.is_equal(*self._required_version_components):
                    log.info(
                        f"Found installed .NET runtime version {v} which satisfies requirement of {required_version_str}"
                    )
                    return True
        return False
    ...
    def install_dotnet_with_script(version: str, base_path: str) -> str:
        ...
        log.info(
            f"Installing .NET {version} runtime using official Microsoft install script..."
        )
        ...
        try:
            ...
            if not dotnet_exe.exists():
                raise SolidLSPException(
                    f"dotnet executable not found at {dotnet_exe} after installation"
                )
