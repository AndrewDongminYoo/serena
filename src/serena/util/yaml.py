...
ITEM_COMMENT_INDEX_AFTER = (
    2  # (post-value; must be an instance of CommentToken at this index)
)
...
def load_yaml(
    ...
    if (
        commented_map is None
    ):  # ruamel returns None for empty documents, but we want an empty CommentedMap
        commented_map = CommentedMap()
    normalise_yaml_comments(commented_map, comment_normalisation)
    return commented_map


def normalise_yaml_comments(
    commented_map: CommentedMap, comment_normalisation: YamlCommentNormalisation
) -> None:
    """
            ...
            if len(comment_entry) == 0:
                ...
                if all(isinstance(item, CommentToken) for item in comment_entry):
                    ...
                    if not comment_str.startswith("\n"):
                        comment_str = "\n" + comment_str
                    return CommentToken(
                        value=comment_str, start_mark=start_mark, end_mark=None
                    )
                else:
                    types = set(type(item) for item in comment_entry)
                    log.warning(
                        "Unhandled types in list-valued comment entry: %s; not updating entry",
                        types,
                    )
                    return None
        else:
    ...
    match comment_normalisation:
        ...
        case (
            ...
            for i, key in enumerate(keys):
                ...
                if current_comment[ITEM_COMMENT_INDEX_BEFORE] is None:
                    if (
                        i == 0
                        and doc_comment is not None
                        and doc_comment[DOC_COMMENT_INDEX_PRE] is not None
                    ):
                        # move document pre-comment to leading comment of first key
                        current_comment[ITEM_COMMENT_INDEX_BEFORE] = make_list(
                            doc_comment[DOC_COMMENT_INDEX_PRE]
                        )
                        doc_comment[DOC_COMMENT_INDEX_PRE] = None
                    elif (
                        preceding_comment is not None
                        and preceding_comment[ITEM_COMMENT_INDEX_AFTER] is not None
                    ):
                        # move trailing comment of preceding key to leading comment of current key
                        current_comment[ITEM_COMMENT_INDEX_BEFORE] = (
                            trailing_to_leading(
        ...
        case _:
            raise ValueError(
                f"Unhandled comment normalisation: {comment_normalisation}"
            )


def save_yaml(
    path: str, data: dict | CommentedMap, preserve_comments: bool = True
) -> None:
    ...
    for key in target.keys():
        if key in source:
            ...
            for index in indices:
                if is_forced_update or yaml_comment_entry_is_empty(
                    target_comment[index]
                ):
    ...
    match comment_normalisation:
        ...
        case (
            ...
            transfer_yaml_comments_by_index(
                ...
                forced_update_keys=forced_update_keys,
