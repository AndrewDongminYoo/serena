...
class WebViewWithTray:
    ...
    def _monitor_parent_process(self) -> None:
        ...
        log.info(
            "Starting parent process monitor thread (parent_pid=%d, pid=%s)",
            pid,
            os.getpid(),
        )
        try:
        ...
        log.info(
            "Parent process (pid=%d) has exited, shutting down dashboard viewer", pid
        )
        self._terminate()
    ...
    def _start_tray(self) -> None:
        ...
        self._tray_icon = pystray.Icon(
            "dashboard_viewer", icon_img, self._title, menu, **kwargs
        )
