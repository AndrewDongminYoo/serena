...
class PublishedDiagnosticsSnapshot:
    def __init__(
        ...
        for edited_file_path in edited_file_paths:
            ...
            generation_by_after_path[edited_file_path.after_relative_path] = (
                language_server.get_published_diagnostics_generation(
                    edited_file_path.after_relative_path
                )
            )

            cached_diagnostics = (
                language_server.get_cached_published_text_document_diagnostics(
            ...
            if cached_diagnostics is None:
                try:
                    cached_diagnostics = (
                        language_server.request_text_document_diagnostics(
                            edited_file_path.before_relative_path,
                            min_severity=2,
                        )
                    )
            ...
            warning_identities_by_before_path[edited_file_path.before_relative_path] = {
                DiagnosticIdentity.from_diagnostic(diagnostic)
                for diagnostic in cached_diagnostics or []
            }
...
class GroupedDiagnostics:
    def __init__(self) -> None:
        self._grouped_diagnostics: dict[
            str, dict[str, dict[str, list[dict[str, Any]]]]
        ] = {}

    def add(
        self, relative_path: str, name_path: str, diagnostic: ls_types.Diagnostic
    ) -> None:
        severity_name = self._diagnostic_severity_name(diagnostic.get("severity"))
        self._grouped_diagnostics.setdefault(relative_path, {}).setdefault(
            severity_name, {}
        ).setdefault(name_path, []).append(self._diagnostic_output_dict(diagnostic))
        ...
        for edited_file_path in edited_files:
            ...
            published_diagnostics = (
                language_server.request_published_text_document_diagnostics(
                    ...
                    after_generation=before_snapshot.generation_by_after_path.get(
                        edited_file_path.after_relative_path, -1
                    ),
                    timeout=2.5,
                    min_severity=2,
                    allow_cached=True,
            ...
            existing_warning_identities = (
                before_snapshot.warning_identities_by_before_path.get(
            ...
            for diagnostic in published_diagnostics:
                ...
                if (
                    diagnostic_identity in existing_warning_identities
                    or diagnostic_identity in new_warning_identities
                ):
                    continue
                ...
                name_path = (
                    owner_symbol.get_name_path()
                    if owner_symbol is not None
                    else "<file>"
                )
                grouped_diagnostics.add(
                    edited_file_path.after_relative_path, name_path, diagnostic
                )
