        ...
        for i, line in enumerate(lines):
            if compiled_pattern.search(line):
                ...
                for j in range(context_start, context_end + 1):
                    ...
                    context_lines.append(
                        TextLine(
                            line_number=j, line_content=lines[j], match_type=match_type
                        )
                    )
...
class ContentReplacer:
    ...
    def __init__(
        self, mode: Literal["literal", "regex"], allow_multiple_occurrences: bool
    ):
        """
    ...
    def _create_replacement_function(
        regex_pattern: str, repl_template: str, regex_flags: int
        ...
        def validate_and_replace(match: re.Match) -> str:
            ...
            if "\n" in matched_text and re.search(
                regex_pattern, matched_text[1:], flags=regex_flags
            ):
                raise ValueError(
    ...
    def replace(
        ...
        if self.mode == "literal":
            ...
            raise ValueError(
                f"Invalid mode: '{self.mode}', expected 'literal' or 'regex'."
            )

        regex_flags = re.DOTALL | re.MULTILINE

        # create replacement function with validation and backreference handling
        repl_fn = self._create_replacement_function(
            regex, repl, regex_flags=regex_flags
        )
...
def find_text_coordinates(
    content: str, regex: str, require_unique: bool = False
) -> TextCoords | None:
    """
    ...
    if len(matches) == 0:
        ...
        if require_unique and len(matches) > 1:
            raise ValueError(
                f"Match must be unique; found {len(matches)} matches for regex: {regex}"
            )
        match = matches[0]
