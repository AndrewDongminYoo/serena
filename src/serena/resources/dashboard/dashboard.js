...
class BannerRotation {
  ...
  randomizeInitialBanner(type) {
    const slideClass =
      type === "platinum" ? ".platinum-banner-slide" : ".gold-banner-slide";
  ...
  bindArrowButtons() {
    ...
    $(".banner-arrow").on("click", function (e) {
      ...
      const direction = $(this).hasClass("banner-arrow-right")
        ? "next"
        : "prev";
      if (target === "platinum") {
...
class Dashboard {
  constructor() {
    ...
    this.$cancelExecutionCancelBtn.click(
      this.closeCancelExecutionModal.bind(this),
    );
    this.$modalCloseCancelExecution.click(
      this.closeCancelExecutionModal.bind(this),
    );
    this.$editSerenaConfigSaveBtn.click(
      this.saveSerenaConfigFromModal.bind(this),
    );
    this.$editSerenaConfigCancelBtn.click(
      this.closeEditSerenaConfigModal.bind(this),
    );
    this.$modalCloseEditSerenaConfig.click(
      this.closeEditSerenaConfigModal.bind(this),
    );
  ...
  loadConfigOverview() {
    if (this.waitingForConfigPollingResult) {
      console.log(
        "Still waiting for previous config poll result, skipping this poll",
      );
      return;
    }
    ...
    $.ajax({
      ...
      error: function (xhr, status, error) {
        ...
        self.$configDisplay.html(
          '<div class="error-message">Error loading configuration</div>',
        );
        self.$basicStatsDisplay.html(
          '<div class="error-message">Error loading stats</div>',
        );
        self.$projectsDisplay.html(
          '<div class="error-message">Error loading projects</div>',
        );
        self.$availableToolsDisplay.html(
          '<div class="error-message">Error loading tools</div>',
        );
        self.$availableModesDisplay.html(
          '<div class="error-message">Error loading modes</div>',
        );
  ...
  startConfigPolling() {
    ...
    this.configPollInterval = setInterval(
      this.loadConfigOverview.bind(this),
      1000,
    );
  }
  ...
  displayConfig(config) {
    try {
      ...
      if (this.jetbrainsMode) {
        ...
        if (config.languages && config.languages.length > 0) {
          ...
          config.languages.forEach(function (language, index) {
            ...
            if (isRemovable) {
              html +=
                '<span class="language-remove" data-language="' +
                language +
                '">&times;</span>';
            }
            html += "</div>";
          ...
          if (config.active_project && config.active_project.name) {
            ...
            if (this.isAddingLanguage) {
              html +=
                '<div id="add-language-spinner" class="language-spinner">';
            } else {
              html +=
      ...
      html +=
        '<div class="config-value">' + (config.encoding || "N/A") + "</div>";
      ...
      config.active_tools.forEach(function (tool) {
        html +=
          '<div class="tool-item" title="' + tool + '">' + tool + "</div>";
      });
      html += "</div>";
      html += "</div>";
      ...
      if (config.active_project && config.active_project.name) {
        ...
        const memoryCount =
          (config.available_memories && config.available_memories.length) || 0;
        html += "<span>Available Memories (" + memoryCount + ")</span>";
        html +=
          '<span class="toggle-icon' +
          (wasMemoriesExpanded ? " expanded" : "") +
          '">▼</span>';
        html += "</h3>";
        html +=
        ...
        if (config.available_memories && config.available_memories.length > 0) {
          config.available_memories.forEach(function (memory) {
            html +=
              '<div class="memory-item removable" data-memory="' +
              memory +
              '">';
            html += memory;
            html +=
              '<span class="memory-remove" data-memory="' +
              memory +
              '">&times;</span>';
            html += "</div>";
          });
        ...
        html +=
          '<button id="create-memory-btn" class="memory-add-btn">+ Add Memory</button>';
        html += "</div>";
        html += "</div>";
      }
      ...
      $("#edit-serena-config-btn").click(
        this.openEditSerenaConfigModal.bind(this),
      );
      ...
      this.$configDisplay.html(
        '<div class="error-message">Error displaying configuration: ' +
          error.message +
          "</div>",
      );
    }
  ...
  displayBasicStats(stats) {
    ...
    const maxCalls = Math.max(
      ...sortedTools.map((tool) => stats[tool].num_calls),
    );
    ...
    sortedTools.forEach(function (toolName) {
      ...
      html +=
        '<div class="stat-tool-name" title="' +
        toolName +
        '">' +
        toolName +
        "</div>";
  ...
  displayProjects(projects) {
    if (!projects || projects.length === 0) {
      this.$projectsDisplay.html(
        '<div class="no-stats-message">No projects registered.</div>',
      );
      return;
    }
  ...
  displayAvailableTools(tools) {
    if (!tools || tools.length === 0) {
      this.$availableToolsDisplay.html(
        '<div class="no-stats-message">All tools are active.</div>',
      );
      return;
    }
  ...
  displayAvailableModes(modes) {
    if (!modes || modes.length === 0) {
      this.$availableModesDisplay.html(
        '<div class="no-stats-message">No modes available.</div>',
      );
      return;
    }
  ...
  loadLastExecution(onComplete) {
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.status === "success") {
          if (
            response.last_execution !== null &&
            response.last_execution.logged
          ) {
  ...
  loadExecutions() {
    ...
    if (this.waitingForExecutionsPollingResult) {
      console.log(
        "Still waiting for previous executions poll result, skipping this poll",
      );
    } else {
  ...
  displayActiveExecutionsQueue(executions) {
    ...
    executions.forEach(function (execution) {
      ...
      if (isRunning) {
        ...
        html +=
          '<div class="execution-meta">queued · #' +
          execution.task_id +
          "</div>";
      }
  ...
  displayLastExecution(execution) {
    if (!execution) {
      this.$lastExecutionDisplay.html(
        '<div class="no-stats-message">No executions yet.</div>',
      );
      return;
    }
    ...
    html +=
      '<div class="last-execution-status">' +
      (isSuccess ? "Succeeded" : "Failed") +
      "</div>";
    html +=
      '<div class="last-execution-name">' +
  ...
  displayCancelledExecutions() {
    ...
    cancelledExecs.forEach(function (execution) {
      ...
      html +=
        ...
        (isAbandoned ? "abandoned" : "cancelled") +
        '">';
      html +=
  ...
  cancelExecution(executionData) {
    ...
    console.log(
      "cancelExecution called with full execution data:",
      executionData,
    );
      ...
      success: function (response) {
        ...
        if (response.status === "success") {
          if (response.was_cancelled) {
            console.log(
              "Task " + executionData.task_id + " was successfully cancelled",
            );
            // Add to cancelled list (only managed in JS, not persisted)
            const alreadyCancelled = self.cancelledExecutions.some(
              function (exec) {
            ...
            if (!alreadyCancelled) {
              ...
              console.log(
                "Cancelled executions array now contains:",
                self.cancelledExecutions,
              );
            } else {
  ...
  updateTitle(activeProject) {
    document.title = activeProject
      ? `${activeProject} – Serena Dashboard`
      : "Serena Dashboard";
  }
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (response.messages && response.messages.length > 0) {
          ...
          $("#log-container").html(
            '<div class="loading">No log messages found.</div>',
          );
        }

        self.updateLogButtons(
  ...
  loadStats() {
    ...
    $.when(
      ...
      .done(function (statsResp, estimatorResp) {
        ...
        const tokenCountEstimatorName =
          estimatorResp[0].token_count_estimator_name;
  ...
  displayStats(stats, tokenCountEstimatorName) {
    ...
    $("#estimator-name").html(
      `<strong>Token count estimator:</strong> ${tokenCountEstimatorName}`,
    );
    ...
    const totalTokens = names.map(
      (n) => stats[n].input_tokens + stats[n].output_tokens,
    );

    // Calculate totals for summary table
    const totalCalls = counts.reduce((sum, count) => sum + count, 0);
    const totalInputTokens = inputTokens.reduce(
      (sum, tokens) => sum + tokens,
      0,
    );
    const totalOutputTokens = outputTokens.reduce(
      (sum, tokens) => sum + tokens,
      0,
    );
  ...
  updateChartsTheme() {
    ...
    if (this.tokensChart && this.tokensChart.options.scales) {
      ...
      if (
        this.tokensChart.options.plugins &&
        this.tokensChart.options.plugins.legend
      ) {
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.status === "success") {
          ...
          alert(
            "Error removing language " + language + ": " + response.message,
          );
        }
      },
      error: function (xhr, status, error) {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (languages.length === 0) {
          ...
          self.$modalLanguageSelect.append(
          ...
          languages.forEach(function (language) {
            self.$modalLanguageSelect.append(
              $("<option>").val(language).text(language),
            );
          });
  ...
  startMemoryRename() {
    ...
    this.$editMemoryRenameInput
      .val(this.currentMemoryName)
      .show()
      .focus()
      .select();
  }
  ...
  displayNews(newsIds, newsData) {
    ...
    newsIds.forEach(function (newsId) {
      ...
      let $newsContainer = $('<div class="news-container">').attr(
        "data-news-id",
        newsId,
      );
  ...
  openEditSerenaConfigModal() {
    ...
    this.$editSerenaConfigContent.off("input").on("input", function () {
      ...
      self.serenaConfigContentDirty =
        currentContent !== self.originalSerenaConfigContent;
    });
  }
  ...
  shutdown() {
    ...
    const _shutdown = function () {
      ...
      self.$errorContainer.html(
        '<div class="error-message">Shutting down ...</div>',
      );
      setTimeout(function () {
        window.close();
      }, 1000);
