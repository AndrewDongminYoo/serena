...
class ListQueryableProjectsTool(
    Tool, ToolMarkerOptional, ToolMarkerDoesNotRequireActiveProject
):
    """
    ...
    def apply(self, symbol_access: bool = True) -> str:
        ...
        if symbol_access:
            ...
            if backend.is_jetbrains():
                ...
                matched_clients = JetBrainsPluginClientManager().match_clients(
                    registered_projects
                )
...
class QueryProjectTool(Tool, ToolMarkerOptional, ToolMarkerDoesNotRequireActiveProject):
    ...
    def apply(self, project_name: str, tool_name: str, tool_params_json: str) -> str:
        ...
        assert (
            tool.is_readonly()
        ), f"Tool {tool_name} is not read-only and cannot be executed in another project."
        if self._is_project_server_required(tool):
            ...
            registered_project = self.agent.serena_config.get_registered_project(
                project_name
            )
            assert (
                registered_project is not None
            ), f"Project {project_name} is not registered and cannot be queried."
