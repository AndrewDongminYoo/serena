...
class JetBrainsFindSymbolTool(Tool, ToolMarkerSymbolicRead, ToolMarkerOptional):
    ...
    def apply(
        ...
        return self._limit_length(
            result,
            max_answer_chars,
            shortened_result_factories=[create_shortened_result],
        )


class JetBrainsMoveTool(
    Tool, ToolMarkerSymbolicEdit, ToolMarkerOptional, ToolMarkerBeta
):
    """
...
class JetBrainsFindReferencingSymbolsTool(
    Tool, ToolMarkerSymbolicRead, ToolMarkerOptional
):
    """
    ...
    symbol_dict_grouper = JetBrainsSymbolDictGrouper(
        ["relative_path", "type"], ["type"], collapse_singleton=True
    )
    ...
    def apply(
        ...
        for symbol_dict in symbol_dicts:
            if "reference_line_no" in symbol_dict:
                ...
                if (
                    not SymbolDTOUtil.is_external_symbol(symbol_dict)
        ...
        return self._limit_length(
            ...
            shortened_result_factories=[
                create_shortened_result_counts_per_file,
                create_shortened_result_num_results,
            ],
        )
...
class JetBrainsGetSymbolsOverviewTool(Tool, ToolMarkerSymbolicRead, ToolMarkerOptional):
    ...
    symbol_dict_grouper = JetBrainsSymbolDictGrouper(
        ["type"], ["type"], collapse_singleton=True, map_name_path_to_name=True
    )
    ...
    def apply(
        ...
        with JetBrainsPluginClient.from_project(self.project) as client:
            symbol_overview = client.get_symbols_overview(
                relative_path=relative_path,
                depth=depth,
                include_file_documentation=include_file_documentation,
            )
        ...
        if self.USE_COMPACT_FORMAT:
            ...
            if documentation:
                ...
                shortened_result_factories.append(
                    lambda: self._to_json(grouped_symbols)
                )  # shortened result without docstring
            ...
            if depth > 0:
                ...
                def create_short_result_depth_0() -> str:
                    ...
                    for d in depth_0_symbols:
                        d.pop("children", None)
                    compact_depth_0_result = self.symbol_dict_grouper.group(
                        depth_0_symbols
                    )
        ...
        return self._limit_length(
            json_result,
            max_answer_chars,
            shortened_result_factories=shortened_result_factories,
        )
...
class JetBrainsTypeHierarchyTool(Tool, ToolMarkerSymbolicRead, ToolMarkerOptional):
    ...
    @staticmethod
    def _transform_hierarchy_nodes(
        nodes: list[jb.TypeHierarchyNodeDTO] | None,
    ) -> dict[str, list]:
        """
    ...
    def apply(
        ...
        with JetBrainsPluginClient.from_project(self.project) as client:
            ...
            if hierarchy_type in ("super", "both"):
                ...
                supertypes = self._transform_hierarchy_nodes(
                    supertypes_response.get("hierarchy")
                )
            ...
            if hierarchy_type in ("sub", "both"):
                ...
                if "num_levels_not_included" in subtypes_response:
                    levels_not_included["subtypes"] = subtypes_response[
                        "num_levels_not_included"
                    ]
                subtypes = self._transform_hierarchy_nodes(
                    subtypes_response.get("hierarchy")
                )
...
class JetBrainsFindImplementationsTool(
    Tool, ToolMarkerSymbolicRead, ToolMarkerOptional
):
    """
