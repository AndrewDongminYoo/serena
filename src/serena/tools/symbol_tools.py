...
class GetSymbolsOverviewTool(Tool, ToolMarkerSymbolicRead):
    ...
    symbol_dict_grouper = LanguageServerSymbolDictGrouper(
        ["kind"], ["kind"], collapse_singleton=True
    )

    def apply(
        self, relative_path: str, depth: int = 0, max_answer_chars: int = -1
    ) -> str:
        """
...
class FindSymbolTool(Tool, ToolMarkerSymbolicRead):
    ...
    def apply(
        ...
        def create_short_result_relative_path_to_name_paths() -> str:
            ...
            for s in symbols:
                relative_path_to_name_paths[
                    s.location.relative_path or "unknown"
                ].append(s.get_name_path())
...
class FindReferencingSymbolsTool(Tool, ToolMarkerSymbolicRead):
    ...
    symbol_dict_grouper = LanguageServerSymbolDictGrouper(
        ["relative_path", "kind"], ["kind"], collapse_singleton=True
    )
    ...
    def apply(
        ...
        return self._limit_length(
            result_json, max_answer_chars, shortened_result_factories=shortened_results
        )
...
class FindImplementationsTool(Tool, ToolMarkerSymbolicRead):
    ...
    def apply(
        ...
        if include_info:
            info_by_symbol = symbol_retriever.request_info_for_symbol_batch(
...
class FindDeclarationTool(Tool, ToolMarkerSymbolicRead):
    ...
    def apply(
        ...
        if not containing_symbol_name_path:
            ...
            symbol = symbol_retriever.find_unique(
                name_path_pattern=containing_symbol_name_path,
                within_relative_path=relative_path,
            )
...
class RenameSymbolTool(Tool, ToolMarkerSymbolicEdit):
    ...
    def apply(
        ...
        status_message = code_editor.rename_symbol(
            name_path, relative_path=relative_path, new_name=new_name
        )
...
class SafeDeleteSymbol(Tool, ToolMarkerSymbolicEdit):
    def apply(
        ...
        symbol = ls_symbol_retriever.find_unique(
            name_path_pattern,
        ...
        assert (
            symbol_rel_path is not None
        ), f"Symbol {name_path_pattern} has no relative path, this is likely a bug."
        assert (
            symbol_rel_path == relative_path
        ), f"Symbol {name_path_pattern} is not in the expected relative path {relative_path}."
        symbol_name_path = symbol.get_name_path()

        symbol_line = symbol.line
        symbol_col = symbol.column
        assert (
            symbol_line is not None and symbol_col is not None
        ...
        references_locations = lang_server.request_references(
            symbol_rel_path, symbol_line, symbol_col
        )
        ...
        if references_locations:
            for ref_loc in references_locations:
                ...
                if ref_relative_path is None:
                    continue
                file_to_lines[ref_relative_path].append(
                    ref_loc["range"]["start"]["line"]
                )
        if file_to_lines:
