...
from serena.util.ls_diagnostics import (
    DiagnosticsDiff,
    EditedFilePath,
    PublishedDiagnosticsSnapshot,
)
...
class Component(ABC):
    ...
    def create_language_server_symbol_retriever(
        ...
        assert (
            self.agent.get_language_backend().is_lsp()
        ), "Language server symbol retriever can only be created for LSP language backend"
    ...
    def create_ls_code_editor(self) -> "LanguageServerCodeEditor":
        ...
        if not self.agent.is_using_language_server():
            raise Exception(
                "Cannot create LanguageServerCodeEditor; agent is not in language server mode."
            )
...
class Tool(Component):
    ...
    def get_apply_fn(self) -> ApplyMethodProtocol:
        ...
        if apply_fn is None:
            raise RuntimeError(
                f"apply not defined in {self}. Did you forget to implement it?"
            )
        return apply_fn
    ...
    def get_apply_docstring_from_cls(cls) -> str:
        ...
        if not docstring:
            raise AttributeError(
                f"apply method has no (or empty) docstring in {cls}. Did you forget to implement it?"
            )
        return docstring.strip()
    ...
    def get_apply_fn_metadata_from_cls(cls) -> FuncMetadata:
        ...
        return func_metadata(
            apply_fn, skip_names=["self", "cls", cls.SESSION_ID_PARAM_NAME]
        )
    ...
    def _log_tool_application(self, frame: Any, session_id: str) -> None:
        ...
        for param, value in frame.f_locals.items():
            ...
            if param == "kwargs":
                ...
                params[param] = value
        log.info(
            f"{self.get_name_from_cls()}: {dict_string(params)}; session_id: {session_id}"
        )
    ...
    def _limit_length(
        ...
        if max_answer_chars <= 0:
            raise ValueError(
                f"Must be positive or the default (-1), got: {max_answer_chars=}"
            )
    ...
    def is_symbolic(self) -> bool:
        return issubclass(self.__class__, ToolMarkerSymbolicRead) or issubclass(
            self.__class__, ToolMarkerSymbolicEdit
        )
    ...
    def apply_ex(self, log_call: bool = True, catch_exceptions: bool = True, mcp_ctx: Context | None = None, **kwargs) -> str:  # type: ignore
            ...
            try:
                ...
                if client_params is not None:
                    ...
                    client_str = (
                        ...
                        else client_info.name + " " + client_info.version
                    )
                ...
                try:
                    ...
                    if e.is_language_server_terminated():
                        ...
                        if affected_language is not None:
                            ...
                            self.agent.get_language_server_manager_or_raise().restart_language_server(
                                affected_language
                            )
                            result = apply_fn(**apply_kwargs)
                        else:
        ...
        try:
            ...
            Exception
        ) as e:  # typically TimeoutError (other exceptions caught in task)
...
class EditingToolWithDiagnostics(Tool, ToolMarkerCanEdit):
    ...
    class DiagnosticsContext:
        def __init__(
            self, tool: "EditingToolWithDiagnostics", *edited_relative_paths: str
        ) -> None:
            self._tool = tool
            self._is_diagnostics_enabled = (
                tool.ENABLE_DIAGNOSTICS and tool.agent.is_using_language_server()
            )
            self._edited_files = [
                EditedFilePath(path, path) for path in edited_relative_paths
            ]
            self._before_edit_diagnostics_snapshot: (
            ...
            if self._is_diagnostics_enabled:
                ...
                self._before_edit_diagnostics_snapshot = PublishedDiagnosticsSnapshot(
                    self._edited_files, self._symbol_retriever
                )
        ...
        def format_result(
            ...
            diagnostics_diff = DiagnosticsDiff(
                self._before_edit_diagnostics_snapshot,
                self._edited_files,
                self._symbol_retriever,
            )
...
class ToolRegistry:
    ...
    def __init__(self) -> None:
        ...
        inclusion_predicate = (
            lambda c: "apply" in c.__dict__
        )  # include only concrete tool classes that implement apply
        for cls in iter_subclasses(Tool, inclusion_predicate=inclusion_predicate):
            ...
            if name in self._tool_dict:
                raise ValueError(
                    f"Duplicate tool name found: {name}. Tool classes must have unique names."
                )
    ...
    def get_registered_tools_by_module(self) -> dict[str, list[RegisteredTool]]:
        ...
        for module in sorted(module_dict.keys()):
            sorted_module_dict[module] = sorted(
                module_dict[module], key=lambda t: t.tool_name
            )
    ...
    def print_tool_overview(
        self,
        tools: Iterable[type[Tool] | Tool] | None = None,
    ...
    def check_valid_tool_name(
        self, tool_name: str, caller_context_for_logging: str = ""
    ) -> bool:
        ...
        if self.is_deleted_tool_name(tool_name):
            log.warning(
                f"Tool name is deleted: {tool_name}{caller_context_for_logging}"
            )
            return False
        if not self.is_valid_tool_name(tool_name):
            raise ValueError(
                f"Invalid tool name: {tool_name}{caller_context_for_logging}"
            )
        return True
