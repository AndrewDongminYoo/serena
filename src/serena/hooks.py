...
class PreToolUseHook(Hook, ABC):
    def __init__(self, client: HookClient):
        ...
        _tool_name = (
            self._input_data.get("tool_name")
            or self._input_data.get("toolName", "")
            or ""
        )
        ...
        self._tool_input: dict | None = self._input_data.get(
            "tool_input"
        ) or self._input_data.get("toolInput")

        # only relevant in claude code at the moment, (not all events include this field; default to empty string)
        raw_permission_mode = (
            self._input_data.get("permission_mode")
            or self._input_data.get("permissionMode")
            or ""
        )
...
class PreToolUseRemindAboutSerenaHook(PreToolUseHook):
    ...
    * or the configured reset period elapses *between two consecutive calls of that
    ...
    class ToolUseCounter:
        ...
        def is_hook_active(self, now: datetime) -> bool:
            ...
            if self.last_deny_timestamp is None:
                return True
            return (
                now - self.last_deny_timestamp
            ).total_seconds() >= self._MIN_DENY_INTERVAL_SECONDS
        ...
        def update(self, hook: "PreToolUseRemindAboutSerenaHook") -> None:
            ...
            if is_grep:
                ...
                if (
                    self.last_grep_use_timestamp is not None
                    and (now - self.last_grep_use_timestamp).total_seconds()
                    <= grep_period
                ):
            ...
            if is_code_file_read:
                ...
                if (
                    ...
                    and (now - self.last_read_file_use_timestamp).total_seconds()
                    <= read_period
                ):
            ...
            if is_grep or is_code_file_read:
                ...
                if (
                    ...
                    and (now - self.last_non_symbolic_use_timestamp).total_seconds()
                    <= combined_period
                ):
    ...
    _READ_FILE_VERB_SUBSTRINGS: frozenset[str] = frozenset(
        ("read", "view", "open", "show")
    )
    ...
    #: shell-command tool calls whose ``cmd`` or ``command`` field starts with one of these.
    _READ_SHELL_COMMANDS: frozenset[str] = frozenset(
    ...
    def __init__(self, client: HookClient):
        ...
        if self._tool_input is not None:
            self._command = self._tool_input.get(
    ...
    def is_read_code_file_call(self) -> bool:
        ...
        if self._client == HookClient.CODEX and self._command_args_str is not None:
            return any(
                self._is_code_file_path(argument)
                for argument in self._iter_shell_path_arguments()
            )
...
class HookCommands(AutoRegisteringGroup):
    def __init__(self) -> None:
        super().__init__(
            name="serena-hook",
            help="Commands that send reminders to agents when appropriate, to be used in hooks.",
        )
    ...
    @click.command(
        "cleanup",
        help="Set this as hook at session end all hook data for the current session",
    )
    @_client_option
