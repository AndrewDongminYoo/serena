...
class TopLevelCommands(AutoRegisteringGroup):
    ...
    @click.command(
        "start-mcp-server",
        help="Starts the Serena MCP server.",
        context_settings={"max_content_width": _MAX_CONTENT_WIDTH},
    )
    @click.option(
        "--project",
        ...
        type=PROJECT_TYPE,
        default=None,
        help="Path or name of project to activate at startup.",
    )
    @click.option(
        "--project-file",
        "project",
    ...
    @click.argument(
        "project_file_arg", type=PROJECT_TYPE, required=False, default=None, metavar=""
    )
    @click.option(
        "--context",
        type=str,
    ...
    @click.option(
        ...
        default=8000,
        show_default=True,
        help="Listen port for the MCP server (when using corresponding transport).",
    )
    @click.option(
    ...
    def start_mcp_server(
        ...
        if project_from_cwd:
            if project is not None or project_file_arg is not None:
                raise click.UsageError(
                    "--project-from-cwd cannot be used with --project or positional project argument"
                )
            ...
            if project is not None:
                ...
                log.warning(
                    "No project root found from %s; not activating any project",
                    os.getcwd(),
                )
        ...
        if default_modes or added_modes:
            mode_selection_def = ModeSelectionDefinitionWithAddedModes(
                default_modes=default_modes or None, added_modes=added_modes or None
            )

        factory = SerenaMCPFactory(
            context=context, project=project_file, memory_log_handler=memory_log_handler
        )
        server = factory.create_mcp_server(
            ...
            language_backend=(
                LanguageBackend.from_str(language_backend) if language_backend else None
            ),
    ...
    @click.command(
        "print-system-prompt",
        help="Print the system prompt for a project.",
        context_settings={"max_content_width": _MAX_CONTENT_WIDTH},
    )
    @click.argument(
        "project", type=click.Path(exists=True), default=os.getcwd(), required=False
    )
    @click.option(
        "--log-level",
        type=click.Choice(["DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"]),
    ...
    @click.option(
        "--only-instructions",
        is_flag=True,
        help="Print only the initial instructions, without prefix/postfix.",
    )
    @click.option(
        "--context",
        type=str,
    ...
    @click.option(
        "--width", type=int, default=1400, show_default=True, help="Window width."
    )
    @click.option(
        "--height", type=int, default=900, show_default=True, help="Window height."
    )
...
class ModeCommands(AutoRegisteringGroup):
    ...
    @click.command(
        "create",
        help="Create a new mode or copy an internal one.",
        context_settings={"max_content_width": _MAX_CONTENT_WIDTH},
    )
    @click.option(
        "--name",
    ...
    @click.option(
        ...
        type=str,
        default=None,
        help="Copy from an internal mode.",
    )
    ...
    @click.command(
        "edit",
        help="Edit a custom mode YAML file.",
        context_settings={"max_content_width": _MAX_CONTENT_WIDTH},
    )
    ...
    def edit(mode_name: str) -> None:
        ...
        if not os.path.exists(path):
            if mode_name in SerenaAgentMode.list_registered_mode_names(
                include_user_modes=False
            ):
                click.echo(
    ...
    @click.command(
        "delete",
        help="Delete a custom mode file.",
        context_settings={"max_content_width": _MAX_CONTENT_WIDTH},
    )
    @click.argument("mode_name")
...
class ContextCommands(AutoRegisteringGroup):
    ...
    def __init__(self) -> None:
        super().__init__(
            name="context",
            help="Manage Serena contexts. You can run `context <command> --help` for more info on each command.",
        )
    ...
    @click.command(
        "create",
        help="Create a new context or copy an internal one.",
        context_settings={"max_content_width": _MAX_CONTENT_WIDTH},
    )
    @click.option(
        "--name",
    ...
    @click.option(
        ...
        type=str,
        default=None,
        help="Copy from an internal context.",
    )
    def create(name: str, from_internal: str) -> None:
        if not (name or from_internal):
            raise click.UsageError("Provide at least one of --name or --from-internal.")
    ...
    @click.command(
        "edit",
        help="Edit a custom context YAML file.",
        context_settings={"max_content_width": _MAX_CONTENT_WIDTH},
    )
    ...
    def edit(context_name: str) -> None:
        ...
        if not os.path.exists(path):
            if context_name in SerenaAgentContext.list_registered_context_names(
                include_user_contexts=False
            ):
                click.echo(
    ...
    @click.command(
        "delete",
        help="Delete a custom context file.",
        context_settings={"max_content_width": _MAX_CONTENT_WIDTH},
    )
    @click.argument("context_name")
...
class ProjectCommands(AutoRegisteringGroup):
    ...
    def __init__(self) -> None:
        super().__init__(
            name="project",
            help="Manage Serena projects. You can run `project <command> --help` for more info on each command.",
        )

    @staticmethod
    def _create_project(
        project_path: str, name: str | None, language: tuple[str, ...]
    ) -> RegisteredProject:
        """
        ...
        if language:
            for lang in language:
                try:
                    ...
                    raise ValueError(
                        f"Unknown language '{lang}'. Supported: {all_langs}"
                    )
        ...
        languages_str = (
            ", ".join([lang.value for lang in generated_conf.languages])
            if generated_conf.languages
            else "N/A"
        )
        click.echo(
            f"Generated project with languages {{{languages_str}}} at {yml_path}."
        )
    ...
    @click.command(
        "create",
        help="Create a new Serena project configuration.",
        context_settings={"max_content_width": _MAX_CONTENT_WIDTH},
    )
    @click.argument(
        "project_path",
    ...
    @click.option(
        ...
        type=str,
        default=None,
        help="Project name; defaults to directory name if not specified.",
    )
    @click.option(
        "--language",
        type=str,
    ...
    @click.option(
        ...
        type=float,
        default=10,
        help="Timeout for indexing a single file (only used if --index is set).",
    )
    def create(
        project_path: str,
        name: str | None,
        ...
        try:
            registered_project = ProjectCommands._create_project(
                project_path, name, language
            )
            if index:
                click.echo("Indexing project...")
                ProjectCommands._index_project(
                    registered_project, log_level, timeout=timeout
                )
        except FileExistsError as e:
            raise click.ClickException(
                f"Project already exists: {e}\nUse 'serena project index' to index an existing project."
            )
    ...
    @click.option(
        ...
        type=str,
        default=None,
        help="Project name (only used if auto-creating project.yml).",
    )
    @click.option(
        "--language",
        type=str,
    ...
    @click.option(
        "--timeout", type=float, default=10, help="Timeout for indexing a single file."
    )
    def index(
        project: str,
        name: str | None,
        ...
        serena_config = SerenaConfig.from_config_file()
        registered_project = serena_config.get_registered_project(
            project, autoregister=True
        )
        if registered_project is None:
            # Project not found; auto-create it
            click.echo(
                f"No existing project found for '{project}'. Attempting auto-creation ..."
            )
            try:
                registered_project = ProjectCommands._create_project(
    ...
    @staticmethod
    def _index_project(
        registered_project: RegisteredProject, log_level: str, timeout: float
    ) -> None:
        lvl = logging.getLevelNamesMapping()[log_level.upper()]
        logging.configure(level=lvl)
        serena_config = SerenaConfig.from_config_file()
        ...
        try:
            log_file = os.path.join(
                proj.project_root, ".serena", "logs", "indexing.txt"
            )
            ...
            files_failed = []
            language_file_counts: dict[Language, int] = collections.defaultdict(
                lambda: 0
            )
            ...
            reported_language_file_counts = {
                k.value: v for k, v in language_file_counts.items()
            }
            click.echo(
                f"Indexed files per language: {dict_string(reported_language_file_counts, brackets=None)}"
            )
            ls_mgr.save_all_caches()
            ...
            if len(files_failed) > 0:
                ...
                with open(log_file, "w") as f:
                    for file, exception in zip(
                        files_failed, collected_exceptions, strict=True
                    ):
                        f.write(f"{file}\n")
                        f.write(f"{exception}\n")
                click.echo(
                    f"Failed to index {len(files_failed)} files, see:\n{log_file}"
                )
        finally:
            ls_mgr.stop_all()
    ...
    @click.argument(
        "path", type=click.Path(exists=False, file_okay=True, dir_okay=True)
    )
    @click.argument(
        "project",
    ...
    def is_ignored_path(path: str, project: str) -> None:
        ...
        click.echo(
            f"Path '{path}' IS {'ignored' if is_ignored else 'IS NOT ignored'} by the project configuration."
        )
    ...
    @click.argument(
        "file", type=click.Path(exists=True, file_okay=True, dir_okay=False)
    )
    @click.argument(
        "project",
        type=click.Path(exists=True, file_okay=False, dir_okay=True),
    ...
    @click.option(
        ...
        "-v",
        is_flag=True,
        help="Print detailed information about the indexed symbols.",
    )
    def index_file(file: str, project: str, verbose: bool) -> None:
        ...
        if proj.is_ignored_path(file, ignore_non_source_files=True):
            click.echo(
                f"'{file}' is ignored or declared as non-code file by the project configuration, won't index."
            )
            exit(1)
        ls_mgr = proj.create_language_server_manager()
        try:
            for ls in ls_mgr.iter_language_servers():
                ...
                if verbose:
                    ...
                    for symbol in symbols:
                        click.echo(
                            f"  - {symbol['name']} at line {symbol['selectionRange']['start']['line']} of kind {symbol['kind']}"
                        )
                ls.save_cache()
                click.echo(
                    f"Successfully indexed file '{file}', {len(symbols)} symbols saved to cache in {ls.cache_dir}."
                )
        finally:
            ls_mgr.stop_all()
    ...
    def health_check(project: str) -> None:
        ...
        with FileLoggerContext(log_file, append=False, enabled=True):
            ...
            try:
                ...
                overview_data = agent.execute_task(
                    lambda: overview_tool.get_symbol_overview(target_file)
                )
                ...
                log.info(
                    "Using symbol for testing: %s (kind: %s)", symbol_name, symbol_kind
                )
                ...
                find_symbol_result = agent.execute_task(
                    lambda: find_symbol_tool.apply(
                        symbol_name, relative_path=target_file, include_body=True
                    )
                )
                find_symbol_data = json.loads(find_symbol_result)
                log.info(
                    "FindSymbolTool found %d matches for symbol %s",
                    len(find_symbol_data),
                    symbol_name,
                )

                # Test 3: FindReferencingSymbolsTool
                log.info(
                    "Testing FindReferencingSymbolsTool for symbol: %s", symbol_name
                )
                try:
                    find_refs_result = agent.execute_task(
                        lambda: find_refs_tool.apply(
                            symbol_name, relative_path=target_file
                        )
                    )
                    find_refs_data = json.loads(find_refs_result)
                    log.info(
                        "FindReferencingSymbolsTool found %d references for symbol %s",
                        len(find_refs_data),
                        symbol_name,
                    )
                except Exception as e:
                    log.warning(
                        "FindReferencingSymbolsTool failed for symbol %s: %s",
                        symbol_name,
                        str(e),
                    )
                    find_refs_data = []
                ...
                try:
                    search_result = agent.execute_task(
                        lambda: search_pattern_tool.apply(
                            substring_pattern=symbol_name,
                            restrict_search_to_code_files=True,
                        )
                    )
                    search_data = json.loads(search_result)
                    pattern_matches = sum(
                        len(matches) for matches in search_data.values()
                    )
                    log.info(
                        "SearchForPatternTool found %d pattern matches for %s",
                        pattern_matches,
                        symbol_name,
                    )
                except Exception as e:
                    log.warning(
                        "SearchForPatternTool failed for pattern %s: %s",
                        symbol_name,
                        str(e),
                    )
                    pattern_matches = 0
                ...
                if tools_working:
                    ...
                    click.echo(
                        "⚠️  Health check completed with warnings - Check log for details"
                    )
...
class ToolCommands(AutoRegisteringGroup):
    ...
    @click.option(
        ...
        "include_optional",
        is_flag=True,
        help="List all tools, including those not enabled by default.",
    )
    @click.option(
        "--only-optional",
        is_flag=True,
        help="List only optional tools (those not enabled by default).",
    )
    def list(
        quiet: bool = False, include_optional: bool = False, only_optional: bool = False
    ) -> None:
        ...
        if quiet:
            ...
            ToolRegistry().print_tool_overview(
                include_optional=include_optional, only_optional=only_optional
            )
    ...
    @click.option(
        ...
        type=str,
        default=None,
        help="Context name or path to context file.",
    )
...
class PromptCommands(AutoRegisteringGroup):
    def __init__(self) -> None:
        super().__init__(
            name="prompts",
            help="Commands related to Serena's prompts that are outside of contexts and modes.",
        )
    ...
    @click.command(
        "list",
        help="Lists prompt names and YAML files that can be overridden.",
        context_settings={"max_content_width": _MAX_CONTENT_WIDTH},
    )
    def list() -> None:
        # list prompt names
        ...
        serena_prompt_yaml_names = [
            os.path.basename(f)
            for f in glob.glob(PROMPT_TEMPLATES_DIR_INTERNAL + "/*.yml")
        ]
        for prompt_yaml_name in serena_prompt_yaml_names:
            ...
            if os.path.exists(user_prompt_yaml_path):
                click.echo(
                    f" * {user_prompt_yaml_path} merged with default prompts in {prompt_yaml_name}"
                )
            else:
    ...
    def create_override(prompt_yaml_name: str) -> None:
        ...
        serena_prompt_yaml_path = os.path.join(
            PROMPT_TEMPLATES_DIR_INTERNAL, prompt_yaml_name
        )
    ...
    @click.command(
        "edit-override",
        help="Edit an existing prompt override file",
        context_settings={"max_content_width": _MAX_CONTENT_WIDTH},
    )
    @click.argument("prompt_yaml_name")
    def edit_override(prompt_yaml_name: str) -> None:
        ...
        if not os.path.exists(user_prompt_yaml_path):
            click.echo(
                f"Override file '{prompt_yaml_name}' not found. Create it with: prompts create-override {prompt_yaml_name}"
            )
            return
        _open_in_editor(user_prompt_yaml_path)
    ...
    @click.command(
        "list-overrides",
        help="List existing prompt override files",
        context_settings={"max_content_width": _MAX_CONTENT_WIDTH},
    )
    ...
    @click.command(
        "delete-override",
        help="Delete a prompt override file",
        context_settings={"max_content_width": _MAX_CONTENT_WIDTH},
    )
    @click.argument("prompt_yaml_name")
