...
class SerenaPaths:
    ...
    def __init__(self) -> None:
        ...
        """
        self.user_prompt_templates_dir: str = os.path.join(
            self.serena_user_home_dir, "prompt_templates"
        )
        """
        ...
        """
        self.user_contexts_dir: str = os.path.join(
            self.serena_user_home_dir, "contexts"
        )
        """
        ...
        """
        self.news_legacy_last_read_id_file: str = os.path.join(
            self.serena_user_home_dir, "last_read_news_snippet_id.txt"
        )
        """
        file containing the ID of the last read news snippet
        """
        self.news_read_items_file: str = os.path.join(
            self.serena_user_home_dir, "news_read.pkl"
        )
        """
        file containing the ID of the last read news snippet
        """
        self.news_etag_file: str = os.path.join(
            self.serena_user_home_dir, "news_etag.txt"
        )
        """
        ...
        """
        global_memories_path = Path(
            os.path.join(self.serena_user_home_dir, "memories", "global")
        )
    ...
    def get_next_log_file_path(self, prefix: str) -> str:
        ...
        log_dir = os.path.join(
            self.serena_user_home_dir, "logs", datetime.now().strftime("%Y-%m-%d")
        )
        os.makedirs(log_dir, exist_ok=True)
        self.last_returned_log_file_path = os.path.join(
            log_dir, prefix + "_" + datetime_tag() + f"_{os.getpid()}" + ".txt"
        )
    ...
    def from_str(backend_str: str) -> "LanguageBackend":
        for backend in LanguageBackend:
            if backend.value.lower() == backend_str.lower():
                return backend
        raise ValueError(
            f"Unknown language backend '{backend_str}': valid values are {[b.value for b in LanguageBackend]}"
        )
...
class LineEnding(Enum):
    ...
    def from_str(cls, value: str) -> "LineEnding":
        ...
        try:
            ...
            raise ValueError(
                f"Invalid line_ending: {value!r}. Valid values are: {valid}"
            ) from e
...
class ProjectConfig(SharedConfig, ModeSelectionDefinitionWithAddedModes):
    ...
    def autogenerate(
        ...
        with LogTime("Project configuration auto-generation", logger=log):
            ...
            if languages is None:
                ...
                language_composition = determine_programming_language_composition(
                    str(project_root)
                )
                ...
                if len(language_composition) == 0:
                    ...
                    if len(other_language_pairs) > 0 and interactive:
                        ...
                        print(
                            f"Additionally detected {len(other_language_pairs)} other language(s).\n"
                        )
                        print(
                            "Note: Enable only languages you need symbolic retrieval/editing capabilities for."
                        )
                        print(
                            "      Additional language servers use resources and some languages may require additional"
                        )
                        print(
                            "      system-level installations/configuration (see Serena documentation)."
                        )
                        print("\nWhich additional languages do you want to enable?")
                        for lang, perc in other_language_pairs:
                            enable = ask_yes_no(
                                "Enable %s (%.2f%% of source files)?"
                                % (lang.value, perc),
                                default=False,
                            )
                            if enable:
            ...
            if save_to_disk:
                project_yml_path = serena_config.get_project_yml_location(
                    str(project_root)
                )
                log.info("Saving project configuration to %s", project_yml_path)
                save_yaml(project_yml_path, config_with_comments)
                project_local_yml_path = os.path.join(
                    os.path.dirname(project_yml_path), cls.SERENA_LOCAL_PROJECT_FILE
                )
    ...
    def default_project_yml_path(cls, project_root: str | Path) -> str:
        ...
        """
        return os.path.join(
            str(project_root), SERENA_MANAGED_DIR_NAME, cls.SERENA_PROJECT_FILE
        )
    ...
    def _from_dict(cls, data: dict[str, Any], local_override_keys: list[str]) -> Self:
        ...
        if symbol_info_budget is not None:
            try:
                ...
                raise ValueError(
                    f"symbol_info_budget must be a number or null, got: {symbol_info_budget_raw}"
                ) from e
            if symbol_info_budget < 0:
                raise ValueError(
                    f"symbol_info_budget cannot be negative, got: {symbol_info_budget}"
                )

        language_backend_value = data.get("language_backend")
        language_backend = (
            LanguageBackend.from_str(language_backend_value)
            if language_backend_value
            else None
        )

        line_ending_value = data.get("line_ending")
        line_ending = (
            LineEnding.from_str(line_ending_value) if line_ending_value else None
        )
        ...
        if "base_modes" in data and data["base_modes"] is not None:
            log.warning(
                "The base_modes setting in project.yml is deprecated and will be ignored."
            )
    ...
    def _to_yaml_dict(self) -> dict:
        ...
        d["language_backend"] = (
            self.language_backend.value if self.language_backend is not None else None
        )
        d["line_ending"] = (
            self.line_ending.value if self.line_ending is not None else None
        )
    ...
    def load(
        ...
        if os.path.exists(local_yaml_path):
            local_yaml_data, _ = cls._load_yaml_dict(
                local_yaml_path, apply_defaults=False
            )
            if local_yaml_data:
        ...
        project_config = cls._from_dict(
            yaml_data, local_override_keys=local_override_keys
        )

        # if the configuration was incomplete, re-save it to disk
        if not was_complete:
            log.info(
                "Project configuration in %s was incomplete, re-saving with default values for missing fields",
                yaml_path,
            )
    ...
    def save(self, project_yml_path: str, save_project_local_yml: bool = True) -> None:
        ...
        config_with_comments, _ = self._load_yaml_dict(
            config_path, self.YAML_COMMENT_NORMALISATION
        )
        for key in cur_dict:
        ...
        template_config, _ = self._load_yaml_dict(
            PROJECT_TEMPLATE_FILE, self.YAML_COMMENT_NORMALISATION
        )
        transfer_yaml_comments(
            template_config,
            config_with_comments,
            self.YAML_COMMENT_NORMALISATION,
        ...
        if save_project_local_yml:
            ...
            if self._local_override_keys and os.path.exists(project_local_yml_path):
                log.info(
                    "Saving updated local project configuration to %s",
                    project_local_yml_path,
                )
                local_config_with_comments, _ = self._load_yaml_dict(
                    project_local_yml_path,
                    comment_normalisation=YamlCommentNormalisation.NONE,
...
class RegisteredProject(ToStringMixin):
    ...
    def from_project_root(
        cls, project_root: str | Path, serena_config: "SerenaConfig"
    ) -> "RegisteredProject":
...
class SerenaConfig(SharedConfig, ModeSelectionDefinitionWithBaseModes):
    ...
    def _determine_config_file_path(cls) -> str:
        ...
        if not os.path.exists(config_path):
            ...
            if os.path.exists(old_config_path):
                log.info(
                    f"Moving Serena configuration file from {old_config_path} to {config_path}"
                )
    ...
    def from_config_file(cls, generate_if_missing: bool = True) -> "SerenaConfig":
        ...
        if not os.path.exists(config_file_path):
            if not generate_if_missing:
                raise FileNotFoundError(
                    f"Serena configuration file not found: {config_file_path}"
                )
            log.info(
                f"Serena configuration file not found at {config_file_path}, autogenerating..."
            )
        ...
        try:
            ...
            raise ValueError(
                f"Error loading Serena configuration from {config_file_path}: {e}"
            ) from e

        # create the configuration instance
        instance = cls(
            _loaded_commented_yaml=loaded_commented_yaml,
        ...
        def get_value_or_default(field_name: str) -> Any:
            ...
            if field_name not in loaded_commented_yaml:
                num_migrations += 1
            return loaded_commented_yaml.get(
                field_name, get_dataclass_default(SerenaConfig, field_name)
            )

        # transfer regular fields that do not require type conversion
        for (
            field_name
        ) in instance._iter_config_file_mapped_fields_without_type_conversion():
        ...
        for path in loaded_commented_yaml["projects"]:
            ...
            if not path_exists or (
                path.is_dir()
                and not os.path.isfile(instance.get_project_yml_location(str(path)))
            ):
                log.warning(
                    f"Project path {path} does not exist or no associated project configuration file found, skipping."
                )
                continue
            if path.is_file():
            ...
            try:
                project_config = ProjectConfig.load(
                    path, serena_config=instance
                )  # instance is sufficiently populated
            except Exception as e:
                log.error(
    ...
    def get_registered_project(
        self, project_root_or_name: str, autoregister: bool = False
        ...
        if autoregister:
            ...
            if os.path.isfile(config_path):
                registered_project = RegisteredProject.from_project_root(
                    project_root_or_name, serena_config=self
                )
    ...
    def add_project_from_path(self, project_root: Path | str) -> "Project":
        ...
        project_config = ProjectConfig.load(
            project_root, serena_config=self, autogenerate=True
        )
    ...
    def save(self) -> None:
        ...
        assert (
            self._loaded_commented_yaml is not None
        ), "Cannot save configuration without loaded YAML"
        ...
        for (
            field_name
        ) in self._iter_config_file_mapped_fields_without_type_conversion():
            commented_yaml[field_name] = getattr(self, field_name)

        # convert project objects into list of paths
        commented_yaml["projects"] = sorted(
        ...
        template_yaml = load_yaml(
            SERENA_CONFIG_TEMPLATE_FILE,
            comment_normalisation=YamlCommentNormalisation.LEADING,
        )
        transfer_yaml_comments(
            template_yaml,
            commented_yaml,
            YamlCommentNormalisation.LEADING,
            force_update_all=True,
        )
    ...
    @staticmethod
    def _resolve_serena_folder_location(
    ...
    def get_configured_project_serena_folder(self, project_root: str | Path) -> str:
        ...
        return self._resolve_serena_folder_location(
            self.project_serena_folder_location, placeholders
        )
