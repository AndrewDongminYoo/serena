...
class MemoriesManager:
    ...
    def __init__(
        ...
        self._read_only_memory_patterns = [
            re.compile(pattern) for pattern in set(read_only_memory_patterns)
        ]
        self._ignored_memory_patterns = [
    ...
    def get_memory_file_path(self, name: str) -> Path:
        ...
        if ".." in parts:
            raise ValueError(
                f"Memory name cannot contain '..' segments for security reasons. Got: {name}"
            )
        ...
        assert (
            self._project_memory_dir is not None
        ), "Project dir was not passed at initialization"
    ...
    def _list_memories(
        self, search_dir: Path, base_dir: Path, prefix: str = ""
    ) -> MemoriesList:
        result = self.MemoriesList()
        if not search_dir.exists():
            return result
        for md_file in search_dir.rglob("*.md"):
            rel = str(md_file.relative_to(base_dir).with_suffix("")).replace(
                os.sep, "/"
            )
    ...
    def list_global_memories(self, subtopic: str = "") -> MemoriesList:
        ...
        return self._list_memories(
            dir_path, self._global_memory_dir, self.GLOBAL_TOPIC + "/"
        )
...
class Project(ToStringMixin):
    def __init__(
        ...
        *,
        ...
        self._serena_data_folder = serena_config.get_project_serena_folder(
            self.project_root
        )
        log.info("Serena project data folder: %s", self._serena_data_folder)

        read_only_memory_patterns = (
            serena_config.read_only_memory_patterns
            + project_config.read_only_memory_patterns
        )
        ignored_memory_patterns = (
            serena_config.ignored_memory_patterns
            + project_config.ignored_memory_patterns
        )
        ...
        serena_data_gitignore_path = os.path.join(
            self._serena_data_folder, ".gitignore"
        )
        ...
        with LogTime(
            ...
            try:
                ...
                if len(self.project_config.ignored_paths) > 0:
                    ...
                    log.debug(
                        f"Project ignored paths: {self.project_config.ignored_paths}"
                    )
    ...
    def create_language_server_manager(self) -> LanguageServerManager:
        ...
        try:
            ...
            ls_specific_settings = {
                **self.serena_config.ls_specific_settings,
                **self.project_config.ls_specific_settings,
            }
    ...
    def get_language_server_manager_or_raise(self) -> LanguageServerManager:
        if self.language_server_manager is None:
            ...
            if self._agent is not None:
                msg.with_text(
                    "For details, please check the logs. "
                    + self._agent.get_log_inspection_instructions()
                )
            msg.with_text(
