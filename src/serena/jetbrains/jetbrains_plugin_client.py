...
class JetBrainsPluginClientManager:
    ...
    def match_clients(
        self, registered_projects: list[RegisteredProject]
    ) -> list[MatchedClient]:
        """
    ...
    def get_matched_client(
        ...
        def find_match() -> Optional["JetBrainsPluginClient"]:
            for matched_client in self._matched_clients:
                if (
                    matched_client.registered_project.project_root
                    == registered_project.project_root
                ):
...
class JetBrainsPluginClient(ToStringMixin):
    ...
    def __init__(self, port: int, timeout: int = PLUGIN_REQUEST_TIMEOUT):
        ...
        try:
            status_response: PluginStatusDTO = cast(
                jb.PluginStatusDTO, self._make_request("GET", "/status")
            )
            ...
            log.warning(
                "Failed to obtain status from JetBrains plugin service at port %d: %s",
                port,
                e,
                exc_info=e,
            )
        ...
        try:
            if method.upper() == "GET":
                ...
                response = self._session.post(
                    url, data=json_data, timeout=self._timeout
                )
            else:
            ...
            if response is not None:
                ...
                if self.is_version_at_least(2023, 2, 6):
                    ...
                    is_recoverable_error = (
                        True  # assume recoverable for older versions (mix of errors)
                    )
    ...
    def _postprocess_symbol_collection_response(
        self, response_dict: jb.SymbolCollectionResponse
    ) -> None:
        """
        ...
        def convert_html(
            key: Literal["documentation", "quick_info"], symbol: jb.SymbolDTO
        ) -> None:
            if key in symbol:
    ...
    def find_symbol(
        ...
        symbol_collection = cast(
            jb.SymbolCollectionResponse,
            self._make_request("POST", "/findSymbol", request_data),
        )
    ...
    def find_references(
        self, name_path: str, relative_path: str, include_quick_info: bool
    ) -> jb.SymbolCollectionResponse:
        """
        ...
        symbol_collection = cast(
            jb.SymbolCollectionResponse,
            self._make_request("POST", "/findReferences", request_data),
        )
        self._postprocess_symbol_collection_response(symbol_collection)
        return symbol_collection
    ...
    def get_symbols_overview(
        ...
        request_data = {
            "relativePath": relative_path,
            "depth": depth,
            "includeFileDocumentation": include_file_documentation,
        }
        response = cast(
            jb.GetSymbolsOverviewResponse,
            self._make_request("POST", "/getSymbolsOverview", request_data),
        )
    ...
    def get_supertypes(
        ...
        return cast(
            jb.TypeHierarchyResponse,
            self._make_request("POST", "/getSupertypes", request_data),
        )
    ...
    def get_subtypes(
        ...
        return cast(
            jb.TypeHierarchyResponse,
            self._make_request("POST", "/getSubtypes", request_data),
        )
    ...
    def rename_symbol(
        ...
        rename_in_text_occurrences: bool,
    ) -> None:
        """
        Renames a symbol.
    ...
    def find_declaration(
        ...
        symbol_collection = cast(
            jb.SymbolCollectionResponse,
            self._make_request("POST", "/findDeclaration", request_data),
        )
        self._postprocess_symbol_collection_response(symbol_collection)
        return symbol_collection

    def find_implementations(
        self, relative_path: str, name_path: str, include_quick_info: bool
    ) -> jb.SymbolCollectionResponse:
        """
        ...
        symbol_collection = cast(
            jb.SymbolCollectionResponse,
            self._make_request("POST", "/findImplementations", request_data),
        )
        self._postprocess_symbol_collection_response(symbol_collection)
        return symbol_collection
    ...
    def run_inspections(
        ...
        return cast(
            jb.RunInspectionsResponse,
            self._make_request("POST", "/runInspectionsOnFile", request_data),
        )
    ...
    def list_inspections(
        ...
        return cast(
            jb.ListInspectionsResponse,
            self._make_request("POST", "/listInspections", request_data),
        )
