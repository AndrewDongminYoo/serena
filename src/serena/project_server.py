...
class ProjectServer:
    ...
    def _get_project(self, project_root_or_name: str) -> "Project":
        ...
        if registered_project is None:
            raise ValueError(
                f"Project '{project_root_or_name}' is not registered with Serena."
            )
    ...
    def _query_project(self, req: QueryProjectRequest) -> str:
        ...
        with self._agent.active_project_context(project):
            ...
            if not tool.is_readonly():
                raise ValueError(
                    f"Tool '{req.tool_name}' is not read-only and cannot be executed via the query_project route"
                )
    ...
    def __init__(
        ...
        try:
            ...
            raise ConnectionError(
                f"ProjectServer is not reachable at {self._base_url}. Make sure the server is running."
            )
        except requests_lib.RequestException as e:
            raise ConnectionError(f"ProjectServer health check failed: {e}")

    def query_project(
        self, project_name: str, tool_name: str, tool_params_json: str
    ) -> str:
        """
        ...
        response = requests_lib.post(
            f"{self._base_url}/query_project", json=payload, timeout=self._timeout
        )
        response.raise_for_status()
