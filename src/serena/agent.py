...
from serena.dashboard import (
    SerenaDashboardAPI,
    SerenaDashboardTrayManager,
    SerenaDashboardViewer,
    open_url_in_browser,
)
...
class ToolSet:
    ...
    def apply(
        self, *tool_inclusion_definitions: "ToolInclusionDefinition"
    ) -> "ToolSet":
        """
        ...
        def get_updated_tool_name(tool_name: str) -> str:
            ...
            if tool_name in self.LEGACY_TOOL_NAME_MAPPING:
                ...
                log.warning(
                    "Tool name '%s' is deprecated, please use '%s' instead",
                    tool_name,
                    new_tool_name,
                )
        ...
        for definition in tool_inclusion_definitions:
            if definition.is_fixed_tool_set():
                ...
                for fixed_tool in definition.fixed_tools:
                    ...
                    if registry.check_valid_tool_name(
                        fixed_tool, " (in fixed tools set)"
                    ):
                        tool_names.add(fixed_tool)
                log.info(
                    f"{definition} defined a fixed tool set with {len(tool_names)} tools: {', '.join(tool_names)}"
                )
            else:
                included_tools = []
                excluded_tools = []
                for included_tool in definition.included_optional_tools:
                    ...
                    if (
                        registry.check_valid_tool_name(
                            included_tool, " (in included optional tools)"
                        )
                        and included_tool not in tool_names
                    ):
                ...
                for excluded_tool in definition.excluded_tools:
                    ...
                    registry.check_valid_tool_name(
                        excluded_tool, " (in excluded tools)"
                    )
                ...
                if included_tools:
                    log.info(
                        f"{definition} included {len(included_tools)} tools: {', '.join(included_tools)}"
                    )
                if excluded_tools:
                    log.info(
                        f"{definition} excluded {len(excluded_tools)} tools: {', '.join(excluded_tools)}"
                    )
    ...
    def to_available_tools(self, all_tools: dict[type[Tool], Tool]) -> AvailableTools:
        return AvailableTools(
            [t for t in all_tools.values() if self.includes_name(t.get_name())]
        )
...
class ActiveModes:
    ...
    def apply(self, mode_selection: ModeSelectionDefinition) -> None:
        ...
        self._dynamically_activated_mode_names = (
            set(self._configured_default_modes or []) | self._added_modes
        )
        self._active_mode_names = sorted(
            set(self._configured_base_modes or [])
            | self._dynamically_activated_mode_names
        )
    ...
    def get_modes(self) -> Sequence[SerenaAgentMode]:
        return [
            self.get_mode_instance(mode_name) for mode_name in self._active_mode_names
        ]

    def get_dynamically_activated_modes(self) -> Sequence[SerenaAgentMode]:
        return [
            self.get_mode_instance(mode_name)
            for mode_name in self._dynamically_activated_mode_names
        ]

    def get_base_modes(self) -> Sequence[SerenaAgentMode]:
        return [
            self.get_mode_instance(mode_name)
            for mode_name in self._configured_base_modes or []
        ]
...
class ProjectPromptProvisionStatus:
    ...
    def __init__(self, newly_activated_mode_names: set[str] | None = None):
        ...
        self._session_status_dict: dict[
            str, ProjectPromptProvisionStatus.SessionStatus
        ] = defaultdict(lambda: self.SessionStatus())
    ...
    def get_modes_with_prompts_to_be_provided_for_project_activation(
        self, session_id: str
...
class DashboardManager:
    ...
    def __init__(
        ...
        if mode_str is not None:
            try:
                ...
                log.warning(
                    f"Invalid dashboard interface mode '{mode_str}' specified; falling back to platform default '{mode.value}'."
                )
        else:
            mode = self.Mode.from_platform()
        ...
        match self._mode:
            ...
            case self.Mode.TRAY_MANAGER:
                init_fn = lambda: self._tray_manager_register(
                    open_on_launch=open_dashboard_on_launch,
                    active_project=active_project,
                )
                threading.Thread(
                    target=init_fn, name="init-DashboardTrayManager", daemon=True
                ).start()
            case self.Mode.BROWSER:
                if open_dashboard_on_launch:
                    if not system_has_usable_display():
                        log.info(
                            "Not opening the Serena dashboard because no usable display was detected."
                        )
                    else:
    ...
    @staticmethod
    def _start_dashboard_viewer_process_function(
        url: str, minimized: bool, parent_process_id: int
    ) -> None:
        """
        ...
        try:
            SerenaDashboardViewer(
                url, start_minimized=minimized, parent_process_id=parent_process_id
            ).run()
    ...
    def _start_dashboard_viewer(self, minimized: bool) -> None:
        ...
        self._dashboard_viewer_process = multiprocessing.Process(
            target=self._start_dashboard_viewer_process_function,
            args=(self.url, minimized, os.getpid()),
            daemon=True,
        )
        self._dashboard_viewer_process.start()

    def _tray_manager_register(
        self, open_on_launch: bool, active_project: Project | None
    ) -> None:
        """
        ...
        with LogTime("Dashboard tray manager initialisation"):
            with self._tray_manager_lock:
                ...
                project_name = (
                    active_project.project_name if active_project is not None else None
                )
    ...
    def update_active_project(self, active_project: Project | None) -> None:
        ...
        if self._mode == self.Mode.TRAY_MANAGER:
            with self._tray_manager_lock:
                ...
                SerenaDashboardTrayManager.update_project(
                    port=self._port, project=project_name
                )
...
class SerenaAgent:
    def __init__(
        ...
        registered_project_to_activate: RegisteredProject | None = (
            self.serena_config.get_registered_project(project, autoregister=True)
        ...
        log.info(
            f"Will record tool usage statistics with token count estimator: {token_count_estimator.name}."
        )
        ...
        if (
            registered_project_to_activate is not None
            and registered_project_to_activate.project_config.language_backend
            is not None
        ):
            ...
            log.info(
                f"Using language backend as configured in project.yml: {self._language_backend.name}"
            )
        else:
            log.info(
                f"Using language backend from global configuration: {self._language_backend.name}"
            )
        ...
        if project is not None:
            try:
                self.activate_project_from_path_or_name(
                    project, update_active_modes=False, update_active_tools=False
                )
        ...
        log.info(
            f"Number of exposed tools: {len(self._exposed_tools)}. Exposed tools: {self._exposed_tools.tool_names}"
        )
    ...
    def _send_usage_info(self) -> None:
        ...
        try:
            requests.get(
                "https://oraios-software.de/serena_usage.php", params=params, timeout=1
            )
        except Exception as e:
    ...
    def _create_base_toolset(
        ...
        if (
            serena_config.web_dashboard
            and not serena_config.web_dashboard_open_on_launch
            and not serena_config.gui_log_window
        ):
    ...
    def open_dashboard(self) -> bool:
        ...
        if not system_has_usable_display():
            log.warning(
                "Not opening the Serena web dashboard because no usable display was detected."
            )
            return False
    ...
    def create_system_prompt(self, session_id: str = "global") -> str:
        ...
        global_memories = MemoriesManager(
            serena_data_folder=None,
            read_only_memory_patterns=self.serena_config.read_only_memory_patterns,
        ).list_global_memories()
        global_memories_str = (
            dict_string(global_memories.to_dict()) if len(global_memories) > 0 else ""
        )
        log.info(
            "Generating system prompt with available_tools=(see active tools), available_markers=%s",
            available_markers,
        )
        ...
        for mode in self.get_active_modes():
            if mode.has_prompt():
                if not self._project_prompt_status.is_mode_prompt_already_provided(
                    mode.name, session_id
                ):
        ...
        system_prompt = self.prompt_factory.create_system_prompt(
            ...
            mode_system_prompts=[
                self._format_prompt(mode.prompt) for mode in relevant_modes
            ],
        ...
        if (
            ...
            and not self._project_prompt_status.is_project_activation_message_already_provided(
                session_id
            )
        ):
    ...
    def get_project_activation_message(self, session_id: str) -> str:
        ...
        if self._project_prompt_status.is_project_activation_message_already_provided(
            session_id
        ):
        ...
        if self._language_backend == LanguageBackend.LSP:
            languages_str = ", ".join(
                [lang.value for lang in proj.project_config.languages]
            )
    ...
    def _update_active_modes(self, log_message: bool = True) -> None:
        ...
        if log_message:
            ...
            log.info(
                f"Active modes ({len(active_mode_names)}): {', '.join(active_mode_names)}"
            )
    ...
    def _update_active_tools(self) -> None:
        ...
        log.info(
            f"Active tools ({len(self._active_tools)}): {', '.join(self._active_tools.tool_names)}"
        )

        # check if a tool was activated that is not in the exposed tool set and issue a warning if so
        active_tools_not_exposed = set(self._active_tools.tool_names) - set(
            self._exposed_tools.tool_names
        )
    ...
    def _activate_project(
        ...
        if (
            self._active_project is not None
            and self._active_project.project_root == project.project_root
        ):
            return False
        ...
        if self._active_project is not None:
            log.info(
                f"Shutting down previously active project '{self._active_project.project_name}' before switching"
            )
        ...
        if update_active_modes:
            ...
            self._update_active_modes()
            newly_activated_mode_names = (
                set(self._active_modes.get_mode_names()) - active_mode_names_before
            )
        else:
            newly_activated_mode_names = None

        self._project_prompt_status = ProjectPromptProvisionStatus(
            newly_activated_mode_names=newly_activated_mode_names
        )
    ...
    def get_current_config_overview(self) -> str:
        ...
        if (
            self._active_project
            and self._active_project.project_config.language_backend is not None
        ):
            result_str += " (project override)"
        result_str += (
            f" (global default: {self.serena_config.language_backend.value})\n"
        )
        result_str += (
            "Available projects:\n"
    ...
    def add_language(self, language: Language) -> None:
        ...
        """
        self.execute_task(
            lambda: self.get_active_project_or_raise().add_language(language),
            name=f"AddLanguage:{language.value}",
        )
    ...
    def remove_language(self, language: Language) -> None:
        ...
        """
        self.issue_task(
            lambda: self.get_active_project_or_raise().remove_language(language),
            name=f"RemoveLanguage:{language.value}",
        )
    ...
    def on_shutdown(self, timeout: float = 2.0) -> None:
        ...
        if self._active_project is not None:
            log.info(
                f"Shutting down active project '{self._active_project.project_name}' ..."
            )
